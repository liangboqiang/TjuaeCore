...
use tjuaeui_common::constants::COOKIE_MAX_AGE_DAYS;
use tjuaeui_db::{DbError, IUserRepository, models::User};
...
impl From<AuthError> for ApiError {
    fn from(err: AuthError) -> Self {
        match err {
            AuthError::InvalidCredentials => ApiError::Unauthorized("用户名或密码无效".into()),
            AuthError::WeakPassword(msg) => ApiError::BadRequest(msg),
            AuthError::InvalidUsername(msg) => ApiError::BadRequest(msg),
            AuthError::TokenExpired => ApiError::Unauthorized("令牌已过期".into()),
            AuthError::TokenInvalid(msg) => ApiError::Unauthorized(msg),
            AuthError::TokenBlacklisted => ApiError::Unauthorized("令牌已撤销".into()),
...
async fn login_handler(
    ...
    if req.username.len() > 32 {
        return Err(ApiError::BadRequest("用户名不得超过 32 个字符".into()));
    }
    if req.password.len() > 128 {
        return Err(ApiError::BadRequest("密码不得超过 128 个字符".into()));
    }
    ...
    if !password_valid {
        return Err(ApiError::Unauthorized("用户名或密码无效".into()));
    }

    let user = found_user.ok_or_else(|| ApiError::Unauthorized("用户名或密码无效".into()))?;
...
async fn change_password_handler(
    ...
    let user = state
        ...
        .await
        .map_err(|e| ApiError::Internal(format!("Database error: {e}")))?
        .ok_or_else(|| ApiError::NotFound("找不到用户".into()))?;
...
async fn refresh_handler(
    ...
    let payload = state
        .jwt_service
        .verify(&req.token)
        .map_err(|_| ApiError::Unauthorized("令牌无效或已过期".into()))?;
...
async fn ws_token_handler(
    ...
    let token = extract_token_from_headers(&headers).ok_or_else(|| ApiError::Unauthorized("未找到令牌".into()))?;
    ...
    state
        ...
        .await
        .map_err(|e| ApiError::Internal(format!("Database error: {e}")))?
        .ok_or_else(|| ApiError::Unauthorized("找不到用户".into()))?;
...
async fn qr_login_handler(
    ...
    let user = state
        ...
        .await
        .map_err(|e| ApiError::Internal(format!("Database error: {e}")))?
        .ok_or_else(|| ApiError::Internal("尚未配置主用户".into()))?;
...
<div class="card">
  <h1>TjuaeUI</h1>
  <p id="status" class="status">正在处理登录……</p>
</div>
<script>
(function() {
...
async fn resolve_webui_admin(user_repo: &dyn IUserRepository) -> Result<User, ApiError> {
    user_repo
        ...
        .await
        .map_err(|e| ApiError::Internal(format!("Database error: {e}")))?
        .ok_or_else(|| ApiError::NotFound("尚未配置 WebUI 管理员用户".into()))
}
