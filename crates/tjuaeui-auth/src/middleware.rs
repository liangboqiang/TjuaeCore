...
use tjuaeui_db::IUserRepository;
...
///
/// Flow:
/// 1. Extract bearer token from `Authorization` header or `tjuaeui-session` cookie
...
pub async fn auth_middleware(
    ...
    let token =
        extract_token_from_headers(request.headers()).ok_or_else(|| ApiError::Unauthorized("需要身份验证".into()))?;

    let payload = state.jwt_service.verify(&token).map_err(|e| {
        tracing::debug!("Token verification failed: {e}");
        ApiError::Unauthorized("令牌无效或已过期".into())
    })?;
    ...
    let user = state
        ...
        .map_err(|e| {
            ...
            ApiError::Internal("身份验证服务不可用".into())
        })?
        .ok_or_else(|| ApiError::Unauthorized("身份验证主体无效".into()))?;
