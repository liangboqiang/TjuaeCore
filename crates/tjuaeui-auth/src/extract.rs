...
use tjuaeui_common::constants::COOKIE_NAME;
...
///
/// Priority: `Authorization: Bearer <token>` > `tjuaeui-session` cookie.
...
mod tests {
    ...
    fn token_from_cookie() {
        let headers = headers_with(&[("cookie", "tjuaeui-session=cookie_token; other=val")]);
    ...
    fn token_header_takes_priority_over_cookie() {
        let headers = headers_with(&[
            ...
            ("cookie", "tjuaeui-session=cookie_token"),
        ]);
    ...
    fn ws_token_from_cookie() {
        let headers = headers_with(&[("cookie", "tjuaeui-session=ws_cookie")]);
    ...
    fn cookie_value_malformed_before_target() {
        ...
        let headers = headers_with(&[("cookie", "bad_entry; tjuaeui-session=tok123")]);
        assert_eq!(extract_cookie_value(&headers, "tjuaeui-session"), Some("tok123".into()));
    }
    ...
    fn token_from_cookie_with_malformed_entries() {
        ...
        let headers = headers_with(&[("cookie", "garbage; tjuaeui-session=abc; nope")]);
