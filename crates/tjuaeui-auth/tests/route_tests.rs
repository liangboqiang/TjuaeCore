...
use tjuaeui_auth::{AuthRouterState, CookieConfig, JwtService, QrTokenStore, auth_routes, hash_password};
...
async fn t4_1_login_success() {
    ...
    assert!(set_cookie.contains("tjuaeui-session="));
    assert!(set_cookie.contains("HttpOnly"));

    let json = body_json(resp).await;
    assert_eq!(json["success"], true);
    assert_eq!(json["message"], "登录成功");
