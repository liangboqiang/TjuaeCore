...
async fn repo() -> (Arc<dyn ITeamRepository>, tjuaeui_db::Database) {
...
async fn update_team_can_patch_workspace() {
    ...
    repo.update_team(
        "t1",
        &UpdateTeamParams {
            workspace: Some("/tmp/tjuaeui-team-shared-workspace".into()),
            ..Default::default()
        },
    )
    .await
    ...
    assert_eq!(updated.workspace, "/tmp/tjuaeui-team-shared-workspace");
}
