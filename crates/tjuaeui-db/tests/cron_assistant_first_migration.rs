...
async fn seed_legacy_assistant_identity(pool: &sqlx::SqlitePool) {
    ...
    for (id, backend, agent_type, name, source, sort_order) in [
        ("agent-tjuaecli", "", "tjuaecli", "TjuaeCLI", "internal", 100),
    ...
    for (definition_id, assistant_key, agent_backend, source_ref) in [
        ("def-tjuaecli", "tjuaeui-assistant", "tjuaecli", "tjuaeui-assistant"),
...
#[tokio::test]
async fn migration_015_populates_tjuae_cli_catalog_by_agent_type() {
    ...
    sqlx::query(
        "INSERT INTO agent_metadata (
            id, name, backend, command, agent_type, enabled, agent_source, sort_order, created_at, updated_at
         ) VALUES ('agent-tjuaecli', 'TjuaeCLI', NULL, '', 'tjuaecli', 1, 'internal', 100, 1, 1)",
    )
    .execute(&pool)
    .await
    .unwrap();
...
#[tokio::test]
async fn migration_016_clears_internal_tjuae_cli_overrides_only() {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
...
async fn migration_013_normalizes_legacy_cron_agent_identity() {
    ...
    insert_legacy_cron(
        ...
        "conv_tjuaecli",
        "tjuaecli",
        r#"{"backend":"provider-1","name":"Tjuae","assistant_id":"tjuaeui-assistant","model_id":"gpt-5"}"#,
    )
    .await;
    insert_legacy_cron(
        &pool,
    ...
    let tjuae_cli = sqlx::query(
        "SELECT
            ...
            enabled
         FROM cron_jobs WHERE id = 'cron_tjuaecli'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    assert_eq!(tjuae_cli.get::<String, _>("assistant_id"), "tjuaeui-assistant");
    assert_eq!(tjuae_cli.get::<String, _>("provider_id"), "provider-1");
    assert_eq!(tjuae_cli.get::<String, _>("model"), "gpt-5");
    assert_eq!(tjuae_cli.get::<i64, _>("enabled"), 1);
