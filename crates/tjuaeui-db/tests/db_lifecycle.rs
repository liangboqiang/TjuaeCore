use sqlx::Row;
use tjuaeui_db::{DatabaseInitOptions, init_database, init_database_memory, init_database_with_options};
...
async fn system_user_has_valid_timestamps() {
    let before = tjuaeui_common::now_ms();
    let db = init_database_memory().await.unwrap();
    let after = tjuaeui_common::now_ms();
...
//
// Repro for the concurrent-startup secondary symptom: two processes opening the same
...
// `tjuaecore doctor` racing the server) both decide to apply the same
...
fn concurrent_init_database_does_not_panic_on_unique_conflict() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("tjuaeui-backend.db");
    ...
    let lock = path.with_file_name("tjuaeui-backend.db.migrate.lock");
