...
async fn repo() -> (Arc<dyn IRemoteAgentRepository>, tjuaeui_db::Database) {
...
async fn full_crud_lifecycle() {
    ...
    // Update status
    r.update_status(&created.id, "connected", Some(tjuaeui_common::now_ms()))
        .await
        .unwrap();
