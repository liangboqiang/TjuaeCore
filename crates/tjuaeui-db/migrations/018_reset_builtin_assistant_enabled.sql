...
-- initial experience: only the TjuaeUI butler is enabled by default; every
...
--
-- The butler is identified by its manifest source_ref 'tjuaeui-assistant'.
--
...
SET enabled = CASE
        WHEN assistant_definition_id IN (
            ...
            WHERE source = 'builtin' AND source_ref = 'tjuaeui-assistant'
        ) THEN 1
        ELSE 0
    END,
...
SET enabled = CASE WHEN assistant_id = 'tjuaeui-assistant' THEN 1 ELSE 0 END,
    updated_at = CAST(strftime('%s', 'now') AS INTEGER) * 1000
