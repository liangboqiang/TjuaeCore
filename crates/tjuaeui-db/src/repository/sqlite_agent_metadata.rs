...
struct AgentMetadataSafeRow {
    ...
    last_check_at: Option<tjuaeui_common::TimestampMs>,
    last_success_at: Option<tjuaeui_common::TimestampMs>,
    last_failure_at: Option<tjuaeui_common::TimestampMs>,
    command_override: Option<String>,
    env_override: Option<String>,
    created_at: tjuaeui_common::TimestampMs,
    updated_at: tjuaeui_common::TimestampMs,
}
...
mod tests {
    ...
    async fn seed_rows_populated_after_migrations() {
        ...
        assert!(
            rows.iter()
                .any(|r| r.name == "TjuaeCLI" && r.agent_source == "internal")
        );
    ...
    async fn seed_rows_include_icon_backfill() {
        ...
        assert_eq!(
            tjuae_cli_modes["available_modes"]
                .as_array()
                .expect("tjuaecli available modes")
                .iter()
                .filter_map(|item| item.get("id").and_then(serde_json::Value::as_str))
                .collect::<Vec<_>>(),
            vec!["default", "auto_edit", "yolo"]
        );
        let tjuae_cli_config_options: serde_json::Value =
            serde_json::from_str(tjuae_cli.config_options.as_deref().expect("tjuaecli config options")).unwrap();
        assert_eq!(
            tjuae_cli_config_options["config_options"][0]["options"][1]["value"].as_str(),
            Some("auto_edit")
        );
