//! Data-dir process-level instance guard (concurrent-startup option A).
//!
//! A long-lived, non-blocking exclusive `flock` on a lock file next to the
//! database file. The winning tjuaecore holds it for its whole lifetime so a
//! second tjuaecore yields structurally before touching the database or binding
...
pub fn instance_lock_path(db_path: &Path) -> PathBuf {
    ...
    let new_name = match p.file_name().and_then(|s| s.to_str()) {
        ...
        None => "tjuaeui.instance.lock".to_string(),
    };
...
mod tests {
    ...
    fn instance_lock_path_appends_instance_lock_suffix() {
        ...
        let path = instance_lock_path(Path::new("/data/tjuaeui/tjuaeui-backend.db"));
        assert_eq!(path, PathBuf::from("/data/tjuaeui/tjuaeui-backend.db.instance.lock"));
    }

    #[test]
    fn guard_is_exclusive_and_releases_on_drop() {
        let dir = std::env::temp_dir().join(format!("tjuaeui-instance-lock-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let db_path = dir.join("tjuaeui-backend.db");
