...
///
/// Injected by `tjuaeui-app` — wraps `tjuaeui_auth::extract_token_from_ws_headers`
/// so that `tjuaeui-realtime` does not depend on `tjuaeui-auth` directly.
