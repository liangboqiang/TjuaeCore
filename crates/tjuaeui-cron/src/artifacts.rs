...
use serde_json::json;
use tjuaeui_api_types::{ConversationArtifactResponse, WebSocketMessage};
use tjuaeui_common::generate_id;
use tjuaeui_db::ConversationArtifactRow;
...
mod tests {
    ...
    fn builds_skill_suggest_response() {
        ...
        assert_eq!(response.kind, tjuaeui_api_types::ConversationArtifactKind::SkillSuggest);
        assert_eq!(response.status, tjuaeui_api_types::ConversationArtifactStatus::Pending);
    ...
    fn builds_cron_trigger_payload() {
        ...
        let response = artifact_response_from_row(&row).unwrap();
        assert_eq!(response.kind, tjuaeui_api_types::ConversationArtifactKind::CronTrigger);
