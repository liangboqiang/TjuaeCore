...
use tjuaeui_ai_agent::task_manager::IWorkerTaskManager;
use tjuaeui_api_types::{AssistantConversationRequest, CreateConversationRequest};
use tjuaeui_common::{
...
use tjuaeui_db::models::MessageRow;
use tjuaeui_db::{ConversationRowUpdate, IConversationRepository};
...
const DEPRECATED_AGENT_TYPE_MESSAGE: &str = "新对话不再支持此 Agent 类型。";
...
impl JobExecutor {
    ...
    pub async fn get_conversation_row(
        &self,
        conversation_id: &str,
    ) -> Result<Option<tjuaeui_db::models::ConversationRow>, CronError> {
    ...
    pub async fn get_assistant_snapshot(
        &self,
        conversation_id: &str,
    ) -> Result<Option<tjuaeui_db::models::ConversationAssistantSnapshotRow>, CronError> {
        self.conversation_repo
    ...
    pub async fn insert_tips_message(
        ...
        let row = MessageRow {
            ...
            hidden: false,
            created_at: tjuaeui_common::now_ms(),
        };
...
impl JobExecutor {
        ...
        match self.get_conversation_row(conversation_id).await {
            ...
            Ok(None) => {
                return ExecutionResult::Error {
                    message: format!("找不到对话：{conversation_id}"),
                };
            }
            Err(e) => {
                error!(
    ...
    async fn conversation_turn_failed_message(
            ...
            .await
        {
            ...
            Ok(None) => format!("对话回合 {} 失败，Agent 未提供错误详情", outcome.turn_id),
            Err(err) => {
                warn!(
                    conversation_id,
                ...
                format!("对话回合 {} 失败，Agent 未提供错误详情", outcome.turn_id)
            }
        }
    }
    ...
    async fn resolve_target_conversation_user_id(&self, conversation_id: &str) -> Result<String, CronError> {
        let Some(row) = self.get_conversation_row(conversation_id).await? else {
            return Err(CronError::Scheduler(format!("找不到对话：{conversation_id}")));
        };
...
/// Only tjuae_cli conversations carry meaningful model info in `conversations.model`;
    ...
    match write_raw_skill_file(data_dir, &job.id, raw_content).await {
        ...
        Err(CronError::InvalidSkillContent(_)) => {
            let description = job
                .description
                .clone()
                .unwrap_or_else(|| format!("为 {} 保存的定时任务技能", job.name));
            write_skill_file(
                data_dir,
                &job.id,
                &job.name,
...
mod tests {
    ...
    use tjuaeui_ai_agent::AgentStreamEvent;
    use tjuaeui_ai_agent::agent_task::{AgentInstance, IAgentTask, IMockAgent};
    use tjuaeui_ai_agent::protocol::events::{FinishEventData, TextEventData};
    use tjuaeui_ai_agent::types::{BuildTaskOptions, SendMessageData};
    use tjuaeui_api_types::{AgentModeResponse, ConfigOptionConfirmation, SetConfigOptionResponse, WebSocketMessage};
    use tjuaeui_common::{AgentKillReason, ConversationStatus, PaginatedResult, TimestampMs};
    use tjuaeui_db::{
    ...
    fn sample_job() -> CronJob {
        CronJob {
            ...
            agent_config: Some(CronAgentConfig {
                ...
                model: None,
                config_options: None,
                workspace: Some(ensure_named_workspace_path("tjuaeui-cron-sample-job-workspace")),
            }),
    ...
    fn build_prompt_existing_mode_no_skill() {
        ...
        assert!(prompt.contains("[定时任务执行]"));
        assert!(prompt.contains("任务指令：\ndo something"));
    }
    ...
    fn build_prompt_new_conv_with_skill() {
        ...
        assert!(prompt.contains("工作区已经加载包含详细说明的技能文件"));
    ...
    fn build_prompt_new_conv_no_skill() {
        ...
        let prompt = build_prompt(&job, None);
        assert!(prompt.contains("创建 \"SKILL_SUGGEST.md\""));
    }
    ...
    async fn hydrated_registry() -> Arc<AgentRegistry> {
        ...
        let repo = Arc::new(tjuaeui_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    ...
    async fn parse_agent_type_known_types() {
        ...
        assert_eq!(
            parse_agent_type(&registry, "tjuaecli").await.unwrap(),
            AgentType::TjuaeCli
        );
    }
    ...
    fn resolve_model_returns_none_for_acp() {
        // Model info only applies to tjuae_cli; ACP ignores it.
        let job = sample_job();
    ...
    #[test]
    fn resolve_model_returns_none_for_non_tjuae_cli_type() {
        let job = CronJob {
    ...
    #[test]
    fn resolve_model_tjuae_cli_with_full_config() {
        let job = CronJob {
            agent_type: "tjuaecli".into(),
            agent_config: Some(CronAgentConfig {
                name: "OpenAI".into(),
                cli_path: None,
                is_preset: None,
        ...
        let model = resolve_model(&job).expect("tjuaecli + full config returns Some");
        assert_eq!(model.provider_id, "4056cdea");
        assert_eq!(model.model, "gpt-5");
    }

    #[test]
    fn resolve_model_tjuae_cli_uses_model_payload() {
        let job = CronJob {
            agent_type: "tjuaecli".into(),
            agent_config: Some(CronAgentConfig {
                name: "OpenAI".into(),
        ...
        let model = resolve_model(&job).expect("tjuaecli model payload returns Some");
        assert_eq!(model.provider_id, "4056cdea");
        assert_eq!(model.model, "gpt-5");
    }

    #[test]
    fn resolve_model_tjuae_cli_without_config_returns_none() {
    ...
    #[test]
    fn resolve_model_tjuae_cli_without_model_returns_none() {
        let job = CronJob {
            agent_type: "tjuaecli".into(),
            agent_config: Some(CronAgentConfig {
                name: "Bogus".into(),
    ...
    #[tokio::test]
    async fn build_conversation_extra_without_saved_skill_omits_skill_overrides() {
        let registry = hydrated_registry().await;
        let job = CronJob {
            execution_mode: ExecutionMode::NewConversation,
            ..sample_job()
    ...
    #[tokio::test]
    async fn build_conversation_extra_for_existing_replacement_omits_skill_overrides() {
        let registry = hydrated_registry().await;
        let job = CronJob {
            execution_mode: ExecutionMode::Existing,
            ..sample_job()
    ...
    async fn build_conversation_extra_preserves_agent_workspace() {
        ...
        assert_eq!(
            extra["workspace"],
            ensure_named_workspace_path("tjuaeui-cron-sample-job-workspace")
        );
    }
    ...
    async fn execute_inner_new_conversation_without_saved_skill_requests_skill_suggest() {
        ...
        assert!(sent_messages[0].content.contains("创建 \"SKILL_SUGGEST.md\""));
    ...
    async fn execute_inner_uses_conversation_workspace_when_job_workspace_missing() {
        ...
        assert_eq!(
            ...
            ensure_named_workspace_path("tjuaeui-cron-existing-conversation-workspace")
        );
    ...
    async fn execute_inner_persists_agent_workspace_when_conversation_workspace_missing() {
        ...
        assert_eq!(
            value["workspace"],
            ensure_named_workspace_path("tjuaeui-cron-agent-workspace")
        );
    }
    ...
    async fn execute_inner_inserts_right_side_user_message_for_cron_prompt() {
        ...
        let repo = Arc::new(MissingWorkspaceConversationRepo::new(
            "conv_1",
            serde_json::json!({
                "workspace": ensure_named_workspace_path("tjuaeui-cron-existing-conversation-workspace")
            }),
        ));
    ...
    fn make_executor_for_busy_tests() -> JobExecutor {
        ...
        impl IWorkerTaskManager for StubTaskManager {
            ...
            async fn get_or_build_task(
                ...
                _: &str,
                _: BuildTaskOptions,
            ) -> Result<AgentInstance, tjuaeui_ai_agent::AgentError> {
                Err(tjuaeui_ai_agent::AgentError::internal("stub"))
            }
            fn kill(
                &self,
                _: &str,
                _: Option<tjuaeui_common::AgentKillReason>,
            ) -> Result<(), tjuaeui_ai_agent::AgentError> {
                Ok(())
            }
            fn kill_and_wait(
                &self,
            ...
            fn collect_idle(&self, _: tjuaeui_common::TimestampMs) -> Vec<String> {
                vec![]
            }
        }
        ...
        impl IConversationRepository for StubConvRepo {
            async fn get(&self, _id: &str) -> Result<Option<tjuaeui_db::models::ConversationRow>, tjuaeui_db::DbError> {
                Ok(None)
            }
            async fn create(&self, _row: &tjuaeui_db::models::ConversationRow) -> Result<(), tjuaeui_db::DbError> {
                Ok(())
            }
            async fn update(&self, _id: &str, _updates: &ConversationRowUpdate) -> Result<(), tjuaeui_db::DbError> {
                Ok(())
            }
            ...
            ) -> Result<PaginatedResult<tjuaeui_db::models::ConversationRow>, tjuaeui_db::DbError> {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
                    has_more: false,
            ...
            async fn find_by_source_and_chat(
                ...
                _chat_id: &str,
                _agent_type: &str,
            ) -> Result<Option<tjuaeui_db::models::ConversationRow>, tjuaeui_db::DbError> {
                Ok(None)
            }
            async fn list_by_cron_job(
                ...
                _user_id: &str,
                _cron_job_id: &str,
            ) -> Result<Vec<tjuaeui_db::models::ConversationRow>, tjuaeui_db::DbError> {
                Ok(vec![])
            }
            ...
            async fn insert_message(
                &self,
                _message: &tjuaeui_db::models::MessageRow,
            ) -> Result<(), tjuaeui_db::DbError> {
                Ok(())
            }
            async fn update_message(&self, _id: &str, _updates: &MessageRowUpdate) -> Result<(), tjuaeui_db::DbError> {
                Ok(())
            }
            async fn delete_messages_by_conversation(&self, _conv_id: &str) -> Result<(), tjuaeui_db::DbError> {
                Ok(())
            }
            async fn get_message_by_msg_id(
                ...
                _msg_id: &str,
                _msg_type: &str,
            ) -> Result<Option<tjuaeui_db::models::MessageRow>, tjuaeui_db::DbError> {
                Ok(None)
            }
        ...
        impl tjuaeui_realtime::EventBroadcaster for StubBroadcaster {
        ...
        impl tjuaeui_conversation::skill_resolver::SkillResolver for StubSkillResolver {
            async fn resolve_skills(
                &self,
                _names: &[String],
            ) -> Vec<tjuaeui_conversation::skill_resolver::ResolvedAgentSkill> {
                Vec::new()
            }
            ...
            async fn link_workspace_skills(
                ...
                _rel_dirs: &[&str],
                _skills: &[tjuaeui_conversation::skill_resolver::ResolvedAgentSkill],
            ) -> usize {
                0
            }
        }

        let stub_broadcaster: Arc<dyn tjuaeui_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
        let stub_repo: Arc<dyn IConversationRepository> = Arc::new(StubConvRepo);
        let agent_metadata_repo: Arc<dyn tjuaeui_db::IAgentMetadataRepository> = Arc::new(StubAgentMetadataRepo);
        let acp_session_repo: Arc<dyn tjuaeui_db::IAcpSessionRepository> = Arc::new(StubAcpSessionRepo);
    ...
    impl RecordingAgent {
        fn new(conversation_id: &str, mode: &str, initialized: bool) -> Self {
            ...
            Self {
                conversation_id: conversation_id.to_owned(),
                workspace: ensure_named_workspace_path("tjuaeui-cron-agent-workspace"),
                event_tx,
    ...
    impl IAgentTask for RecordingAgent {
        ...
        async fn send_message(&self, data: SendMessageData) -> Result<(), tjuaeui_ai_agent::AgentSendError> {
        ...
        async fn cancel(&self) -> Result<(), tjuaeui_ai_agent::AgentError> {
            Ok(())
        }

        fn kill(&self, _reason: Option<AgentKillReason>) -> Result<(), tjuaeui_ai_agent::AgentError> {
            Ok(())
        }
    }
    ...
    impl IMockAgent for RecordingAgent {
        async fn mode(&self) -> Result<AgentModeResponse, tjuaeui_ai_agent::AgentError> {
        ...
        async fn set_config_option(
            ...
            option_id: &str,
            value: &str,
        ) -> Result<SetConfigOptionResponse, tjuaeui_ai_agent::AgentError> {
            if option_id != "mode" {
                return Err(tjuaeui_ai_agent::AgentError::bad_request(format!(
    ...
    impl IWorkerTaskManager for FailingTaskManager {
        ...
        async fn get_or_build_task(
            ...
            Err(tjuaeui_ai_agent::AgentError::bad_gateway(self.message.clone()))
        }
    ...
    impl IConversationRepository for ExistingConversationRepo {
        async fn get(&self, id: &str) -> Result<Option<tjuaeui_db::models::ConversationRow>, tjuaeui_db::DbError> {
            Ok(Some(tjuaeui_db::models::ConversationRow {
                id: id.to_owned(),
                user_id: "cron".into(),
        ...
        async fn insert_message(&self, _message: &tjuaeui_db::models::MessageRow) -> Result<(), tjuaeui_db::DbError> {
            Ok(())
        }
    ...
    struct MissingWorkspaceConversationRepo {
        ...
        inserted_messages: Mutex<Vec<tjuaeui_db::models::MessageRow>>,
    ...
    impl MissingWorkspaceConversationRepo {
        ...
        fn inserted_messages(&self) -> Vec<tjuaeui_db::models::MessageRow> {
    ...
    impl tjuaeui_realtime::EventBroadcaster for RecordingBroadcaster {
    ...
    impl IConversationRepository for MissingWorkspaceConversationRepo {
        ...
        async fn update(&self, _id: &str, updates: &ConversationRowUpdate) -> Result<(), tjuaeui_db::DbError> {
        ...
        async fn insert_message(&self, message: &tjuaeui_db::models::MessageRow) -> Result<(), tjuaeui_db::DbError> {
    ...
    fn make_executor_with_task_manager_and_repo(
        ...
        let broadcaster: Arc<dyn tjuaeui_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
    ...
    #[async_trait::async_trait]
    impl tjuaeui_db::IAcpSessionRepository for StubAcpSessionRepo {
        async fn get(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<tjuaeui_db::models::AcpSessionRow>, tjuaeui_db::DbError> {
            Ok(None)
        }
        async fn create(
            &self,
            _params: &tjuaeui_db::CreateAcpSessionParams<'_>,
        ) -> Result<tjuaeui_db::models::AcpSessionRow, tjuaeui_db::DbError> {
            Err(tjuaeui_db::DbError::Init("stub".into()))
        }
        async fn update_session_id(
            &self,
            _conversation_id: &str,
            _session_id: &str,
        ) -> Result<bool, tjuaeui_db::DbError> {
            Ok(false)
        }
        async fn clear_session_id(&self, _conversation_id: &str) -> Result<bool, tjuaeui_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _conversation_id: &str) -> Result<bool, tjuaeui_db::DbError> {
            Ok(false)
        }
        async fn load_runtime_state(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<tjuaeui_db::PersistedSessionState>, tjuaeui_db::DbError> {
            Ok(None)
        }
        async fn save_runtime_state(
            &self,
            _conversation_id: &str,
            _params: &tjuaeui_db::SaveRuntimeStateParams<'_>,
        ) -> Result<bool, tjuaeui_db::DbError> {
            Ok(false)
        }
    }
    ...
    #[async_trait::async_trait]
    impl tjuaeui_db::IAgentMetadataRepository for StubAgentMetadataRepo {
        async fn list_all(&self) -> Result<Vec<tjuaeui_db::models::AgentMetadataRow>, tjuaeui_db::DbError> {
            Ok(Vec::new())
        }
        async fn get(&self, _id: &str) -> Result<Option<tjuaeui_db::models::AgentMetadataRow>, tjuaeui_db::DbError> {
            Ok(None)
        }
        async fn find_by_source_and_name(
            ...
            _agent_source: &str,
            _name: &str,
        ) -> Result<Option<tjuaeui_db::models::AgentMetadataRow>, tjuaeui_db::DbError> {
            Ok(None)
        }
        ...
        async fn upsert(
            &self,
            _params: &tjuaeui_db::models::UpsertAgentMetadataParams<'_>,
        ) -> Result<tjuaeui_db::models::AgentMetadataRow, tjuaeui_db::DbError> {
            Err(tjuaeui_db::DbError::Init("stub".into()))
        }
        async fn apply_handshake(
            &self,
            _id: &str,
            _params: &tjuaeui_db::models::UpdateAgentHandshakeParams<'_>,
        ) -> Result<Option<tjuaeui_db::models::AgentMetadataRow>, tjuaeui_db::DbError> {
            Ok(None)
        }
        async fn update_availability_snapshot(
            &self,
            _id: &str,
            _params: &tjuaeui_db::models::UpdateAgentAvailabilitySnapshotParams<'_>,
        ) -> Result<Option<tjuaeui_db::models::AgentMetadataRow>, tjuaeui_db::DbError> {
            Ok(None)
        }
        ...
        async fn set_enabled(&self, _id: &str, _enabled: bool) -> Result<bool, tjuaeui_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _id: &str) -> Result<bool, tjuaeui_db::DbError> {
            Ok(false)
        }
    }
}
