...
async fn system_resume(
    ...
    let is_internal = headers.get("x-tjuaeui-internal").and_then(|value| value.to_str().ok()) == Some("1");
    if !is_internal {
        return Err(ApiError::Forbidden("仅限内部访问".into()));
    }
...
async fn create_conversation_cron(
    ...
    let user_id = header_value(&headers, "x-tjuaeui-user-id")?;
    let conversation_id = header_value(&headers, "x-tjuaeui-conversation-id")?;
    let Json(req) = body.map_err(ApiError::from)?;
    let resp = state
        .cron_service
...
fn header_value(headers: &HeaderMap, name: &'static str) -> Result<String, ApiError> {
    headers
        ...
        .map(str::to_owned)
        .ok_or_else(|| ApiError::BadRequest(format!("缺少必需请求头：{name}")))
}
...
mod tests {
    ...
    fn header_value_rejects_missing_or_blank_required_header() {
        ...
        assert!(matches!(
            header_value(&headers, "x-tjuaeui-user-id"),
            Err(ApiError::BadRequest(message)) if message.contains("x-tjuaeui-user-id")
        ));

        headers.insert("x-tjuaeui-user-id", "   ".parse().unwrap());
    ...
    fn header_value_trims_required_header() {
        let mut headers = HeaderMap::new();
        headers.insert("x-tjuaeui-user-id", " user_1 ".parse().unwrap());

        assert_eq!(header_value(&headers, "x-tjuaeui-user-id").unwrap(), "user_1");
    }
    ...
    fn conversation_error_maps_through_boundary_mapper() {
        let err: ApiError =
            CronError::Conversation(tjuaeui_conversation::ConversationError::NotFound { id: "conv-1".into() }).into();
        assert!(matches!(err, ApiError::NotFound(msg) if msg == "找不到对话：conv-1"));
    }
