...
use tjuaeui_common::{ProviderWithModel, TimestampMs};
use tjuaeui_db::models::CronJobRow;
...
fn parse_schedule(
    ...
    match kind {
        "at" => {
            let at_ms = value
                .parse::<TimestampMs>()
                .map_err(|e| CronError::InvalidSchedule(format!("at_ms '{value}' 无效：{e}")))?;
            Ok(CronSchedule::At {
                at_ms,
        ...
        "every" => {
            let every_ms = value
                .parse::<i64>()
                .map_err(|e| CronError::InvalidSchedule(format!("every_ms '{value}' 无效：{e}")))?;
        ...
        other => Err(CronError::InvalidSchedule(format!("未知的计划类型：{other}"))),
    }
}
...
mod tests {
    ...
    #[test]
    fn domain_to_dto_keeps_model_for_tjuae_cli_assistant_jobs() {
        let job = CronJob {
