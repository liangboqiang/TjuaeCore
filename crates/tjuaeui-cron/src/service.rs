...
impl CronService {
    ...
    pub async fn create_for_conversation_helper(
        ...
        Ok(CreateConversationCronResponse {
            job_id: job.id.clone(),
            message: format!("已创建定时任务“{}”（{}）", job.name, job.id),
        })
    }
    ...
    async fn verify_conversation_helper_context(
        ...
        self.executor
            ...
            .ok_or_else(|| {
                CronError::Conversation(tjuaeui_conversation::ConversationError::NotFound {
    ...
    async fn add_job_internal(
        ...
        validate_tjuae_cli_agent_config(&resolved_agent_type, req.agent_config.as_ref())?;
    ...
    pub async fn update_job(&self, job_id: &str, req: UpdateCronJobRequest) -> Result<CronJob, CronError> {
        ...
        {
            return Err(CronError::InvalidAgentConfig("持续对话任务必须保留原助手".into()));
        }
        if let Some(config_dto) = &req.agent_config {
            ...
            validate_tjuae_cli_agent_config(&job.agent_type, Some(&config_dto))?;
    ...
    async fn resolve_new_job_agent_type(
        &self,
        agent_config: Option<&tjuaeui_api_types::CronAgentConfigWriteDto>,
    ) -> Result<String, CronError> {
        let Some(assistant_id) = agent_config.and_then(|config| config.assistant_id.as_deref()) else {
            return Err(CronError::InvalidAgentConfig(
                "新建定时任务必须提供 assistant_id".into(),
            ));
        };
    ...
    async fn resolve_job_agent_type(&self, job: &CronJob) -> Result<String, CronError> {
        ...
        let Some(assistant_id) = job
            ...
            return Err(CronError::InvalidAgentConfig("定时任务必须提供 assistant_id".into()));
        };
    ...
    async fn build_agent_config_from_conversation(
        ...
        String,
        Option<tjuaeui_api_types::CronAgentConfigWriteDto>,
        Option<String>,
    ) {
        ...
        // the cached vendor-label fallback (`"tjuaecli"`) from sneaking back in
        // (agent-options lookup regression).
        let model_resolved = tjuaeui_conversation::task_options::provider_model_from_conversation_row(row);
        ...
        let legacy_agent_label = if row.r#type == "tjuaecli" {
            Some("tjuaecli".to_owned())
        } else {
            model
        ...
        let backend = if row.r#type == "tjuaecli" {
            model
                .map(|value| value.provider_id.clone())
                .filter(|value| !value.is_empty())
                .or_else(|| get_string(&extra, &["backend"]))
                .or_else(|| assistant_backend.clone())
                .unwrap_or_else(|| "tjuaecli".to_owned())
        } else {
            assistant_backend
                .clone()
                .or_else(|| {
        ...
        let agent_config = tjuaeui_api_types::CronAgentConfigWriteDto {
            name: assistant_name
            ...
            model: (row.r#type == "tjuaecli").then(|| model.cloned()).flatten(),
            config_options: None,
    ...
    async fn build_cron_agent_config(
        ...
        config: tjuaeui_api_types::CronAgentConfigWriteDto,
        ...
        let assistant_backend = self
            ...
            .await?
            .ok_or_else(|| CronError::InvalidAgentConfig(format!("无法为助手 '{assistant_id}' 解析运行时后端")))?;
...
impl tjuaeui_common::OnConversationDelete for CronService {
...
fn runtime_agent_type_for_backend(backend: &str) -> &'static str {
    if backend == "tjuaecli" { "tjuaecli" } else { "acp" }
}
...
fn normalize_model(
    ...
    if runtime_agent_type == "tjuaecli" && (model.provider_id.is_empty() || model.model.is_empty()) {
        return Err(CronError::InvalidAgentConfig(
            "tjuaecli 定时任务必须提供 agent_config.model.provider_id 和 agent_config.model.model".into(),
        ));
    }
...
fn validate_tjuae_cli_agent_config(
    agent_type: &str,
    agent_config: Option<&tjuaeui_api_types::CronAgentConfigWriteDto>,
) -> Result<(), CronError> {
    if agent_type != "tjuaecli" {
        return Ok(());
    }
...
fn validate_skill_body_content(content: &str) -> Result<(), CronError> {
    ...
    if trimmed.is_empty() {
        return Err(CronError::InvalidSkillContent("内容不能为空".into()));
    }

    let lower = trimmed.to_lowercase();
    for pattern in PLACEHOLDER_PATTERNS {
        if lower.starts_with(pattern) {
            return Err(CronError::InvalidSkillContent("内容看起来仍是占位文本".into()));
        }
    }
...
fn sanitize_agent_config_dto(
    mut config: tjuaeui_api_types::CronAgentConfigWriteDto,
) -> tjuaeui_api_types::CronAgentConfigWriteDto {
...
mod tests {
    ...
    // -- validate_tjuae_cli_agent_config ----------------------------------------

    fn agent_cfg_dto(provider_id: &str) -> tjuaeui_api_types::CronAgentConfigWriteDto {
        tjuaeui_api_types::CronAgentConfigWriteDto {
            name: "provider".into(),
            cli_path: None,
    ...
    #[test]
    fn validate_tjuae_cli_accepts_valid_config() {
        let cfg = agent_cfg_dto("4056cdea");
        assert!(validate_tjuae_cli_agent_config("tjuaecli", Some(&cfg)).is_ok());
    }

    #[test]
    fn validate_tjuae_cli_rejects_missing_config() {
        let err = validate_tjuae_cli_agent_config("tjuaecli", None).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }

    #[test]
    fn validate_tjuae_cli_rejects_empty_provider_id() {
        let cfg = agent_cfg_dto("");
        let err = validate_tjuae_cli_agent_config("tjuaecli", Some(&cfg)).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }

    #[test]
    fn validate_tjuae_cli_rejects_whitespace_provider_id() {
        let cfg = agent_cfg_dto("   ");
        let err = validate_tjuae_cli_agent_config("tjuaecli", Some(&cfg)).unwrap_err();
        assert!(matches!(err, CronError::InvalidAgentConfig(_)));
    }
    ...
    #[test]
    fn validate_tjuae_cli_ignores_non_tjuae_cli_type() {
        // ACP / other types may legitimately omit agent_config or leave model empty.
        assert!(validate_tjuae_cli_agent_config("acp", None).is_ok());
        let cfg = agent_cfg_dto("");
        assert!(validate_tjuae_cli_agent_config("claude", Some(&cfg)).is_ok());
    }

    #[test]
    fn sanitize_agent_config_dto_clears_legacy_ids_when_assistant_id_present() {
        let config = tjuaeui_api_types::CronAgentConfigWriteDto {
            name: "Helper".into(),
            cli_path: None,
            assistant_id: Some("assistant-1".into()),
    ...
    fn sanitize_agent_config_dto_rejects_legacy_custom_agent_id_without_assistant_id() {
        let err = serde_json::from_value::<tjuaeui_api_types::CronAgentConfigWriteDto>(serde_json::json!({
            "name": "Helper",
