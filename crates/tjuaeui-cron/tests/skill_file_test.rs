...
fn unique_temp_dir(label: &str) -> std::path::PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    std::env::temp_dir().join(format!("tjuaeui-cron-{label}-{}-{nanos}", std::process::id()))
}
...
fn build_skill_content_matches_frontend_shape() {
    ...
    assert!(content.contains("这是一个定时任务：**Daily Report**"));
    assert!(content.contains("计划：Every day at 9am"));
    assert!(content.contains("## 执行说明"));
...
fn validate_skill_content_rejects_placeholders() {
    ...
    assert!(err.to_string().contains("模板占位符"));
}
...
async fn write_raw_skill_file_validates_before_writing() {
    ...
    assert!(err.to_string().contains("技能文件必须以 YAML frontmatter 开头"));
