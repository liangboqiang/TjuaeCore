...
//! - TjuaeCli stub returns appropriate errors

use std::sync::Arc;

use serde_json::json;
use std::sync::atomic::{AtomicI64, Ordering};
use tjuaeui_ai_agent::manager::tjuae_cli::TjuaeCliAgentManager;
use tjuaeui_ai_agent::task_manager::AgentFactory;
use tjuaeui_ai_agent::types::{BuildTaskOptions, SendMessageData, TjuaeCliResolvedConfig};
use tjuaeui_ai_agent::*;
use tjuaeui_ai_agent::{SkillIndex, build_system_instructions_with_skills_index};
use tjuaeui_common::{AgentKillReason, AgentType, ConversationStatus, ProviderWithModel, TimestampMs, now_ms};
...
// ---------------------------------------------------------------------------
// TjuaeCli agent tests (real implementation with AgentEngine)
// ---------------------------------------------------------------------------

fn make_tjuae_cli_config() -> TjuaeCliResolvedConfig {
    TjuaeCliResolvedConfig {
        ...
        session_directory: std::env::temp_dir().join("tjuaecli-test-sessions"),
        session_mode: None,
        skills: Vec::new(),
...
async fn tjuae_cli_agent_kill_succeeds() {
    let agent = TjuaeCliAgentManager::new("conv-1".into(), "/proj".into(), make_tjuae_cli_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn tjuae_cli_agent_confirm_succeeds() {
    let agent = TjuaeCliAgentManager::new("conv-1".into(), "/proj".into(), make_tjuae_cli_config(), None)
        .await
        .unwrap();
    // `confirm` is an inherent method on `TjuaeCliAgentManager` (reached via
    // `AgentInstance::TjuaeCli(..)` in production); the test calls it
...
#[tokio::test]
async fn tjuae_cli_agent_metadata() {
    let agent = TjuaeCliAgentManager::new("conv-abc".into(), "/work".into(), make_tjuae_cli_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn collect_idle_ignores_tjuae_cli_agent_type() {
    ...
    mgr.get_or_build_task("acp-1", make_opts(AgentType::Acp, "acp-1"))
        .await
        .unwrap();
    mgr.get_or_build_task("tjuaecli-1", make_opts(AgentType::TjuaeCli, "tjuaecli-1"))
        .await
        .unwrap();
