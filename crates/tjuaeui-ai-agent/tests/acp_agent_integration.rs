...
use tjuaeui_ai_agent::factory::acp_assembler::{WorkspaceInfo, assemble_acp_params};
use tjuaeui_ai_agent::manager::acp::AcpAgentManager;
use tjuaeui_ai_agent::registry::AgentRegistry;
use tjuaeui_ai_agent::{AgentInstance, AgentStreamEvent, IAgentTask};
use tjuaeui_common::ConversationStatus;
...
async fn make_mock_agent(script: &str, backend: &str) -> (Arc<AcpAgentManager>, broadcast::Receiver<AgentStreamEvent>) {
    ...
    let config = tjuaeui_ai_agent::AcpBuildExtra {
        agent_id: None,
...
fn acp_build_extra_populates_skills_from_extra_json() {
    ...
    let extra: tjuaeui_ai_agent::AcpBuildExtra = serde_json::from_value(json).unwrap();
...
async fn acp_agent_type_is_acp() {
    ...
    assert_eq!(agent.agent_type(), tjuaeui_common::AgentType::Acp);
...
async fn acp_agent_kill_terminates_process() {
    ...
    agent.kill(Some(tjuaeui_common::AgentKillReason::IdleTimeout)).unwrap();
