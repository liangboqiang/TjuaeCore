...
use tjuaeui_api_types::TryConnectCustomAgentResponse;
...
async fn run_handshake(proc: &CliAgentProcess) -> ProbeOutcome {
    let Some((stdin, stdout)) = proc.take_stdio().await else {
        return ProbeOutcome::Fail("启动 SDK 进程后无法获取标准输入输出".to_string());
    };
    ...
    let protocol = tokio::select! {
        ...
        res = connect => match res {
            ...
            Err(e) => return ProbeOutcome::Fail(format!("ACP 初始化失败：{e}")),
        },
        exit = proc.wait_for_exit() => {
            ...
            return if stderr.is_empty() {
                ProbeOutcome::Fail(format!("CLI 在 ACP 初始化完成前已退出（状态={status}）"))
            } else {
                ProbeOutcome::Fail(format!("CLI 在 ACP 初始化完成前已退出（状态={status}）：{stderr}"))
            };
        }
    };
    ...
    let outcome = match protocol.new_session(NewSessionRequest::new(std::env::temp_dir())).await {
        ...
        Err(e) => ProbeOutcome::Fail(format!("ACP 创建会话失败：{e}")),
    };
