...
const OPENCLAW_GATEWAY_MESSAGE: &str = "无法连接 OpenClaw Gateway";
const OPENCLAW_GATEWAY_DETAIL: &str = "OpenClaw Gateway 未运行，或无法通过 127.0.0.1:18789 连接。\n\n请启动 OpenClaw Gateway 后重试，可运行：\nopenclaw gateway status\nopenclaw gateway start";
const AWS_SSO_EXPIRED_DETAIL: &str = "令牌已过期。请使用对应配置文件运行 'aws sso login' 以刷新此 SSO 会话。";
    ...
    pub fn from_agent_error_ref(err: &AgentError) -> Self {
        ...
        match err {
            AgentError::WorkspacePathRuntimeUnavailable(path) => Self {
                stream_error: AgentStreamErrorData {
                    message: "当前 Agent 无法在此工作区路径中运行".into(),
                    code: Some(AgentErrorCode::WorkspacePathRuntimeUnavailable),
                    ownership: Some(AgentErrorOwnership::TjuaeUI),
            ...
            AgentError::Forbidden(_) => Self::new(
                "TjuaeUI 在请求到达 Agent 前阻止了该请求",
                AgentErrorCode::TjuaeUIPermissionError,
                AgentErrorOwnership::TjuaeUI,
                Some(detail),
                false,
                true,
            ...
            AgentError::NotFound(msg) if msg.starts_with("Session not found") || msg.starts_with("未找到会话") => {
                Self::new(
                    "未找到 Agent 会话",
            ...
            AgentError::BadRequest(msg) if msg.contains("Method not supported") || msg.contains("不支持此方法") => {
                Self::new(
                    "所选 Agent 不支持此操作",
            ...
            AgentError::BadRequest(msg) if msg.contains("Invalid parameters") || msg.contains("参数无效") => {
                Self::new(
                    "所选 Agent 拒绝了请求参数",
            ...
            let classified = acp_agent_internal_texts(message, data.as_ref())
                ...
                .find_map(|text| {
                    ...
                    classify_agent_lifecycle(&lower)
                        ...
                        .or_else(|| classify_tjuaeui_state(&lower))
...
fn classify_agent_lifecycle(lower: &str) -> Option<ClassifiedError> {
    ...
    if lower.contains("agent process exited before initialize handshake completed")
        || lower.contains("agent 进程在完成初始化握手前退出")
    {
        return Some(agent_error(
            "所选 Agent 在启动完成前退出",
    ...
    if lower.contains("process exited with code") || lower.contains("进程已退出（退出代码") {
        return Some(agent_error(
            "所选 Agent 进程意外退出",
            AgentErrorCode::UserAgentDisconnected,
            true,
    ...
    if lower.contains("initialize handshake timed out") || (lower.contains("初始化握手") && lower.contains("超时"))
    {
        return Some(agent_error(
            "所选 Agent 未在规定时间内完成启动",
    ...
    if lower.contains("failed to connect mcp") {
        return Some(agent_error(
            "所选 Agent 已与 MCP 服务器断开连接",
            AgentErrorCode::UserAgentDisconnected,
            true,
            AgentErrorResolutionKind::ReconnectAgent,
        ));
    }
    if lower.contains("session not found") || lower.contains("未找到会话") {
        return Some(agent_session_error(
            "未找到 Agent 会话",
            AgentErrorCode::UserAgentSessionNotFound,
        ));
...
fn classify_tjuaeui_state(lower: &str) -> Option<ClassifiedError> {
    if lower.contains("conversation is already processing") {
        return Some(ClassifiedError {
            message: "当前响应仍在运行",
            code: AgentErrorCode::TjuaeUIConversationBusy,
            ownership: AgentErrorOwnership::TjuaeUI,
            retryable: true,
            feedback_recommended: false,
...
mod tests {
    ...
    #[test]
    fn preserves_runtime_workspace_validation_as_structured_tjuaeui_error() {
        let err =
        ...
        assert_eq!(err.ownership(), Some(AgentErrorOwnership::TjuaeUI));
        assert_eq!(
    ...
    fn classifies_provider_504_html_body_as_timeout_with_stripped_detail() {
        let raw = "TjuaeCli agent error: Provider error: API error 504: <html>\r\n<head><title>504 Gateway Time-out</title></head>\r\n<body>\r\n<center><h1>504 Gateway Time-out</h1></center>\r\n<hr><center>openresty</center>\r\n</body>\r\n</html>";
    ...
    fn classifies_acp_internal_mcp_connect_failure_from_structured_data() {
        ...
        assert_eq!(detail, "Agent 内部错误（代码 -32603）：Failed to connect MCP servers");
    }
    ...
    fn classifies_agent_setup_failures() {
        ...
        for managed_binary_missing in [
            "expected managed Claude ACP platform binary missing: C:\\Users\\user\\AppData\\Roaming\\TjuaeUI\\tjuaeui\\runtime\\acp\\claude-agent-acp\\0.58.1\\win32-x64\\node_modules\\@anthropic-ai\\claude-agent-sdk-win32-x64\\claude.exe",
            "expected managed Codex ACP platform binary missing: C:\\Users\\user\\AppData\\Roaming\\TjuaeUI\\tjuaeui\\runtime\\acp\\codex-acp\\1.1.2\\win32-x64\\node_modules\\@openai\\codex-win32-x64\\vendor\\x86_64-pc-windows-msvc\\bin\\codex.exe",
        ] {
            assert_classification(
    ...
    fn classifies_provider_billing_auth_and_rate_limit() {
        assert_classification(
            "TjuaeCli agent error: Provider error: API error 402: {\"error\":{\"message\":\"Insufficient Balance\"}}",
            AgentErrorCode::UserLlmProviderBillingRequired,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::CheckProviderBilling,
        );
        assert_classification(
            "TjuaeCli agent error: Provider error: API error 400: {\"type\":\"error\",\"error\":{\"type\":\"invalid_request_error\",\"message\":\"Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits.\"}}",
            AgentErrorCode::UserLlmProviderBillingRequired,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::CheckProviderBilling,
        );
        assert_classification(
            "TjuaeCli agent error: Provider error: API error 401: invalid x-api-key",
            AgentErrorCode::UserLlmProviderAuthFailed,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::CheckProviderCredentials,
        );
        assert_classification(
            "TjuaeCli agent error: Provider error: Rate limited, retry after 5000ms",
            AgentErrorCode::UserLlmProviderRateLimited,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::Retry,
        );
    ...
    fn classifies_provider_request_model_and_context_errors() {
        ...
        assert_classification(
            "TjuaeCli agent error: provider repeatedly returned malformed tool calls (3/3); stopped to avoid wasting tokens",
            AgentErrorCode::UserLlmProviderInvalidRequest,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::ChangeModel,
        );
    ...
    fn classifies_bedrock_invalid_model_identifier_as_model_not_found() {
        assert_classification(
            "TjuaeCli agent error: Provider error: API error 400: {\"message\":\"The provided model identifier is invalid.\"}",
            AgentErrorCode::UserLlmProviderModelNotFound,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::ChangeModel,
        );
    ...
    fn classifies_provider_endpoint_network_timeout_and_empty_response() {
        ...
        assert_classification(
            "TjuaeCli agent error: API error: Connection error: error decoding response body",
            AgentErrorCode::UserLlmProviderNetworkError,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::Retry,
        );
        assert_classification(
            "TjuaeCli agent error: API error: error sending request for url",
            AgentErrorCode::UserLlmProviderNetworkError,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::Retry,
        );
    ...
    fn classifies_bare_provider_404_as_endpoint_not_found() {
        let detail = "TjuaeCli agent error: Provider error: API error 404: {\"detail\":\"Not Found\"}";
        assert_classification(
            detail,
            AgentErrorCode::UserLlmProviderEndpointNotFound,
            AgentErrorOwnership::UserLlmProvider,
    ...
    #[test]
    fn classifies_tjuaeui_conversation_busy_after_agent_and_provider_checks() {
        assert_classification(
            "Conflict: Conversation is already processing a message",
            AgentErrorCode::TjuaeUIConversationBusy,
            AgentErrorOwnership::TjuaeUI,
