...
use tjuaeui_common::AgentKillReason;
...
impl CloseReason {
    ...
    pub fn user_facing_message(&self) -> String {
        match self {
            CloseReason::UserCancel => "对话已取消".to_owned(),
            CloseReason::Killed { reason } => match reason {
                Some(AgentKillReason::IdleTimeout) => "Agent 已终止：空闲超时".to_owned(),
                Some(AgentKillReason::AgentErrorRecovery) => "Agent 已终止：错误恢复".to_owned(),
                Some(AgentKillReason::TeamMcpRebuild) => "Agent 已终止：团队 MCP 重建".to_owned(),
                Some(AgentKillReason::TeamDeleted) => "Agent 已终止：团队已删除".to_owned(),
                Some(AgentKillReason::ConversationDeleted) => "Agent 已终止：对话已删除".to_owned(),
                Some(AgentKillReason::UserCancelTimeout) => "对话已取消；Agent 已重启".to_owned(),
                Some(AgentKillReason::RuntimeCapabilityChanged) => "Agent 已终止：运行时能力已更改".to_owned(),
                None => "Agent 已终止".to_owned(),
            },
            CloseReason::ProcessExited {
                ...
                if redacted_summary.is_empty() {
                    format!("Agent 进程已退出{detail}")
                } else {
                    format!("Agent 进程已退出{detail}：{redacted_summary}")
                }
            }
...
fn format_exit_detail(exit_code: Option<i32>, signal: Option<&str>) -> String {
    match (exit_code, signal) {
        (Some(code), Some(sig)) => format!("（退出代码 {code}，{sig}）"),
        (Some(code), None) => format!("（退出代码 {code}）"),
        ...
        match self {
            ...
            AcpError::AgentInternal { message, code, data } => {
                ...
                if let Some(data) = data {
                    ...
                    let compact = serde_json::to_string(data).unwrap_or_else(|_| "<无法序列化的数据>".to_owned());
                    write!(f, "（{compact}）")?;
                }
                Ok(())
            }
            AcpError::OtherProtocolError { code, message, data } => {
                write!(f, "Agent 协议错误（代码 {code}）：{message}")?;
                if let Some(data) = data {
                    let compact = serde_json::to_string(data).unwrap_or_else(|_| "<无法序列化的数据>".to_owned());
                    write!(f, "（{compact}）")?;
                }
            ...
            AcpError::NotConnected => f.write_str("ACP 协议未连接"),
            AcpError::InitTimeout { timeout_secs } => {
                write!(f, "初始化握手在 {timeout_secs} 秒后超时")
            }
            AcpError::RequestTimeout { method, timeout_secs } => {
                write!(f, "Agent 请求“{method}”在 {timeout_secs} 秒后超时")
            }
        }
    }
}
...
mod tests {
    ...
    fn close_reason_process_exited_renders_exit_code_and_summary() {
        ...
        .user_facing_message();
        assert!(msg.contains("退出代码 127"), "got {msg}");
    ...
    fn close_reason_process_exited_omits_summary_when_empty() {
        ...
        .user_facing_message();
        assert!(msg.contains("退出代码 1"), "got {msg}");
    ...
    fn request_timeout_display_names_method_and_timeout_without_sensitive_payload() {
        ...
        .to_string();
        assert!(display.contains("在 10 秒后超时"), "got {display}");
    ...
    fn startup_crash_display_includes_exit_code() {
        ...
        let display = err.to_string();
        assert!(display.contains("退出代码 1"), "got {display}");
        assert!(
            display.contains("完成初始化握手前"),
    ...
    fn agent_internal_display_falls_back_when_message_is_sdk_default() {
        ...
        let display = err.to_string();
        assert!(
            display.contains("Agent 内部错误"),
            "Display must include the localized Agent internal error hint when SDK gave us its default message; got {display}"
        );
        assert!(
