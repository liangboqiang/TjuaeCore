...
pub trait IMockAgent: IAgentTask {
    ...
    async fn mode(&self) -> Result<tjuaeui_api_types::AgentModeResponse, AgentError> {
...
impl AgentInstance {
    ...
    /// router. TjuaeCli maintains inline confirmation lists.
    ...
    /// [`tjuaeui_common::normalize_keys_to_snake_case`] so keys land as
    /// `used` / `size` / `cost` to match the TjuaeUI wire convention —
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, AgentError> {
        match self {
            Self::Acp(m) => {
                ...
                let mut value =
                    serde_json::to_value(usage).map_err(|e| AgentError::internal(format!("序列化用量失败：{e}")))?;
    ...
    pub async fn handle_side_question(&self, req: SideQuestionRequest) -> Result<SideQuestionResponse, AgentError> {
        if req.question.trim().is_empty() {
            return Err(AgentError::bad_request("question 不能为空"));
        }
        match self {
            Self::Acp(m) => {
...
#[cfg(test)]
mod tjuae_cli_config_option_tests {
    use std::sync::Arc;

    use tjuaeui_api_types::ConfigOptionConfirmation;
    ...
    fn make_test_config() -> TjuaeCliResolvedConfig {
        TjuaeCliResolvedConfig {
            ...
            session_directory: std::env::temp_dir().join("tjuaecli-agent-task-test-sessions"),
            session_mode: None,
            skills: Vec::new(),
    ...
    async fn tjuae_cli_instance() -> AgentInstance {
        let manager = TjuaeCliAgentManager::new(
            "conv-tjuaecli-config".into(),
            "/project".into(),
            make_test_config(),
            None,
        )
        .await
        .expect("tjuaecli manager should start in tests");
        AgentInstance::TjuaeCli(Arc::new(manager))
    }

    #[tokio::test]
    async fn tjuae_cli_exposes_mode_as_config_option() {
        let instance = tjuae_cli_instance().await;
        ...
        let mode = response
            ...
            .expect("tjuaecli should expose a mode config option");
    ...
    #[tokio::test]
    async fn tjuae_cli_set_config_option_mode_switches_session_mode() {
        let instance = tjuae_cli_instance().await;
    ...
    #[tokio::test]
    async fn tjuae_cli_set_config_option_rejects_invalid_mode() {
        let instance = tjuae_cli_instance().await;
    ...
    #[tokio::test]
    async fn tjuae_cli_set_config_option_rejects_unavailable_option() {
        let instance = tjuae_cli_instance().await;
