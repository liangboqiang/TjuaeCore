...
use crate::types::{BuildTaskOptions, RuntimeCapabilities, TJUAE_RUNTIME_TOKEN_ENV};
...
impl WorkerTaskManagerImpl {
    ...
    fn refresh_runtime_token_for_new_task(&self, options: &mut BuildTaskOptions) {
        ...
        options
            .context
            .runtime_env
            .retain(|(key, _)| key != TJUAE_RUNTIME_TOKEN_ENV);
        options
            .context
            .runtime_env
            .push((TJUAE_RUNTIME_TOKEN_ENV.to_owned(), issue.token));
    }
}
    ...
    fn kill_and_wait(
        ...
        if let Some((id, slot)) = self.tasks.remove(conversation_id) {
            ...
            return Box::pin(async move {
                match slot
                    .get_or_try_init(|| async { Err(AgentError::internal("任务槽在初始化前已移除")) })
                    .await
                {
...
/// Wired up by `tjuaeui-app` so deleting a conversation tears down its
/// agent process. Without this hook, ACP/tjuae_cli subprocesses keep
...
mod tests {
    ...
    use tjuaeui_common::{AgentKillReason, AgentType, ConversationStatus, ProviderWithModel};
    ...
    fn make_team_options_with_runtime_token(conversation_id: &str, runtime_token: &str) -> BuildTaskOptions {
        ...
        let team = tjuaeui_api_types::TeamSessionBinding {
            team_id: "team-1".into(),
            ...
            move |opts: BuildTaskOptions| {
                ...
                async move {
                    if let Some((_, token)) = opts
                        ...
                        .runtime_env
                        .iter()
                        .find(|(key, _)| key == TJUAE_RUNTIME_TOKEN_ENV)
                    {
    ...
    fn collect_idle_finds_finished_and_warmup_only_stale_acp_tasks() {
        ...
        // Non-ACP (TjuaeCli) + Finished + old activity → should NOT be collected
        insert(
            "conv-tjuaecli",
            mock_instance(
                MockAgent::new("conv-tjuaecli", Some(ConversationStatus::Finished))
