...
use std::path::Path;
use std::sync::Arc;
use tjuaeui_common::{CommandSpec, ErrorChain};
...
impl CliAgentProcess {
    ...
    pub async fn spawn_for_sdk(config: CommandSpec) -> Result<Self, AgentError> {
        ...
        let agent_env = tjuaeui_runtime::agent_process_env().await;
...
mod tests {
    ...
    async fn spawn_for_sdk_uses_clean_agent_env_and_explicit_overrides() {
        const CHILD_ENV: &str = "TJUAE_TEST_SDK_AGENT_ENV_CHILD";
        ...
        if std::env::var_os(CHILD_ENV).is_none() {
            ...
            write_fake_shell(
                ...
                r#"#!/bin/sh
printf '%s\n' \
  'TJUAE_SHELL_ONLY=from-shell' \
  'TJUAE_OVERLAY=from-shell' \
...
"#,
        ...
        let mut config = simple_script_config(
            "printf 'shell=%s\nconfig=%s\noverlay=%s\nnpm=%s\nnode=%s\n' \
             \"${TJUAE_SHELL_ONLY:-unset}\" \
             \"${TJUAE_CONFIG_ONLY:-unset}\" \
             \"${TJUAE_OVERLAY:-unset}\" \
             \"${npm_lifecycle_event:-unset}\" \
             \"${NODE_OPTIONS:-unset}\"",
        );
        config.env.push(EnvVar {
            name: "TJUAE_CONFIG_ONLY".into(),
            value: "from-config".into(),
        });
        config.env.push(EnvVar {
            name: "TJUAE_OVERLAY".into(),
            value: "from-config".into(),
        });
