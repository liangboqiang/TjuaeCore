...
pub(super) fn prepare_command_cwd(cwd: &str) -> Result<PathBuf, AgentError> {
    if cwd.trim().is_empty() {
        return Err(AgentError::bad_request("工作区目录为空"));
    }
...
/// to the ACP SDK transport. TjuaeCli uses its own manager and does not rely on
...
impl CliAgentProcess {
    ...
    pub async fn kill(&self, grace_period: Duration) -> Result<(), AgentError> {
        ...
        .await
        .map_err(|_| AgentError::internal(format!("进程 {} 在强制终止后仍未退出", self.pid)))?;
