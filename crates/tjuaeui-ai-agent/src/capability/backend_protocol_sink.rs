...
use serde_json::json;
use tjuae_protocol::events::{ProtocolEvent, ToolCategory};
use tjuae_protocol::writer::ProtocolEmitter;
use tjuaeui_common::{Confirmation, ConfirmationOption, generate_id};
use tokio::sync::broadcast;
use tracing::debug;

use crate::protocol::events::{AcpPermissionEventData, AgentStreamEvent, ToolCallEventData, ToolCallStatus};

/// Implements `ProtocolEmitter` for the tjuaecore context.
///
/// Bridges tjuae_cli `ProtocolEvent` emissions to `AgentStreamEvent` on a
...
mod tests {
    use super::*;
    use tjuae_protocol::events::ToolInfo;
