...
use tjuaeui_api_types::{
    ...
    SetEnabledRequest, TryConnectA2aAgentRequest, TryConnectA2aAgentResponse, TryConnectCustomAgentRequest,
...
pub fn agent_routes(state: AgentRouterState) -> Router {
    Router::new()
        ...
        .route("/api/agents/health-check", post(health_check_all))
        ...
        .route("/api/agents/custom/try-connect-a2a", post(try_connect_a2a))
        .with_state(state)
}
...
async fn try_connect_a2a(
    State(state): State<AgentRouterState>,
    Extension(_user): Extension<CurrentUser>,
    body: Result<Json<TryConnectA2aAgentRequest>, JsonRejection>,
) -> Result<Json<ApiResponse<TryConnectA2aAgentResponse>>, ApiError> {
    let Json(req) = body.map_err(ApiError::from)?;
    Ok(Json(ApiResponse::ok(
        state
            .service
            .try_connect_a2a_agent(req)
            .await
            .map_err(agent_error_to_api_error)?,
    )))
}
