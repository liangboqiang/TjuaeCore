//! `SessionAgentTask` — adapts the clean-slate `tjuaeui_session::SessionBackend`
...
use tjuaeui_api_types::AcpBuildExtra;
...
fn resolve_current_model_efforts(models: &[tjuaeui_session::ModelInfo], current_model: Option<&str>) -> Vec<String> {
...
/// handshake, in the SAME `tjuaeui_session` shape the getters read off live
...
struct CatalogPreload {
    available_models: Vec<tjuaeui_session::ModelInfo>,
    current_model: Option<String>,
    available_modes: Vec<tjuaeui_session::ModeInfo>,
...
impl CatalogPreload {
    ...
    fn from_handshake(handshake: &tjuaeui_api_types::AgentHandshake) -> Self {
        ...
        let (available_models, current_model) = handshake
            ...
            .map(|state| {
                let models = state
                    .available_models
                    .iter()
                    .map(|m| tjuaeui_session::ModelInfo {
        ...
        let (available_modes, current_mode) = handshake
            ...
            .map(|state| {
                let modes = state
                    .available_modes
                    .iter()
                    .map(|m| tjuaeui_session::ModeInfo {
                        id: m.id.to_string(),
                        name: m.name.clone(),
                        description: m.description.clone(),
                    })
...
impl SessionAgentTask {
    ...
    pub fn new_with_preload(
        ...
        backend: Arc<dyn SessionBackend>,
        session_repo: Option<Arc<dyn IAcpSessionRepository>>,
        handshake: &tjuaeui_api_types::AgentHandshake,
        prompt_dump: Option<SessionPromptDump>,
    ) -> Arc<Self> {
        Self::build(
            agent_type,
        ...
        self.backend
            ...
            .map(|p| {
                ...
                tjuaeui_common::Confirmation {
                    ...
                    options: options
                        .into_iter()
                        .map(|o| tjuaeui_common::ConfirmationOption {
                            label: o.name,
    ...
    pub fn confirm(
        ...
        use tjuaeui_session::PermissionDecision;
    ...
    fn effective_catalog(
        ...
        Vec<tjuaeui_session::ModelInfo>,
        Option<String>,
        Vec<tjuaeui_session::ModeInfo>,
        Option<String>,
    ) {
    ...
    pub async fn mode(&self) -> Result<tjuaeui_api_types::AgentModeResponse, AgentError> {
        let caps = self.backend.capabilities();
        ...
        let current_mode = caps.current_mode.or_else(|| self.catalog_preload.current_mode.clone());
        Ok(tjuaeui_api_types::AgentModeResponse {
    ...
    pub async fn get_model(&self) -> Result<tjuaeui_api_types::GetModelInfoResponse, AgentError> {
        ...
        if models.is_empty() && current_model.is_none() && override_model.is_none() {
            return Ok(tjuaeui_api_types::GetModelInfoResponse { model_info: None });
        }
        let available_models: Vec<tjuaeui_api_types::ModelInfoEntry> = models
            .iter()
            .map(|m| tjuaeui_api_types::ModelInfoEntry {
                id: m.id.clone(),
                label: m.name.clone(),
            })
            .collect();
        ...
        Ok(tjuaeui_api_types::GetModelInfoResponse {
            model_info: Some(tjuaeui_api_types::ModelInfoPayload {
    ...
    pub async fn get_config_options(&self) -> Result<tjuaeui_api_types::GetConfigOptionsResponse, AgentError> {
        // Live catalog wins; cold-start resume falls back to the persisted-handshake
        ...
        if !modes.is_empty() {
            config_options.push(tjuaeui_api_types::AcpConfigOptionDto {
                id: "mode".into(),
                name: Some("Mode".into()),
                label: None,
                description: None,
                ...
                options: modes
                    .iter()
                    .map(|m| tjuaeui_api_types::AcpConfigSelectOptionDto {
                        value: m.id.clone(),
        ...
        // path's `thought_level` config option (category-keyed so TjuaeUI's
        ...
        if !efforts.is_empty() {
            config_options.push(tjuaeui_api_types::AcpConfigOptionDto {
                ...
                options: efforts
                    .iter()
                    .map(|e| tjuaeui_api_types::AcpConfigSelectOptionDto {
                        value: e.clone(),
                        name: Some(e.clone()),
                        label: None,
                        description: None,
        ...
        Ok(tjuaeui_api_types::GetConfigOptionsResponse { config_options })
    }
    ...
    pub async fn set_config_option(
        ...
        option_id: &str,
        value: &str,
    ) -> Result<tjuaeui_api_types::SetConfigOptionResponse, AgentError> {
        ...
        match option_id {
            ...
            _ => {
                return Ok(tjuaeui_api_types::SetConfigOptionResponse {
                    confirmation: tjuaeui_api_types::ConfigOptionConfirmation::CommandAck,
                    config_options: None,
                });
            }
        }
        ...
        let observed = snapshot
            ...
            == Some(value);
        Ok(tjuaeui_api_types::SetConfigOptionResponse {
            confirmation: if observed {
                tjuaeui_api_types::ConfigOptionConfirmation::Observed
            } else {
                tjuaeui_api_types::ConfigOptionConfirmation::CommandAck
            },
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<tjuaeui_api_types::SlashCommandItem>, AgentError> {
        let caps = self.backend.capabilities();
        Ok(caps
            .slash_commands
            .iter()
            .map(|c| tjuaeui_api_types::SlashCommandItem {
    ...
    async fn cancel(&self) -> Result<(), AgentError> {
        ...
        self.backend
            .dispatch(Command::Cancel {
                target: tjuaeui_session::CancelTarget::Turn,
            })
            .await
            .map(|_| ())
...
pub struct SessionBuildInputs<'a> {
    ...
    /// 用户主动选择的工作区不由 Tjuae 写入信任；自动会话工作区由启动参数临时信任。
    ...
    /// alias (`yolo` → the row's `yolo_id`; codex `default`/`autoEdit`
    ...
    pub metadata: &'a tjuaeui_api_types::AgentMetadata,
...
fn spec_mode_model(
    ...
    metadata: &tjuaeui_api_types::AgentMetadata,
) -> (tjuaeui_session::SessionSpec, Option<String>, Option<String>) {
    use tjuaeui_session::SessionSpec;
    ...
    // transform the ACP path applies in `initial_mode_from_params`. TjuaeUI persists
    // generic aliases (`yolo`; codex `default`/`autoEdit`); handing
...
/// assembly (`crates/tjuaeui-app/src/session_runtime/mod.rs`): it resolves the
...
pub async fn build_session_instance(
    ...
    spawner: Arc<dyn tjuaeui_process::Spawner>,
...
mod managed_workspace_trust_tests {
    ...
    #[test]
    fn trusts_only_non_empty_managed_workspaces() {
        assert!(codex_managed_workspace_trust_args("", false).is_empty());
        assert!(codex_managed_workspace_trust_args("C:\\work\\user-project", true).is_empty());
...
///     path injected these via `resolve_agent_command_spec`; `TJUAE_*`/`PATH`/…
///     keys are already filtered at the registry (`is_blocked_override_env_key`),
///     so they cannot shadow the runtime context below.
///  2. the `TJUAE_*` conversation runtime context (`TJUAE_USER_ID` /
///     `TJUAE_CONVERSATION_ID` / `TJUAE_HELPER_BIN` / `TJUAE_BASE_URL` /
///     `TJUAE_RUNTIME_TOKEN`) — the legacy path appended these via
///     `apply_acp_launch_policy` for every agent spawn.
fn assemble_spawn_env(
    agent_env: &[tjuaeui_api_types::AgentEnvEntry],
    runtime_env: &[(String, String)],
) -> Vec<tjuaeui_common::EnvVar> {
    let mut env: Vec<tjuaeui_common::EnvVar> = agent_env
        .iter()
        ...
        .collect();
    env.extend(runtime_env.iter().map(|(name, value)| tjuaeui_common::EnvVar {
        name: name.clone(),
        value: value.clone(),
    }));
    env
...
fn build_session_cli_config_dump_value(backend: &str, cfg: &tjuaeui_session::SessionConfig) -> serde_json::Value {
    use tjuaeui_session::McpTransport;
...
fn team_mcp_server_spec(cfg: &tjuaeui_api_types::TeamMcpStdioConfig) -> tjuaeui_session::McpServerSpec {
    use tjuaeui_api_types::TeamMcpStdioConfig as C;
    tjuaeui_session::McpServerSpec {
        name: tjuaeui_api_types::TEAM_MCP_SERVER_NAME.to_owned(),
        transport: tjuaeui_session::McpTransport::Stdio {
...
pub fn spawn_catalog_writeback(
    agent_id: String,
    backend: Arc<dyn tjuaeui_session::SessionBackend>,
...
fn codex_sandbox_for_mode(mode: Option<&str>) -> Option<&'static str> {
    match mode.map(str::trim) {
        ...
        // `normalize_requested_mode` now resolves yolo to it). `full-access` and
        // `:danger-full-access` remain equivalent danger-full-access values.
        Some(":danger-full-access" | "agent-full-access" | "full-access") => Some("danger-full-access"),
...
/// token `full-access` (the Plan B canonical value) and the colon id
/// `:danger-full-access`. Verbatim port of clean-slate
/// `session_runtime::codex_approval_for_mode`.
fn codex_approval_for_mode(mode: Option<&str>) -> Option<&'static str> {
    match mode.map(str::trim) {
        // Recognizes the canonical `agent-full-access` alongside `full-access`
        // and `:danger-full-access` (see codex_sandbox_for_mode).
        Some(":danger-full-access" | "agent-full-access" | "full-access") => Some("never"),
        _ => None,
    }
}
...
fn spawn_event_pump(
    ...
    tokio::spawn(async move {
        ...
        while let Some(env) = events.next().await {
            ...
            {
                let mut config_options: Vec<tjuaeui_api_types::AcpConfigOptionDto> = Vec::new();
                if !modes.is_empty() {
                    config_options.push(tjuaeui_api_types::AcpConfigOptionDto {
                        id: "mode".into(),
                        name: Some("Mode".into()),
            ...
            if let SessionEvent::SubagentUpdate { r#ref, status, .. } = &env.event {
                use tjuaeui_session::SubagentStatus;
                match status {
            ...
            let suppress_intermediate_finish = matches!(&env.event, SessionEvent::TurnResult { is_error, outcome, .. }
                if !workflow_inflight.is_empty()
                    && !*is_error
                    && !matches!(outcome, tjuaeui_session::TurnOutcome::Cancelled { .. }));
...
/// `Orchestrator` uses (`tjuaeui_session::is_unrecoverable_resume_error`), so a
...
fn is_dead_resume_anchor(event: &SessionEvent) -> bool {
    use tjuaeui_session::{TurnOutcome, is_unrecoverable_resume_error};
    ...
    let reason = tjuaeui_session::ErrorReason::Backend {
...
fn empty_turn_tip(outcome: &tjuaeui_session::TurnOutcome) -> Option<TipsEventData> {
    use tjuaeui_session::{StopReason, TruncationKind, TurnOutcome};
...
fn translate_event(event: SessionEvent, conversation_id: &str, terminal_result_seen: bool) -> Vec<AgentStreamEvent> {
    match event {
        ...
        SessionEvent::TurnResult {
            ...
            let is_cancel = matches!(outcome, tjuaeui_session::TurnOutcome::Cancelled { .. });
        ...
        SessionEvent::Detached { exit, redacted_summary } => {
            ...
            match tjuaeui_session::crash_outcome(terminal_result_seen, exit) {
                tjuaeui_session::Outcome::Crashed => {
                ...
                tjuaeui_session::Outcome::CleanNoResult | tjuaeui_session::Outcome::FollowResult => {
                    vec![AgentStreamEvent::Finish(FinishEventData::default())]
                }
            }
        }
            ...
            let entries: Vec<serde_json::Value> = entries
                ...
                .map(|e| {
                    let status = match e.status {
                        tjuaeui_session::PlanStatus::Pending => "pending",
                        tjuaeui_session::PlanStatus::InProgress => "in_progress",
                        tjuaeui_session::PlanStatus::Completed => "completed",
                    };
...
mod build_mapping_tests {
    ...
    fn assemble_spawn_env_orders_agent_overrides_before_runtime_context() {
        let agent_env = vec![
            tjuaeui_api_types::AgentEnvEntry {
        ...
        let runtime_env = vec![
            ("TJUAE_USER_ID".to_owned(), "user-1".to_owned()),
            ("TJUAE_RUNTIME_TOKEN".to_owned(), "tok".to_owned()),
        ];
        ...
        assert_eq!(
            names,
            ["AWS_PROFILE", "HTTPS_PROXY", "TJUAE_USER_ID", "TJUAE_RUNTIME_TOKEN"]
        );
    ...
    fn session_cli_config_dump_value_captures_resolved_surface() {
        use tjuaeui_session::{McpServerSpec, McpTransport, SessionConfig, SessionInit};
        let cfg = SessionConfig {
            ...
            spawn_env: vec![tjuaeui_common::EnvVar {
    ...
    fn test_metadata(backend: Option<&str>, yolo_id: Option<&str>) -> tjuaeui_api_types::AgentMetadata {
        use tjuaeui_api_types::{AgentHandshake, AgentMetadata, AgentSource, AgentSourceInfo, BehaviorPolicy};
        use tjuaeui_common::AgentType;
        AgentMetadata {
            id: "test".into(),
            icon: None,
    ...
    fn mode_alias_is_normalized_via_catalog() {
        // codex: yolo → the row's yolo_id (full-access); default → auto.
        ...
        assert_eq!(mode.as_deref(), Some("full-access"), "yolo → codex native yolo_id");
    ...
    fn catalog_partial_projects_discovered_modes_models_commands() {
        use tjuaeui_session::{ModeInfo, ModelInfo, SlashCommandInfo};
        let caps = tjuaeui_session::Capabilities {
...
mod translate_tests {
    use super::*;
    use crate::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
    use tjuaeui_session::PermissionKind;
    ...
    fn plan_surfaces_with_snake_case_status() {
        use tjuaeui_session::{PlanEntry, PlanStatus};
        let events = translate_event(
            SessionEvent::Plan {
                entries: vec![
                    PlanEntry {
    ...
    fn cancelled_turn_emits_finish_without_error() {
        use tjuaeui_session::{CancelReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
                api_error_status: None,
    ...
    fn errored_turn_emits_rich_error_terminal() {
        use tjuaeui_session::{StopReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
    ...
    fn detached_crash_surfaces_as_rich_disconnect_error() {
        use tjuaeui_session::ExitStatusLite;
        let events = translate_event(
    ...
    fn tip_code(outcome: tjuaeui_session::TurnOutcome) -> Option<(TipType, String)> {
...
mod persist_tests {
    ...
    use super::*;
    use tjuaeui_db::{CreateAcpSessionParams, IAcpSessionRepository, SqliteAcpSessionRepository, init_database_memory};
    ...
    async fn seeded_repo() -> (Arc<dyn IAcpSessionRepository>, tjuaeui_db::Database) {
    ...
    async fn send_dispatch_session_not_found_clears_anchor_and_classifies() {
        ...
        assert_eq!(
            err.code(),
            Some(tjuaeui_api_types::AgentErrorCode::UserAgentSessionNotFound),
    ...
    async fn set_config_option_effort_returns_observed_via_override() {
        ...
        assert!(
            matches!(resp.confirmation, tjuaeui_api_types::ConfigOptionConfirmation::Observed),
    ...
    fn is_dead_resume_anchor_matrix() {
        use tjuaeui_session::{CancelReason, StopReason, TurnOutcome};
...
mod pump_tests {
    ...
    async fn config_change_emits_no_frame_and_bound_id_reaches_finish() {
        let script = vec![
            ...
            env(SessionEvent::TurnResult {
                ...
                epoch: 0,
                outcome: tjuaeui_session::TurnOutcome::EndTurn,
            }),
        ];
    ...
    async fn cancelled_turn_closes_open_tool_calls_as_canceled_before_finish() {
        use tjuaeui_session::{CancelReason, SubagentKind, TurnOutcome};
        let script = vec![
    ...
    async fn completed_tool_call_is_not_recanceled_at_turn_end() {
        use tjuaeui_session::SubagentKind;
        let script = vec![
            env(SessionEvent::TurnStarted { epoch: 1 }),
            env(SessionEvent::ToolCall {
                tool_use_id: "call-1".into(),
    ...
    async fn catalog_updated_projects_config_option_with_model_and_mode() {
        use tjuaeui_session::{ModeInfo, ModelInfo};
    ...
    async fn catalog_updated_projects_available_commands_frame() {
        use tjuaeui_session::SlashCommandInfo;
        let script = vec![env(SessionEvent::CatalogUpdated {
            models: Vec::new(),
            modes: Vec::new(),
            slash_commands: vec![
    ...
    async fn workflow_launch_result_finish_is_suppressed_until_workflow_completes() {
        ...
        let script = vec![
            env(SessionEvent::ToolCall {
                ...
                name: "Task".into(),
                subagent: tjuaeui_session::SubagentKind::Workflow,
                input: serde_json::Value::Null,
                parent_tool_use_id: None,
            }),
    ...
    struct PendingPermBackend(tjuaeui_session::PendingPermissionView);
    ...
    impl SessionBackend for PendingPermBackend {
        ...
        fn pending_permission_requests(&self) -> Vec<tjuaeui_session::PendingPermissionView> {
            vec![self.0.clone()]
        }
    }
    ...
    async fn get_confirmations_recovers_pending_ask_user_question() {
        let backend: Arc<dyn SessionBackend> = Arc::new(PendingPermBackend(tjuaeui_session::PendingPermissionView {
    ...
    fn handshake_with_catalog() -> tjuaeui_api_types::AgentHandshake {
        tjuaeui_api_types::AgentHandshake {
    ...
    async fn live_capabilities_overwrite_stale_preload() {
        ...
        let stale = tjuaeui_api_types::AgentHandshake {
            available_models: Some(serde_json::json!({
