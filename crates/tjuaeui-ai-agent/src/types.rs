    ...
    pub fn apply_conversation_runtime_context(
        ...
        self.context.runtime_env.retain(|(key, _)| {
            !matches!(
                key.as_str(),
                TJUAE_USER_ID_ENV
                    | TJUAE_CONVERSATION_ID_ENV
                    | TJUAE_HELPER_BIN_ENV
                    | TJUAE_BASE_URL_ENV
        ...
        self.context
            .runtime_env
            .push((TJUAE_USER_ID_ENV.to_owned(), user_id.to_owned()));
        self.context
            .runtime_env
            .push((TJUAE_CONVERSATION_ID_ENV.to_owned(), conversation_id.to_owned()));
        if let Some(helper_bin) = helper_bin {
            self.context
                .runtime_env
                .push((TJUAE_HELPER_BIN_ENV.to_owned(), helper_bin.to_owned()));
        }
        if let Some(base_url) = base_url {
            self.context
                .runtime_env
                .push((TJUAE_BASE_URL_ENV.to_owned(), base_url.to_owned()));
        }
        if let Some(runtime_token) = runtime_token {
            self.context
                .runtime_env
                .push((TJUAE_RUNTIME_TOKEN_ENV.to_owned(), runtime_token.to_owned()));
        }
        self.runtime_capabilities.conversation_runtime_context_version = Some(CONVERSATION_RUNTIME_CONTEXT_VERSION);
    }
}

pub const TJUAE_USER_ID_ENV: &str = "TJUAE_USER_ID";
pub const TJUAE_CONVERSATION_ID_ENV: &str = "TJUAE_CONVERSATION_ID";
pub const TJUAE_HELPER_BIN_ENV: &str = "TJUAE_HELPER_BIN";
pub const TJUAE_BASE_URL_ENV: &str = "TJUAE_BASE_URL";
pub const TJUAE_RUNTIME_TOKEN_ENV: &str = "TJUAE_RUNTIME_TOKEN";
...
pub struct TjuaeCliCompatOverrides {
...
/// Fully resolved TjuaeCli configuration passed to the agent manager.
#[derive(Debug, Clone)]
pub struct TjuaeCliResolvedConfig {
    ...
    /// Normal TjuaeUI conversations leave this unset.
    ...
    pub compat_overrides: TjuaeCliCompatOverrides,
    /// Directory for tjuae_cli session persistence files.
    ...
    pub extra_mcp_servers: HashMap<String, tjuae_config::config::McpServerConfig>,
    /// AWS Bedrock credentials (region + access key or profile).
    pub bedrock_config: Option<tjuae_config::config::BedrockConfig>,
...
mod tests {
    use super::*;
    use serde_json::json;
    use tjuaeui_api_types::{AcpBuildExtra, AcpModelInfo, SlashCommandItem, TjuaeCliBuildExtra};
    ...
    fn build_task_options_applies_conversation_runtime_context_once() {
        ...
        use tjuaeui_common::{AgentType, ProviderWithModel};
        ...
        let context = AgentSessionContext {
            ...
            runtime_env: vec![
                ...
                (TJUAE_RUNTIME_TOKEN_ENV.into(), "old-token".into()),
        ...
        options.apply_conversation_runtime_context(
            "user-1",
            "conv-1",
            Some("/Applications/TjuaeUI/tjuaecore"),
        ...
        assert_eq!(
            options
                ...
                .runtime_env
                .iter()
                .filter(|(key, _)| key == TJUAE_USER_ID_ENV)
                .count(),
            1
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(TJUAE_USER_ID_ENV.to_owned(), "user-1".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(TJUAE_CONVERSATION_ID_ENV.to_owned(), "conv-1".to_owned()))
        );
        ...
        assert!(
            options
                .context
                .runtime_env
                .contains(&(TJUAE_BASE_URL_ENV.to_owned(), "http://127.0.0.1:25808".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(TJUAE_RUNTIME_TOKEN_ENV.to_owned(), "runtime-token-1".to_owned()))
        );
        assert_eq!(
            options
                .context
                .runtime_env
                .iter()
                .filter(|(key, _)| key == TJUAE_RUNTIME_TOKEN_ENV)
                .count(),
            1
        );
    ...
    #[test]
    fn tjuae_cli_build_extra_serde_defaults() {
        let json = json!({});
        let extra: TjuaeCliBuildExtra = serde_json::from_value(json).unwrap();
    ...
    #[test]
    fn tjuae_cli_build_extra_serde_with_overrides() {
        let json = json!({
    ...
    #[test]
    fn tjuae_cli_build_extra_serde_with_preset_rules() {
        let json = json!({
    ...
    #[test]
    fn tjuae_cli_build_extra_accepts_frozen_skills_snapshot() {
        let json = json!({
            "preset_rules": "Rules",
            "skills": ["pdf", "cron"]
        });
