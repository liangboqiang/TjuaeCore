...
use tjuaeui_common::{RemoteAgentAuthType, RemoteAgentProtocol, RemoteAgentStatus, decrypt_string, encrypt_string};
use tjuaeui_db::models::RemoteAgentRow;
use tjuaeui_db::{IRemoteAgentRepository, UpdateRemoteAgentParams};
...
impl RemoteAgentService {
    ...
    pub async fn get(&self, id: &str) -> Result<RemoteAgentResponse, AgentError> {
        let row = self
            ...
            .await
            .map_err(db_err)?
            .ok_or_else(|| AgentError::not_found(format!("找不到远程 Agent“{id}”")))?;
    ...
    pub async fn create(&self, req: CreateRemoteAgentRequest) -> Result<RemoteAgentResponse, AgentError> {
        ...
        let row = self
            .repo
            .create(tjuaeui_db::CreateRemoteAgentParams {
                name: &req.name,
    ...
    pub async fn update(&self, id: &str, req: UpdateRemoteAgentRequest) -> Result<RemoteAgentResponse, AgentError> {
        ...
        let row = self.repo.update(id, params).await.map_err(|e| match e {
            tjuaeui_db::DbError::NotFound(msg) => AgentError::not_found(msg),
    ...
    pub async fn test_connection(&self, req: TestRemoteAgentConnectionRequest) -> Result<(), AgentError> {
        ...
        let result = tokio::time::timeout(Duration::from_secs(10), async {
            tokio::task::spawn_blocking(move || {
                tungstenite::connect(&url)
                    .map(|_| ())
                    .map_err(|e| AgentError::bad_gateway(format!("WebSocket 连接失败：{e}")))
            })
            .await
            .map_err(|e| AgentError::internal(format!("任务连接错误：{e}")))?
        })
        .await;
        ...
        match result {
            Ok(Ok(_)) => Ok(()),
            Ok(Err(e)) => Err(e),
            Err(_) => Err(AgentError::timeout("连接在 10 秒后超时")),
        }
    }
    ...
    pub async fn handshake(&self, id: &str) -> Result<HandshakeResponse, AgentError> {
        ...
        let connect_result = tokio::time::timeout(Duration::from_secs(15), async {
            tokio::task::spawn_blocking(move || {
                tungstenite::connect(&url)
                    .map(|_| ())
                    .map_err(|e| AgentError::bad_gateway(format!("握手连接失败：{e}")))
            })
            .await
            .map_err(|e| AgentError::internal(format!("任务连接错误：{e}")))?
        })
        ...
        match connect_result {
            ...
            Err(_) => {
                let _ = self.repo.update_status(id, "error", None).await;
                Err(AgentError::timeout("握手在 15 秒后超时"))
            }
        }
    }
...
fn validate_create_request(req: &CreateRemoteAgentRequest) -> Result<(), AgentError> {
    ...
    if req.url.trim().is_empty() {
        return Err(AgentError::bad_request("url 不能为空"));
    }
    validate_ws_url(&req.url)
}
...
fn validate_ws_url(url: &str) -> Result<(), AgentError> {
    if !url.starts_with("ws://") && !url.starts_with("wss://") {
        return Err(AgentError::bad_request("URL 必须使用 ws:// 或 wss:// 协议"));
    }
    Ok(())
}
...
fn generate_device_keypair(encryption_key: &[u8; 32]) -> Result<(String, String, String), AgentError> {
    ...
    getrandom::getrandom(&mut rng_bytes).map_err(|e| AgentError::internal(format!("随机数生成失败：{e}")))?;

    let signing_key = SigningKey::from_bytes(&rng_bytes);
    let verifying_key = signing_key.verifying_key();

    let device_id = tjuaeui_common::generate_prefixed_id("dev");
...
fn db_err(e: tjuaeui_db::DbError) -> AgentError {
