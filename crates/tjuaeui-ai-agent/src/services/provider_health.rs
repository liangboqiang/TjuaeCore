...
use tjuae_agent::output::null_sink::NullSink;
use tjuae_config::config::{CliArgs, Config};
use tjuaeui_api_types::{
    HealthStatus, ProviderHealthCheckErrorKind, ProviderHealthCheckRequest, ProviderHealthCheckResponse,
};
use tjuaeui_db::{IProviderRepository, models::Provider};
use tracing::{info, warn};

use crate::factory::tjuae_cli::{
    map_tjuae_cli_provider, resolve_bedrock_config, resolve_model_compat_overrides,
    resolve_tjuae_cli_url_and_compat_with_mode,
};
use crate::types::TjuaeCliResolvedConfig;
...
impl ProviderHealthCheckService {
    ...
    pub async fn health_check(
        ...
        if req.provider_id.trim().is_empty() {
            return Err(AgentError::bad_request("provider_id 为必填项"));
        }
        if req.model.trim().is_empty() {
            return Err(AgentError::bad_request("model 为必填项"));
        }
    ...
    fn resolve_probe_config(&self, row: &Provider, model_id: &str) -> Result<TjuaeCliResolvedConfig, AgentError> {
        let api_key = tjuaeui_common::decrypt_string(&row.api_key_encrypted, &self.encryption_key)
        ...
        Ok(TjuaeCliResolvedConfig {
            provider,
            api_key,
            model: model_id.to_owned(),
            base_url,
            system_prompt: Some("你是模型提供商健康探针。只回复 OK，不要使用工具。".into()),
            max_tokens: Some(HEALTH_CHECK_MAX_TOKENS),
            max_turns: Some(1),
            max_tool_call_malformed_turns: Some(1),
            max_tool_call_failure_turns: Some(1),
            compat_overrides,
            session_directory: self.data_dir.join("tjuaecli-health-check-sessions"),
            session_mode: None,
            skills: Vec::new(),
...
async fn run_probe(
    ...
    let mut engine = match build_probe_engine(config_extra).await {
        Ok(engine) => engine,
        Err(error) => {
            let message = format!("TjuaeCLI 探测引擎启动失败：{error}");
    ...
    .await
    {
        ...
        Err(_) => {
            let response = unhealthy_response(
                ...
                model,
                started.elapsed(),
                format!("健康检查超时（{} 秒）", HEALTH_CHECK_TIMEOUT.as_secs()),
...
async fn build_probe_engine(config_extra: TjuaeCliResolvedConfig) -> Result<AgentEngine, AgentError> {
    ...
    let mut config =
        Config::resolve(&cli_args).map_err(|error| AgentError::internal(format!("解析配置失败：{error}")))?;
...
mod tests {
    ...
    use tjuaeui_common::encrypt_string;
    use tjuaeui_db::{CreateProviderParams, DbError, UpdateProviderParams};
    ...
    fn test_service() -> ProviderHealthCheckService {
        ProviderHealthCheckService {
            ...
            data_dir: PathBuf::from("/tmp/tjuaecore-provider-health-test"),
        }
    }
