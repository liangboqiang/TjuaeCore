...
use tjuaeui_db::{AgentMetadataRow, IAgentMetadataRepository, UpdateAgentHandshakeParams};
use tjuaeui_runtime::{RuntimeCommandProbe, probe_node_runtime_supported, probe_runtime_command, resolve_command_path};
...
impl AgentRegistry {
    ...
    async fn apply_handshake_inner(&self, id: &str, snapshot: &AgentHandshake) -> Result<(), AgentError> {
        ...
        let Some(row) = self
            ...
            .await
            .map_err(|e| AgentError::internal(format!("应用握手数据失败：{e}")))?
        else {
            return Ok(());
        };
...
impl AgentRegistry {
    ...
    /// 返回管理记录前持久化显式可用性探测发现的目录。
    ///
    /// 探测调用方需要写后读语义，才能立即填充模型选择器。
    pub(crate) async fn apply_probe_handshake(&self, id: &str, handshake: &AgentHandshake) -> Result<(), AgentError> {
        self.apply_handshake_inner(id, handshake).await
    }
    ...
    pub async fn hydrate(&self) -> Result<(), AgentError> {
        let rows = self
            ...
            .list_all()
            .await
            .map_err(|e| AgentError::internal(format!("加载 agent_metadata 失败：{e}")))?;
    ...
    pub async fn run_slow_probe_recheck(&self) {
        ...
        for id in pending {
            ...
            let params = tjuaeui_db::UpdateAgentAvailabilitySnapshotParams {
    ...
    pub async fn reload_one(&self, id: &str) -> Result<Option<AgentMetadata>, AgentError> {
        let row = self
            ...
            .get(id)
            .await
            .map_err(|e| AgentError::internal(format!("加载 agent_metadata“{id}”失败：{e}")))?;
    ...
    /// `false`. Internal rows (e.g. the tjuae_cli row) intentionally
...
fn decode_row(
    ...
    let inferred_team_capable = behavior_policy.supports_team
        || tjuaeui_common::constants::is_team_capable(backend_str, handshake.agent_capabilities.as_ref());
    ...
    let is_internal_tjuae_cli = is_internal_tjuae_cli(&meta);
    if is_internal_tjuae_cli && command_override.is_some() {
        warn!(
            id = %meta.id,
            name = %meta.name,
            "Ignoring command override for internal TjuaeCLI agent"
        );
    }
    let env_override = parse_env_override(&env_override_raw);
    if is_internal_tjuae_cli && env_override.as_ref().is_some_and(|entries| !entries.is_empty()) {
        warn!(
            id = %meta.id,
            name = %meta.name,
            "Ignoring environment overrides for internal TjuaeCLI agent"
        );
    }

    meta.has_command_override = command_override.is_some() && !is_internal_tjuae_cli;
    meta.env_override_key_count = env_override
        .as_ref()
        .filter(|_| !is_internal_tjuae_cli)
        .map(|v| v.iter().filter(|e| !is_blocked_override_env_key(&e.name)).count())
        .unwrap_or(0);

    if is_internal_tjuae_cli {
        meta.command = None;
    } else if let Some(path) = command_override {
        meta.command = Some(path);
    }
    if !is_internal_tjuae_cli && let Some(extra) = env_override {
        for entry in extra {
...
fn is_internal_tjuae_cli(meta: &AgentMetadata) -> bool {
    meta.agent_type == AgentType::TjuaeCli && meta.agent_source == AgentSource::Internal
}

fn is_internal_commandless_agent(meta: &AgentMetadata) -> bool {
    meta.enabled
        && meta.command.is_none()
        && (meta.agent_source == AgentSource::Internal || meta.agent_type == AgentType::A2a)
}
...
/// unavailable rows go to `info!` so the default tjuaecore.log surfaces
...
fn guidance_for_unavailable_reason(reason: &UnavailableReason) -> String {
    match reason {
        UnavailableReason::Disabled => "启用此 Agent 后即可重新使用。".to_owned(),
        UnavailableReason::NoCommand => "为此 Agent 配置启动命令，然后重新运行连接测试。".to_owned(),
        UnavailableReason::BridgeMissing { bridge } => {
            format!("安装 `{bridge}` 并确保它可在 PATH 中使用，然后重新运行连接测试。")
        }
        UnavailableReason::PrimaryMissing { binary } => {
            format!("安装 `{binary}` 并确保它可在 PATH 中使用，然后重新运行连接测试。")
        }
        UnavailableReason::PrimaryUnusable { binary, .. } => {
            format!("修复或重新安装 `{binary}`，然后重新运行连接测试。")
        }
        UnavailableReason::CommandMissing { command } => {
            format!("安装 `{command}` 并确保它可在 PATH 中使用，然后重新运行连接测试。")
        }
        UnavailableReason::ManagedRuntimeUnavailable { resource, .. } => {
            format!("修复或重新安装托管的 `{resource}` 运行时，然后重新运行连接测试。")
        }
    }
}

/// Keys a user-supplied env override must never set — they would corrupt the
/// agent's runtime environment or TjuaeUI-internal wiring. Case-insensitive.
pub(crate) fn is_blocked_override_env_key(key: &str) -> bool {
    let upper = key.to_ascii_uppercase();
    if upper.starts_with("TJUAE_") || upper.starts_with("TJUAEUI_") {
        return true;
    }
    matches!(
        upper.as_str(),
...
pub(crate) fn guidance_for_snapshot_error_code(error_code: &str) -> &'static str {
    match error_code {
        "command_not_found" => "安装所需 CLI 并确保它可在 PATH 中使用，然后重新运行连接测试。",
        "acp_init_failed" => "已找到 CLI，但 ACP 初始化失败。请在 CLI 中完成登录或设置，然后重新运行连接测试。",
        "auth_required" => {
            "可以连接 Agent，但它要求登录。请通过 CLI 登录（或在环境变量中添加所需 API 密钥），然后重新运行连接测试。"
        }
        "health_check_failed" => "打开一次 CLI 以完成首次运行设置或登录流程，然后重新运行连接测试。",
        "session_send_failed" => "修复导致上次会话失败的提供商凭据或网络问题，然后开始新对话。",
        "no_provider" => "在设置中添加并启用模型提供商，然后重新运行连接测试。",
        "version_probe_failed" => {
            "CLI 已位于 PATH 中，但执行 `--version` 失败，安装可能已损坏。请重新安装 CLI，然后重新运行连接测试。"
        }
        "version_probe_timeout" => {
            "CLI 加载时间超过探测时限，通常是磁盘较慢、安装包较大或防病毒软件干扰所致。Agent 仍可能可用；请运行连接测试并使用更长时限验证。"
        }
        _ => "",
    }
}
...
fn encode_optional(value: &Option<Value>, field: &str) -> Result<Option<String>, AgentError> {
    match value {
        Some(v) => serde_json::to_string(v)
            .map(Some)
            .map_err(|e| AgentError::internal(format!("编码 {field} 失败：{e}"))),
        None => Ok(None),
    }
}
...
mod tests {
    use super::*;
    use tjuaeui_db::{SqliteAgentMetadataRepository, init_database_memory};
    ...
    async fn codex_yolo_id_maps_to_agent_full_access() {
        ...
        // Legacy TjuaeUI yolo aliases resolve to Codex's native
    ...
    /// (TjuaeCLI is `agent_source = internal` with no `command`).
    ...
    async fn list_all_filters_out_unavailable_rows() {
        ...
        // TjuaeCLI (internal, no spawn command) is always available.
        assert!(
            visible.iter().any(|m| m.agent_type == AgentType::TjuaeCli),
            "internal tjuaecli row should survive the filter"
        );
    }
    ...
    async fn list_by_agent_type_counts_seed_rows() {
        ...
        assert_eq!(count(AgentType::TjuaeCli), 1);
    }

    #[tokio::test]
    async fn tjuae_cli_internal_row_is_available_without_command() {
        let reg = registry().await;
        let tjuae_cli = reg
            .list_by_agent_type(AgentType::TjuaeCli)
            .await
            .into_iter()
            .next()
            .unwrap();
        assert_eq!(tjuae_cli.agent_source, AgentSource::Internal);
        assert!(tjuae_cli.command.is_none());
        assert!(tjuae_cli.available);
    }
    ...
    async fn diagnostic_snapshot_pairs_rows_with_reasons() {
        ...
        // The internal tjuae_cli row is always available — its reason
        // slot must be None (sanity check that "available" doesn't
        // accidentally co-occur with a reason).
        let tjuae_cli = snapshot
            .iter()
            .find(|(m, _)| m.agent_type == AgentType::TjuaeCli)
            .expect("tjuaecli seed row");
        assert!(tjuae_cli.0.available);
        assert!(tjuae_cli.1.is_none());
    }
    ...
    fn decode_row_applies_command_override() {
        use tjuaeui_db::AgentMetadataRow;
    ...
    #[test]
    fn decode_row_ignores_internal_tjuae_cli_overrides() {
        use tjuaeui_db::AgentMetadataRow;
        let row = AgentMetadataRow {
            id: "632f31d2".to_string(),
            icon: None,
            name: "TjuaeCLI".to_string(),
            name_i18n: None,
            description: None,
            description_i18n: None,
            backend: None,
            agent_type: "tjuaecli".to_string(),
        ...
        assert_eq!(meta.agent_type, AgentType::TjuaeCli);
