...
use tjuaeui_api_types::{AcpBuildExtra, AgentMetadata};
...
fn append_runtime_env(command_spec: &mut CommandSpec, runtime_env: &[(String, String)]) {
    for (name, value) in runtime_env {
        command_spec.env.push(tjuaeui_common::EnvVar {
            name: name.clone(),
            value: value.clone(),
        });
    }
...
fn codex_sandbox_mode_for_requested_mode(mode: Option<&str>) -> &'static str {
    match mode.map(str::trim) {
        Some("agent-full-access" | "full-access") => "danger-full-access",
        _ => "workspace-write",
    }
}
    ...
    fn apply_acp_launch_policy_adds_runtime_env_and_codex_full_access_config() {
        ...
        apply_acp_launch_policy(
            ...
            AcpLaunchPolicyInput {
                ...
                runtime_env: &[("TJUAE_CONVERSATION_ID".into(), "conv-1".into())],
            },
        );
        ...
        assert!(
            command_spec
                .env
                .iter()
                .any(|entry| entry.name == "TJUAE_CONVERSATION_ID" && entry.value == "conv-1")
        );
    }
