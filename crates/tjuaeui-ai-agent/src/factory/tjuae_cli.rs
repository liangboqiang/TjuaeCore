...
use tjuae_agent::session::SessionManager;
...
use tjuaeui_runtime::ensure_runtime_command_with_reporter;
...
pub(super) async fn build(
    ...
    // in tjuae_cli's build_system_prompt). Mirrors the old architecture's
    // `init_history` injection of `[Assistant System Rules]`.
    // TjuaeCliBuildExtra parses `skills` so Team preset snapshots preserve the
    // target contract. Native skill materialization for TjuaeCli is tracked as a
    // separate follow-up because this factory currently has no stable TjuaeCli
    // skill-loading path.
    ...
    if !extra_mcp_servers.is_empty() {
        info!(
            ...
            "Injecting MCP servers into tjuaecli session"
        );
    }
    ...
    let row = deps
        ...
        .await
        .map_err(|e| AgentError::internal(format!("加载提供商配置失败：{e}")))?
        .ok_or_else(|| AgentError::bad_request(format!("找不到提供商“{provider_id}”")))?;

    let api_key = tjuaeui_common::decrypt_string(&row.api_key_encrypted, &deps.encryption_key)
    ...
    let provider = map_tjuae_cli_provider(&row.platform, &model_id, row.model_protocols.as_deref())?;
    let model_overrides = resolve_model_compat_overrides(&model_id, &row.model_settings)?;

    let (base_url, mut compat_overrides) = resolve_tjuae_cli_url_and_compat_with_mode(
        &row.platform,
        &row.base_url,
        &provider,
        &model_id,
    ...
    if provider == "openai" {
        info!(
            ...
            "Resolved TjuaeCLI OpenAI transport"
        );
    }
    ...
    let session_directory = deps.data_dir.join("tjuaecli-sessions");
        ...
        match session_mgr.load(&ctx.conversation_id) {
            Ok(mut session) => {
                ...
                info!(
                    ...
                    "Loaded existing tjuaecli session for resume"
                );
                Some(session)
            }
            Err(e) => {
                debug!(
                    conversation_id = %ctx.conversation_id,
                    error = %e,
                    "No existing TjuaeCLI session found, starting fresh"
                );
                None
            }
        }
    ...
    let config = TjuaeCliResolvedConfig {
        provider,
        api_key,
        model: model_id,
        base_url,
...
pub(crate) fn map_tjuae_cli_provider(
    platform: &str,
    model_id: &str,
...
/// Resolve base_url and compat overrides for the tjuae_cli provider.
///
...
/// `/v1/messages`.
#[cfg(test)]
fn resolve_tjuae_cli_url_and_compat(
    platform: &str,
    raw_base_url: &str,
    mapped_provider: &str,
    model_id: &str,
    is_full_url: bool,
) -> (Option<String>, TjuaeCliCompatOverrides) {
    resolve_tjuae_cli_url_and_compat_with_mode(platform, raw_base_url, mapped_provider, model_id, is_full_url, None)
}

pub(crate) fn resolve_tjuae_cli_url_and_compat_with_mode(
    platform: &str,
    raw_base_url: &str,
    mapped_provider: &str,
    model_id: &str,
    ...
    let mut compat = TjuaeCliCompatOverrides::default();
...
pub(crate) fn resolve_model_compat_overrides(
    ...
    let settings = serde_json::from_str::<HashMap<String, ModelSettings>>(model_settings_json)
        .map_err(|error| AgentError::bad_request(format!("模型“{model_id}”的设置配置无效：{error}")))?;
...
fn resolve_model_protocol(model_id: &str, model_protocols: Option<&str>) -> Result<String, AgentError> {
    ...
    let map = serde_json::from_str::<Map<String, Value>>(protocols_json)
        .map_err(|error| AgentError::bad_request(format!("模型“{model_id}”的协议配置无效：{error}")))?;
...
pub(crate) fn resolve_bedrock_config(json: Option<&str>) -> Option<tjuae_config::config::BedrockConfig> {
    let bc: tjuaeui_api_types::BedrockConfig = serde_json::from_str(json?).ok()?;
    Some(tjuae_config::config::BedrockConfig {
...
fn resolve_mcp_servers(overrides: &TjuaeCliBuildExtra) -> HashMap<String, McpServerConfig> {
...
#[cfg(test)]
#[path = "tjuae_cli_model_settings_test.rs"]
...
mod tests {
    ...
    impl IMcpServerRepository for MockMcpRepo {
        ...
        async fn find_by_id(&self, id: &str) -> Result<Option<McpServerRow>, tjuaeui_db::DbError> {
            Ok(self.rows.iter().find(|row| row.id == id).cloned())
        }

        async fn find_by_name(&self, name: &str) -> Result<Option<McpServerRow>, tjuaeui_db::DbError> {
    ...
    #[tokio::test]
    async fn tjuae_cli_loads_mcp_servers_from_frozen_selection_snapshot() {
    ...
    #[test]
    fn map_tjuae_cli_provider_table_driven_cases() {
        let cases = [
            ProviderMappingCase {
        ...
        for case in cases {
            let result = map_tjuae_cli_provider(case.platform, case.model_id, case.model_protocols);
    ...
    fn openai_protocol_gpt_5_6_family_uses_responses() {
        ...
        for (platform, raw_base_url, model_id) in cases {
            let (base_url, compat) =
                resolve_tjuae_cli_url_and_compat(platform, raw_base_url, "openai", model_id, false);
    ...
    fn responses_selection_does_not_leak_to_other_providers_or_full_urls() {
        ...
        for (platform, provider, model_id, is_full_url) in cases {
            let (_, compat) = resolve_tjuae_cli_url_and_compat(
                platform,
    ...
    fn openai_protocol_gpt_5_6_full_url_is_rewritten_to_responses() {
        for raw_base_url in [
            ...
            let (base_url, compat) =
                resolve_tjuae_cli_url_and_compat("custom", raw_base_url, "openai", "gpt-5.6-sol", true);
    ...
    fn unrelated_full_url_is_not_rewritten_for_gpt_5_6() {
        let (base_url, compat) = resolve_tjuae_cli_url_and_compat(
            "custom",
    ...
    #[test]
    fn resolve_tjuae_cli_url_and_compat_table_driven_cases() {
        let cases = [
            UrlCompatCase {
            ...
            UrlCompatCase {
                name: "official anthropic keeps tjuaecli defaults",
                platform: "anthropic",
    ...
    fn ui_model_platform_base_url_presets_default_to_openai_chat_completions() {
        ...
        for case in cases {
            let provider = map_tjuae_cli_provider("custom", "m", None).expect(case.name);
            assert_eq!(provider, "openai", "{}", case.name);

            let (base_url, compat) = resolve_tjuae_cli_url_and_compat("custom", case.base_url, &provider, "m", false);
            ...
            assert_eq!(
                intended_tjuae_cli_final_url(&provider, case.base_url, compat.api_path.as_deref()),
    ...
    fn ui_model_platform_special_presets_resolve_default_transports() {
        ...
        for case in cases {
            let provider = map_tjuae_cli_provider(case.platform, "m", None).expect(case.name);
            assert_eq!(provider, case.expected_provider, "{}", case.name);

            let (base_url, compat) =
                resolve_tjuae_cli_url_and_compat(case.platform, case.base_url, &provider, "m", false);
            assert_eq!(base_url.as_deref(), case.expected_base_url, "{}", case.name);
            assert_eq!(compat.api_path.as_deref(), case.expected_api_path, "{}", case.name);

            if let (Some(base_url), Some(expected_final_url)) = (base_url.as_deref(), case.expected_final_url) {
                assert_eq!(
                    intended_tjuae_cli_final_url(&provider, base_url, compat.api_path.as_deref()),
                    expected_final_url,
                    "{}",
                    case.name
                );
    ...
    fn intended_tjuae_cli_final_url(provider: &str, base_url: &str, api_path: Option<&str>) -> String {
    ...
    #[test]
    fn resolved_tjuae_cli_final_url_semantics_table_driven_cases() {
        let cases = [
            FinalUrlCase {
        ...
        for case in cases {
            assert_eq!(
                intended_tjuae_cli_final_url(case.provider, case.base_url, case.api_path),
    ...
    fn resolve_mcp_servers_team_takes_priority() {
        let overrides = TjuaeCliBuildExtra {
    ...
    fn resolve_mcp_servers_empty_when_no_config() {
        let overrides = TjuaeCliBuildExtra::default();
        let result = resolve_mcp_servers(&overrides);
        assert!(result.is_empty());
    }
    ...
    fn preset_rules_merged_into_system_prompt_when_no_existing() {
        ...
        let mut overrides: TjuaeCliBuildExtra = serde_json::from_value(json).unwrap();
