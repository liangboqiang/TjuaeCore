...
use super::{resolve_model_compat_overrides, resolve_tjuae_cli_url_and_compat_with_mode};
...
fn explicit_chat_completions_overrides_gpt_5_6_responses_default() {
    let (base_url, compat) = resolve_tjuae_cli_url_and_compat_with_mode(
        "openai",
...
fn explicit_responses_overrides_non_responses_model_default() {
    let (_, compat) = resolve_tjuae_cli_url_and_compat_with_mode(
        "custom",
...
fn explicit_api_mode_rewrites_known_complete_endpoints_in_both_directions() {
    let (chat_url, chat_compat) = resolve_tjuae_cli_url_and_compat_with_mode(
        "custom",
    ...
    let (responses_url, responses_compat) = resolve_tjuae_cli_url_and_compat_with_mode(
        "custom",
