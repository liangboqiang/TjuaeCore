...
use tjuae_agent::bootstrap::AgentBootstrap;
use tjuae_agent::engine::AgentEngine;
use tjuae_agent::output::OutputSink;
use tjuae_agent::session::Session;
use tjuae_config::compat::ProviderCompat;
use tjuae_config::config::{CliArgs, Config, McpServerConfig, ProviderType};
...
use tjuaeui_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, ErrorChain, TimestampMs, now_ms};
...
use super::error::{tjuae_cli_engine_error_to_send_error, tjuae_cli_runtime_error_summary};

fn resolve_tjuaeui_config(cli_args: &CliArgs) -> Result<Config, AgentError> {
    let mut config = Config::resolve(cli_args).map_err(|e| AgentError::internal(format!("解析配置失败：{e}")))?;

    // TjuaeUI owns the embedded runtime policy. Standalone tjuae_cli max-token
...
#[derive(Clone, Debug)]
struct TjuaeCliFinalInputDumpContext {
    dump_dir: PathBuf,
    provider: String,
    model: String,
...
fn build_tjuae_cli_final_input_dump_value(
    conversation_id: &str,
    workspace: &str,
    context: &TjuaeCliFinalInputDumpContext,
    data: &SendMessageData,
) -> Value {
    serde_json::json!({
        "kind": "tjuaecli-final-input",
        "backend": "tjuaecli",
...
pub struct TjuaeCliAgentManager {
...
impl Drop for TjuaeCliAgentManager {
    fn drop(&mut self) {
...
impl TjuaeCliAgentManager {
    pub async fn new(
        ...
        info!(
            ...
            "Resolved image input capability for TjuaeCLI model"
        );
        let final_input_dump = config_extra
            .prompt_dump_dir
            .clone()
            .map(|dump_dir| TjuaeCliFinalInputDumpContext {
                dump_dir,
        ...
        let mut config = resolve_tjuaeui_config(&cli_args)?;
        ...
        if let Some(mode_str) = &config_extra.session_mode {
            ...
            info!(
                ...
                "TjuaeCLI initial session mode applied"
            );
        }
    ...
    fn request_stop(&self, reason: Option<AgentKillReason>, operation: &'static str) -> bool {
        ...
        info!(
            ...
            was_running,
            operation,
            "TjuaeCLI stop signal requested"
        );

        was_running
    }

    fn dump_tjuae_cli_final_input(&self, data: &SendMessageData) {
        let Some(context) = self.final_input_dump.as_ref() else {
            return;
        };

        let value = build_tjuae_cli_final_input_dump_value(
...
impl IAgentTask for TjuaeCliAgentManager {
    ...
    async fn send_message(&self, data: SendMessageData) -> Result<(), AgentSendError> {
        ...
        let result = tokio::select! {
            ...
            _ = self.cancel_notify.notified() => {
                info!(
                    conversation_id = %self.runtime.conversation_id(),
                    "TjuaeCLI engine.run() cancelled by stop signal"
                );
        ...
        let send_result = match result {
            Some(Ok(_)) => {
                info!(
                    conversation_id = %self.runtime.conversation_id(),
                    elapsed_ms,
                    "TjuaeCLI engine.run() completed, emitting Finish"
                );
                self.runtime.emit_finish(None);
                Ok(())
            }
            Some(Err(e)) => {
                let summary = tjuae_cli_runtime_error_summary(&e);
                error!(
                    conversation_id = %self.runtime.conversation_id(),
                    elapsed_ms,
                    error = %ErrorChain(&e),
                    "TjuaeCLI engine.run() failed, emitting Error"
                );
                error!(
                    target: "tjuaeui_feedback_diagnostics",
                    diagnostic_event = "feedback.runtime.tjuae_error",
                    conversation_id = %self.runtime.conversation_id(),
                    msg_id = %data.msg_id,
                    turn_id = data.turn_id.as_deref().unwrap_or("none"),
                    elapsed_ms,
                    ...
                    "feedback.runtime.tjuae_error"
                );
                let send_error = tjuae_cli_engine_error_to_send_error(&e);
    ...
    pub fn kill_and_wait(&self, reason: Option<AgentKillReason>) -> Pin<Box<dyn Future<Output = ()> + Send>> {
        ...
        Box::pin(async move {
                ...
                .is_err()
            {
                warn!(
                    conversation_id,
                    "Timed out waiting for tjuaecli turn to finish after kill"
                );
            }
        })
    }
...
/// TjuaeCli-specific operations reached through `AgentInstance::TjuaeCli(..)`
...
impl TjuaeCliAgentManager {
    ...
    pub async fn set_mode(&self, mode: &str) -> Result<(), AgentError> {
        ...
        info!(
            ...
            from = prev,
            to = mode,
            "TjuaeCLI session mode switched"
        );
        Ok(())
    }
    ...
    pub async fn config_options(&self) -> Result<GetConfigOptionsResponse, AgentError> {
        Ok(GetConfigOptionsResponse {
            config_options: vec![tjuae_cli_mode_config_option(self.approval_manager.current_mode())],
        })
    }
    ...
    pub async fn set_config_option(&self, option_id: &str, value: &str) -> Result<SetConfigOptionResponse, AgentError> {
        ...
        if option_id != TJUAE_CLI_MODE_OPTION_ID {
            return Err(AgentError::bad_request(format!(
                "Config option '{option_id}' is not available"
            )));
        }
        if !is_tjuae_cli_session_mode(value) {
            return Err(AgentError::bad_request(format!(
...
const TJUAE_CLI_MODE_OPTION_ID: &str = "mode";

fn is_tjuae_cli_session_mode(s: &str) -> bool {
    matches!(s, "default" | "auto_edit" | "yolo")
}

fn tjuae_cli_mode_config_option(current_value: String) -> AcpConfigOptionDto {
    AcpConfigOptionDto {
        id: TJUAE_CLI_MODE_OPTION_ID.to_owned(),
        ...
        options: vec![
            tjuae_cli_mode_select_option("default", "Default"),
            tjuae_cli_mode_select_option("auto_edit", "Auto Edit"),
            tjuae_cli_mode_select_option("yolo", "YOLO"),
        ],
    }
}

fn tjuae_cli_mode_select_option(value: &str, name: &str) -> AcpConfigSelectOptionDto {
