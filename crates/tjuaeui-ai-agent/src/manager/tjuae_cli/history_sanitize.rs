//! Sanitize a resumed tjuae_cli session's message history before it is replayed
//! to a provider.
//!
//! Background: when the user clicks "Stop" on a tool-call mid-stream, tjuae_cli
...
//! `TjuaeCliAgentManager`) so it can be unit-tested in isolation and so we do
