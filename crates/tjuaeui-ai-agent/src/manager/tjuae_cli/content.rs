...
fn strip_attachment_metadata<'a>(content: &'a str, files: &[String]) -> &'a str {
    ...
    let Some((user_text, metadata)) = content.rsplit_once(TJUAE_FILES_MARKER) else {
        return content;
    };
