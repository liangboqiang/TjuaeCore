...
use tjuaeui_api_types::SlashCommandItem;
...
impl AcpAgentManager {
    ...
    pub(super) async fn prompt_existing_session(
        ...
        let sid = session_id
            .ok_or_else(|| AgentError::internal("无法发送提示：没有可用的会话 ID"))
    ...
    async fn emit_snapshot_events(&self) {
        use tjuaeui_api_types::{ModelInfoEntry, ModelInfoPayload};
...
mod tests {
    ...
    use tjuaeui_api_types::{AgentErrorCode, SlashCommandCompletionBehavior, SlashCommandItem};
    ...
    #[test]
    fn classify_real_402_empty_turn_tail_preserves_provider_billing_failure() {
