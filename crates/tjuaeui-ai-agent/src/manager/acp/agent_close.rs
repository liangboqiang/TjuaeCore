...
use crate::manager::acp::agent::{exit_status_parts, is_default_internal_bad_gateway, user_facing_message};
...
impl AcpAgentManager {
    ...
    /// `crates/tjuaeui-ai-agent/src/protocol/error.rs` if its fallback wording
    /// changes.
...
mod tests {
    ...
    async fn crash_without_allowlisted_stderr_keeps_exit_code_only() {
        ...
        match reason {
            CloseReason::ProcessExited {
                ...
                .user_facing_message();
                assert!(msg.contains("退出代码 42"), "got {msg}");
            }
            other => panic!("expected ProcessExited, got {other:?}"),
        }
    }
    ...
    fn manual_cancel_close_reason_renders_distinctly_from_agent_crash() {
        ...
        assert!(cancel_msg.contains("取消"));
        assert!(crash_msg.contains("退出代码 1"));
    }
}
