...
///
/// Return the message intended for direct display in the renderer.
pub(super) fn user_facing_message(err: &AgentError) -> String {
    err.public_message()
}

/// Detect the narrow fallback emitted when the ACP SDK has no useful error
/// message. Match the variant payload rather than its localized `Display`.
pub(super) fn is_default_internal_bad_gateway(err: &AgentError) -> bool {
    const SDK_DEFAULT_MESSAGE_PREFIX: &str = "Agent 内部错误（代码 ";
    matches!(
        err,
        AgentError::BadGateway(message)
            if message.starts_with(SDK_DEFAULT_MESSAGE_PREFIX) && message.ends_with('）')
    )
}
...
async fn spawn_and_connect_acp_once(
    ...
    let (stdin, stdout) = process.take_stdio().await.ok_or_else(|| {
        ...
        AcpStartupConnectError::Agent(AgentError::internal("无法接管 CLI 进程的 stdio"))
    })?;
...
impl AcpAgentManager {
    ...
    pub(crate) async fn mode(&self) -> Result<tjuaeui_api_types::AgentModeResponse, AgentError> {
        let desired = self
            .session
            .read()
            .await
    ...
    pub(crate) async fn set_config_option_confirmed(
        ...
        if option_id.is_empty() {
            return Err(AgentError::bad_request("option_id 不能为空"));
        }
        if value.is_empty() {
            return Err(AgentError::bad_request("value 不能为空"));
        }
        ...
        let Some(_guard) = guard_opt else {
            ...
            return Err(AgentError::conflict("ACP 配置更新正在进行"));
        };
    ...
    async fn set_config_option_confirmed_inner(
        ...
        let (session_id, set_path, resolved_value) = {
            ...
            let set_path = resolve_set_path(&snapshot, option_id, &resolved_value).map_err(|err| match err {
                ConfigSetPathError::OptionNotFound => AgentError::bad_request(format!("配置选项“{option_id}”不可用")),
    ...
    async fn wait_for_observed_config_option(
        ...
        loop {
            ...
            if started.elapsed() >= timeout {
                ...
                return Err(AgentError::timeout("ACP 配置选项确认超时"));
            }
...
impl AcpAgentManager {
    ...
    pub fn confirm(
        ...
        let option_id =
            confirm_option_id(&data).ok_or_else(|| AgentError::bad_request("ACP 确认请求需要 option_id 字符串"))?;
...
mod tests {
    use super::{
        build_acp_final_input_dump_value, exit_status_parts, is_default_internal_bad_gateway,
    ...
    fn rate_limited_has_no_colon_returns_full_string() {
        ...
        assert_eq!(user_facing_message(&err), "请求频率受限");
    }
    ...
    async fn augment_via_process(proc: &Arc<CliAgentProcess>, err: &AgentError) -> Option<String> {
        if !is_default_internal_bad_gateway(err) {
            return None;
        }
    ...
    async fn augments_when_codex_usage_limit_in_stderr() {
        ...
        let err = AgentError::bad_gateway("Agent 内部错误（代码 -32603）");
    ...
    fn session_command_loading_preserves_empty_turn_meta() {
        ...
        assert_eq!(
            ...
            Some(tjuaeui_api_types::SlashCommandCompletionBehavior::NeutralTipOnEmpty)
        );
        assert_eq!(
