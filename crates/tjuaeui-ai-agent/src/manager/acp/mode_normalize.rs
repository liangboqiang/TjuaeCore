use tjuaeui_api_types::AgentMetadata;
...
pub(crate) fn normalize_requested_mode(metadata: &AgentMetadata, mode: &str) -> String {
    ...
    // TjuaeUI persists the generic `yolo` mode while
...
fn is_codex_full_access_semantic_value(mode: &str) -> bool {
    matches!(
        mode,
        CODEX_CANONICAL_FULL_ACCESS_MODE | CODEX_LEGACY_FULL_ACCESS_MODE | "yolo"
    )
}
...
mod tests {
    ...
    fn metadata_with_yolo_id(yolo_id: Option<&str>) -> AgentMetadata {
        use tjuaeui_api_types::{AgentSource, AgentSourceInfo, BehaviorPolicy};
        AgentMetadata {
            id: "test".into(),
            icon: None,
            name: "Test".into(),
    ...
    fn normalize_requested_mode_passes_through_when_no_yolo_id() {
        ...
        // No mapping configured — the generic mode flows through unchanged.
    ...
    /// mode requests hit the native mode id for each vendor.
    #[test]
    ...
    fn uses_meta_resume_true_when_policy_flag_set() {
        use tjuaeui_api_types::BehaviorPolicy;
