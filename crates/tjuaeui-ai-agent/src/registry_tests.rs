...
async fn probe_resolved_command_keeps_bridge_but_version_probe_targets_primary_cli() {
    ...
    missing_primary.agent_source_info.binary_name = Some("tjuaeui-definitely-missing-product-cli".into());
    let reason = probe_resolved_command(&missing_primary).expect_err("npx must not replace the product CLI check");
    assert!(matches!(
        reason,
        UnavailableReason::PrimaryMissing { binary } if binary == "tjuaeui-definitely-missing-product-cli"
    ));
}
...
#[tokio::test]
async fn management_rows_include_tjuae_cli_builtin_mode_catalog() {
    let db = init_database_memory().await.unwrap();
    let repo: Arc<dyn IAgentMetadataRepository> = Arc::new(SqliteAgentMetadataRepository::new(db.pool().clone()));
    let registry = AgentRegistry::new(repo);
    registry.hydrate().await.unwrap();
    ...
    let row = registry
        ...
        .await
        .into_iter()
        .find(|item| item.agent_type == AgentType::TjuaeCli)
        .unwrap();
    let row_json = serde_json::to_value(&row).unwrap();
