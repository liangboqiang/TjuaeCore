...
//! `Vec<agent_client_protocol::McpServer>`, `factory::tjuae_cli::load_user_mcp_servers`
//! → `HashMap<String, McpServerConfig>`) emit SDK/engine-specific types. This
//! module emits the NEUTRAL `tjuaeui_api_types::SessionMcpServer` so the app
//! boundary (`tjuaeui-app`) can convert it once into the crate-local
//! `tjuaeui_session::McpServerSpec`, and each backend serializes that into its own
