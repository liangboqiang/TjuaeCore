...
use tjuaeui_api_types::{BedrockConfig, FetchModelsAnonymousRequest, FetchModelsRequest, FetchModelsResponse};
use tjuaeui_common::decrypt_string;
...
impl ModelFetchService {
    ...
    async fn load_provider_config(&self, provider_id: &str) -> Result<FetchConfig, SystemError> {
        let row = self
            ...
            .await?
            .ok_or_else(|| SystemError::NotFound(format!("找不到模型提供商：{provider_id}")))?;

        let api_key = decrypt_string(&row.api_key_encrypted, &self.encryption_key)?;
        if api_key.trim().is_empty() {
            return Err(SystemError::BadRequest("API 密钥不能为空".into()));
        }
...
fn validate_anonymous_request(req: &FetchModelsAnonymousRequest) -> Result<(), SystemError> {
    if req.platform.trim().is_empty() {
        return Err(SystemError::BadRequest("platform 为必填项".into()));
    }
    if req.base_url.trim().is_empty() {
        return Err(SystemError::BadRequest("baseUrl 为必填项".into()));
    }
    ...
    if req.platform != "bedrock" && req.api_key.trim().is_empty() {
        return Err(SystemError::BadRequest("apiKey 为必填项".into()));
    }
    Ok(())
}
...
mod tests {
    ...
    use tjuaeui_db::{CreateProviderParams, SqliteProviderRepository, init_database_memory};

    const TEST_KEY: [u8; 32] = [0x42; 32];

    async fn setup() -> (ModelFetchService, tjuaeui_db::Database) {
    ...
    async fn create_provider(db: &tjuaeui_db::Database, platform: &str, base_url: &str, api_key: &str) -> String {
