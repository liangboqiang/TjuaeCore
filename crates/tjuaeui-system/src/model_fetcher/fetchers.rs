...
use tjuaeui_api_types::ModelInfo;
use tracing::warn;
...
pub(super) async fn fetch_openai_compatible(
    ...
    let body: OpenAiModelsResponse = resp
        .json()
        .await
        .map_err(|e| SystemError::BadGateway(format!("解析模型响应失败：{e}")))?;
    ...
    match result {
        Ok(resp) if resp.status().is_success() => {
            let body: AnthropicModelsResponse = resp
                .json()
                .await
                .map_err(|e| SystemError::BadGateway(format!("解析 Anthropic 响应失败：{e}")))?;
            Ok(body.data.into_iter().map(|m| ModelInfo::Id(m.id)).collect())
        }
        Ok(resp) => {
            warn!(
    ...
    match result {
        Ok(resp) if resp.status().is_success() => {
            let body: GeminiModelsResponse = resp
                .json()
                .await
                .map_err(|e| SystemError::BadGateway(format!("解析 Gemini 响应失败：{e}")))?;
            let models = body
                .models
                .into_iter()
                .map(|m| {
...
async fn fetch_bedrock(config: &FetchConfig) -> Result<Vec<ModelInfo>, SystemError> {
    let bedrock_cfg = config
        .bedrock_config
        .as_ref()
        .ok_or_else(|| SystemError::BadRequest("Bedrock 必须提供 bedrockConfig".into()))?;

    let region = aws_sdk_bedrock::config::Region::new(bedrock_cfg.region.clone());

    let sdk_config = match bedrock_cfg.auth_method {
        tjuaeui_api_types::BedrockAuthMethod::AccessKey => {
            let key_id = bedrock_cfg
                .access_key_id
                .as_deref()
                .ok_or_else(|| SystemError::BadRequest("accessKeyId 为必填项".into()))?;
            let secret = bedrock_cfg
                .secret_access_key
                .as_deref()
                .ok_or_else(|| SystemError::BadRequest("secretAccessKey 为必填项".into()))?;
        ...
        tjuaeui_api_types::BedrockAuthMethod::Profile => {
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| SystemError::BadGateway(format!("Bedrock API 错误：{e}")))?;
...
fn remote_error(e: &reqwest::Error) -> SystemError {
    if e.is_timeout() {
        SystemError::Timeout("远程 API 请求超时".into())
    } else {
        SystemError::BadGateway(format!("远程 API 请求失败：{e}"))
    }
}
