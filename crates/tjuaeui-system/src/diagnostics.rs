    ...
    pub async fn collect(
        ...
        Ok(FeedbackDiagnosticsResponse {
            ...
            privacy: FeedbackDiagnosticsPrivacyResponse {
                redaction: "tjuaeCore returns selected metadata, ids, titles, status fields, counts, endpoint hosts, selected configuration values, and MCP original_json keeps connection structure while credential values are redacted; raw error and tool-call diagnostic content may be included; non-error message content and prompts are summarized or redacted."
                    .to_owned(),
