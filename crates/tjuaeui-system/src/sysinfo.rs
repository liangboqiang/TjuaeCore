use tjuaeui_api_types::SystemInfoResponse;
...
/// Resolve the cache directory for TjuaeUI.
///
/// Priority: `TJUAE_CACHE_DIR` env → `dirs::cache_dir()/tjuaeui`.
fn resolve_cache_dir() -> String {
    if let Ok(v) = std::env::var("TJUAE_CACHE_DIR")
        && !v.is_empty()
    {
        return v;
    }
    dirs::cache_dir()
        .map(|p| p.join("tjuaeui").to_string_lossy().into_owned())
        .unwrap_or_default()
}

/// Resolve the work (data) directory for TjuaeUI.
///
/// Priority: `TJUAE_WORK_DIR` env → `dirs::data_dir()/tjuaeui`.
fn resolve_work_dir() -> String {
    if let Ok(v) = std::env::var("TJUAE_WORK_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
/// Resolve the log directory for TjuaeUI.
///
/// Priority: `TJUAE_LOG_DIR` env →
///   macOS: `~/Library/Logs/tjuaeui`
///   Linux: `dirs::state_dir()/tjuaeui/logs` (XDG_STATE_HOME)
///   Windows: `dirs::data_dir()/tjuaeui/logs`
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("TJUAE_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
    ...
    {
        return home.join("Library/Logs/tjuaeui").to_string_lossy().into_owned();
    }
    // Linux: XDG state dir
    if let Some(state) = dirs::state_dir() {
        return state.join("tjuaeui/logs").to_string_lossy().into_owned();
    }
    // Fallback: data_dir/tjuaeui/logs
    dirs::data_dir()
        .map(|p| p.join("tjuaeui/logs").to_string_lossy().into_owned())
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn test_env_override_cache_dir() {
        ...
        // the default path contains "tjuaeui".
        let dir = resolve_cache_dir();
        assert!(dir.contains("tjuaeui"), "cache_dir should contain 'tjuaeui': {dir}");
    }

    #[test]
    fn test_env_override_work_dir() {
        let dir = resolve_work_dir();
        assert!(dir.contains("tjuaeui"), "work_dir should contain 'tjuaeui': {dir}");
    }

    #[test]
    fn test_env_override_log_dir() {
        let dir = resolve_log_dir();
        assert!(dir.contains("tjuaeui"), "log_dir should contain 'tjuaeui': {dir}");
    }
}
