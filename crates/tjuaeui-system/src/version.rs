...
use tjuaeui_api_types::{GitHubReleaseAsset, UpdateCheckRequest, UpdateCheckResult, UpdateReleaseInfo};

use crate::error::SystemError;

const DEFAULT_REPO: &str = "liangboqiang/TjuaeUI";
    ...
    async fn fetch_releases(&self, repo: &str) -> Result<Vec<GitHubRelease>, SystemError> {
        ...
        loop {
            ...
            let resp = self
                ...
                .header("User-Agent", "tjuaecore")
                .send()
                .await
                .map_err(|e| SystemError::BadGateway(format!("GitHub API 请求失败：{e}")))?;

            if !resp.status().is_success() {
                let status = resp.status();
                let body = resp.text().await.unwrap_or_default();
                return Err(SystemError::BadGateway(format!("GitHub API 返回 {status}：{body}")));
            }
            ...
            let batch: Vec<GitHubRelease> = resp
                .json()
                .await
                .map_err(|e| SystemError::BadGateway(format!("解析 GitHub 发布信息失败：{e}")))?;
...
fn resolve_repo(from_request: Option<&str>) -> String {
    ...
    if let Ok(v) = std::env::var("TJUAE_GITHUB_REPO")
        && !v.is_empty()
    {
        return v;
    }
