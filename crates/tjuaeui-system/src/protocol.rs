...
use tjuaeui_common::ProtocolType;
...
impl ProtocolDetectionService {
    ...
    async fn probe_openai(&self, base: &str, api_key: &str, timeout: Duration) -> Result<ProbeOutcome, SystemError> {
            ...
            match resp {
                Ok(r) if r.status().is_success() => {
                    let body: DataResponse = r
                        .json()
                        .await
                        .map_err(|e| SystemError::BadGateway(format!("解析失败：{e}")))?;
        ...
        Err(SystemError::BadGateway("OpenAI 探测失败".into()))
    }
    ...
    async fn probe_anthropic(&self, base: &str, api_key: &str, timeout: Duration) -> Result<ProbeOutcome, SystemError> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| SystemError::BadGateway(format!("Anthropic 探测失败：{e}")))?;
        ...
        Err(SystemError::BadGateway(format!("Anthropic 返回状态 {}", resp.status())))
    }
    ...
    async fn probe_gemini(&self, base: &str, api_key: &str, timeout: Duration) -> Result<ProbeOutcome, SystemError> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| SystemError::BadGateway(format!("Gemini 探测失败：{e}")))?;
        ...
        Err(SystemError::BadGateway(format!("Gemini 返回状态 {}", resp.status())))
    }
...
fn success_suggestion(detected: ProtocolType, preferred: Option<ProtocolType>) -> DetectionSuggestion {
    ...
    if should_switch {
        ...
        DetectionSuggestion {
            ...
            message: format!("检测到 {} 协议", protocol_display_name(detected)),
...
fn check_key_suggestion() -> DetectionSuggestion {
    DetectionSuggestion {
        ...
        message: "无法检测协议，请检查 API 密钥和 URL。".into(),
...
async fn test_single_key(
    ...
    let resp = req
        .send()
        .await
        .map_err(|e| SystemError::BadGateway(format!("请求失败：{e}")))?;

    if resp.status().is_success() {
        Ok(())
    } else {
        Err(SystemError::BadGateway(format!("状态：{}", resp.status())))
    }
}
