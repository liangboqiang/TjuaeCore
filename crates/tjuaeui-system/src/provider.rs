...
use tjuaeui_api_types::{CreateProviderRequest, ProviderResponse, UpdateProviderRequest};
use tjuaeui_common::{decrypt_string, encrypt_string};
use tjuaeui_db::{CreateProviderParams, IProviderRepository, UpdateProviderParams, models::Provider};
...
fn validate_create_request(req: &CreateProviderRequest) -> Result<(), SystemError> {
    ...
    if req.name.trim().is_empty() {
        return Err(SystemError::BadRequest("name 为必填项".into()));
    }
...
fn validate_id(id: &str) -> Result<(), SystemError> {
    let trimmed = id.trim();
    if trimmed.is_empty() {
        return Err(SystemError::BadRequest("id 不能为空".into()));
    }
    if trimmed.len() > 128 {
        return Err(SystemError::BadRequest("id 最多包含 128 个字符".into()));
    }
    if !trimmed
        .chars()
...
fn validate_update_request(req: &UpdateProviderRequest) -> Result<(), SystemError> {
    ...
    {
        return Err(SystemError::BadRequest("platform 不能为空".into()));
    }
    if let Some(ref name) = req.name
        && name.trim().is_empty()
    {
        return Err(SystemError::BadRequest("name 不能为空".into()));
    }
...
mod tests {
    use super::*;
    use tjuaeui_db::{SqliteProviderRepository, init_database_memory};
    ...
    fn validate_create_bedrock_allows_empty_base_url_and_api_key() {
        let req = CreateProviderRequest {
            ...
            base_url: "".into(),
            api_key: "".into(),
            bedrock_config: Some(tjuaeui_api_types::BedrockConfig {
                auth_method: tjuaeui_api_types::BedrockAuthMethod::Profile,
