...
fn spawn_keep_awake_process() -> Result<tokio::process::Child, SystemError> {
    let mut command = tjuaeui_runtime::Builder::clean_cli("caffeinate");
...
#[cfg(target_os = "linux")]
fn spawn_keep_awake_process() -> Result<tokio::process::Child, SystemError> {
    let mut command = tjuaeui_runtime::Builder::clean_cli("systemd-inhibit");
    command
        .args([
            "--what=sleep",
            "--why=TjuaeUI scheduled tasks keep-awake is enabled",
            "--mode=block",
            "sleep",
            "infinity",
        ])
