...
use tjuaeui_api_types::{BedrockAuthMethod, BedrockConfig};
    ...
    pub async fn test_bedrock_connection(&self, config: BedrockConfig) -> Result<(), SystemError> {
        ...
        client
            ...
            .map_err(|e| {
                ...
                SystemError::UnprocessableEntity(format!("Bedrock 凭据无效：{e}"))
            })?;
...
fn validate_bedrock_config(config: &BedrockConfig) -> Result<(), SystemError> {
    if config.region.is_empty() {
        return Err(SystemError::BadRequest("region 为必填项".into()));
    }
...
mod tests {
    use super::*;
    use tjuaeui_api_types::BedrockConfig;
