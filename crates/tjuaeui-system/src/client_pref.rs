...
use tjuaeui_api_types::{ClientPreferencesResponse, UpdateClientPreferencesRequest};
...
fn validate_key(key: &str) -> Result<(), SystemError> {
    if key.is_empty() {
        return Err(SystemError::BadRequest("偏好设置键不能为空".into()));
    }
...
mod tests {
    ...
    use tjuaeui_db::{SqliteClientPreferenceRepository, init_database_memory};
    use tracing::Level;
