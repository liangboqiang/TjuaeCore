...
fn build_state(db: &tjuaeui_db::Database, version_check_service: VersionCheckService) -> SystemRouterState {
...
fn make_github_release(tag: &str, draft: bool, prerelease: bool, assets: Vec<serde_json::Value>) -> serde_json::Value {
    json!({
        ...
        "body": "Release notes",
        "html_url": format!("https://github.com/liangboqiang/TjuaeUI/releases/tag/{tag}"),
...
async fn test_check_update_has_new_version() {
    ...
    Mock::given(method("GET"))
        .and(path("/repos/liangboqiang/TjuaeUI/releases"))
