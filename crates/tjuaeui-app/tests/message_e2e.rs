...
async fn update_conversation_workspace(services: &tjuaeui_app::AppServices, conv_id: &str, workspace: &str) {
    let repo = tjuaeui_db::SqliteConversationRepository::new(services.database.pool().clone());
...
async fn upsert_artifact(services: &tjuaeui_app::AppServices, artifact: tjuaeui_db::ConversationArtifactRow) {
    let repo = tjuaeui_db::SqliteConversationRepository::new(services.database.pool().clone());
    tjuaeui_db::IConversationRepository::upsert_artifact(&repo, &artifact)
        .await
        .unwrap();
}
...
async fn t8_2e_get_message_not_found_returns_specific_error() {
    ...
    assert!(json["error"].as_str().unwrap().contains("找不到消息：missing-message"));
}
...
async fn t8_2f_get_message_does_not_leak_cross_user_conversation() {
    ...
    assert!(
        json["error"]
            .as_str()
            .unwrap()
            .contains(&format!("找不到对话：{owner_conv_id}"))
    );
