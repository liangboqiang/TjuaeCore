...
use tjuaeui_db::models::{AssistantSessionRow, AssistantUserRow};
...
async fn enable_plugin_invalid_type() {
    ...
    assert!(data["error"].as_str().unwrap().contains("插件类型无效"));
}
...
#[tokio::test]
async fn get_channel_settings_has_no_implicit_assistant_without_an_active_asset_binding() {
    let (mut app, services) = build_app().await;
    let (token, _csrf) = setup_and_login(&mut app, &services, "admin", "StrongP@ss1").await;
    ...
    // A channel may only select an assistant that has an active AssetCatalog
    // binding for the current user. Do not silently fall back to a generated
    // global assistant when the local library has no active assistant.
...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn tjuaeui_db::IChannelRepository> =
        std::sync::Arc::new(tjuaeui_db::SqliteChannelRepository::new(pool));
    let pairing_svc = tjuaeui_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
