//! E2E coverage for the agent-facing `tjuaecore diagnose` CLI.
...
async fn diagnose_health_reads_backend_health_from_runtime_base_url() {
    ...
    let output = diagnose_command()
        ...
        .env("TJUAE_CONVERSATION_ID", "conv-health")
        .env("TJUAE_USER_ID", "user-health")
        .output()
        .await
        .unwrap();
...
async fn diagnose_http_get_is_get_only_redacted_escape_hatch_for_api_paths() {
    ...
    let mut child = diagnose_command()
        .args(["http", "get"])
        .env("TJUAE_BASE_URL", &base_url)
        .env("TJUAE_CONVERSATION_ID", "conv-http")
        .env("TJUAE_USER_ID", "user-http")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
async fn diagnose_overview_aggregates_common_failure_signals() {
    ...
    let output = diagnose_command()
        .arg("overview")
        .env("TJUAE_BASE_URL", &base_url)
        .env("TJUAE_CONVERSATION_ID", "conv-overview")
        .env("TJUAE_USER_ID", "user-overview")
        .output()
        .await
        .unwrap();
...
#[tokio::test]
async fn diagnose_logs_tail_reads_latest_tjuaecore_log_and_filters_errors() {
    let temp = tempfile::TempDir::new().unwrap();
    let log_dir = temp.path().join("2026").join("07").join("08");
    std::fs::create_dir_all(&log_dir).unwrap();
    std::fs::write(
        log_dir.join("2026-07-08.tjuaecore.log"),
