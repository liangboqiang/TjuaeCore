...
async fn build_app() -> axum::Router {
    ...
    tjuaeui_app::create_router(&services).await.expect("build router")
}
...
async fn health_check_returns_ok_when_agent_metadata_cache_field_has_invalid_utf8() {
    ...
    let app = tjuaeui_app::create_router(&services).await.expect("build router");
...
async fn office_proxy_routes_allow_same_origin_framing() {
    // Regression for liangboqiang/TjuaeUI#3177: the global security headers
