...
//! `tjuaeui_app::create_router_with_states` against an in-memory DB.
...
use tjuaeui_app::{ModuleStates, build_module_states, create_router_with_states};
...
async fn fixture_embedded() -> Fixture {
    ...
    unsafe {
        std::env::remove_var("TJUAE_BUILTIN_SKILLS_PATH");
    }
    ...
    tjuaeui_extension::materialize_if_needed(&data_dir, tjuaeui_extension::builtin_skills_corpus(), "test-fixture")
        .await
    ...
    // `build_module_states` builds a state pointing at `~/.tjuaeui/`,
    ...
    let skill_repo = Arc::new(tjuaeui_db::SqliteSkillRepository::new(services.database.pool().clone()));
    ...
    tjuaeui_extension::sync_skill_catalog_into_repo(&states.skill.skill_paths, skill_repo.as_ref())
        .await
...
async fn unified_skill_list_includes_auto_inject_entries_from_embedded_corpus() {
    ...
    assert!(
        names.contains(&"tjuaeui-config"),
        "tjuaeui-config should be shipped as an auto-inject builtin skill: {names:?}",
    );
    assert!(
        !names.contains(&"tjuaeui-skills"),
        "tjuaeui-skills should not be shipped as an auto-inject builtin skill: {names:?}",
    );
