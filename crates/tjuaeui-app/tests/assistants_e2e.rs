...
//! against a real `tjuaeui_app::create_router_with_states` instance backed by
...
use tjuaeui_app::{AppConfig, AppServices, ModuleStates, build_module_states, create_router_with_states};
use tjuaeui_assistant::{AssistantAgentCatalogPort, AssistantRouterState, AssistantService, BuiltinAssistantRegistry};
...
impl AssistantAgentCatalogPort for TestAgentCatalog {
    async fn list_management_agents(&self) -> Result<Vec<AgentManagementRow>, tjuaeui_assistant::AssistantError> {
        Ok(self.rows.clone())
    }
}
...
fn test_agent_row(id: &str, backend: Option<&str>, agent_type: AgentType, name: &str) -> AgentManagementRow {
    AgentManagementRow {
        ...
        agent_source: match agent_type {
            AgentType::TjuaeCli => AgentSource::Internal,
...
async fn fixture() -> Fixture {
    ...
    // `build_module_states` point at `~/.tjuaeui/` for the state store and
    ...
    // `~/.tjuaeui/` for user data — neither is appropriate for tests.
    ...
    // an explicit `agent_id` resolve to `"tjuaecli"` instead of
    // erroring out — mirroring a configured production setup.
    provider_repo
        .create(tjuaeui_db::CreateProviderParams {
            id: None,
            platform: "openai",
            name: "Test OpenAI",
        ...
        tjuaeui_assistant::service::AssistantServiceDeps {
            ...
            agent_catalog: Some(Arc::new(TestAgentCatalog {
                rows: vec![
                    ...
                    test_agent_row("632f31d2", None, AgentType::TjuaeCli, "TjuaeCLI"),
                ],
            })),
        },
        user_data_dir.clone(),
