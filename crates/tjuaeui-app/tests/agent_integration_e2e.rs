...
use tjuaeui_ai_agent::protocol::events::TextEventData;
use tjuaeui_ai_agent::types::{BuildTaskOptions, SendMessageData};
use tjuaeui_ai_agent::{AgentError, AgentStreamEvent, IWorkerTaskManager};
use tjuaeui_api_types::AgentSource;
...
async fn build_app_with_mock_tasks() -> (axum::Router, tjuaeui_app::AppServices, Arc<MockTaskManager>) {
    let db = tjuaeui_db::init_database_memory().await.unwrap();
    let services = tjuaeui_app::AppServices::from_config(db, &tjuaeui_app::AppConfig::default())
        .await
        .unwrap();
    ...
    let router = tjuaeui_app::create_router(&services).await.expect("build router");
...
async fn upsert_visible_agent_metadata(services: &tjuaeui_app::AppServices, id: &str, agent_type: &str) {
    services
        .agent_registry
        .repo_handle()
...
async fn management_endpoint_keeps_deprecated_runtime_rows_for_diagnostics() {
    ...
    for (id, agent_type) in [
        ...
        ("test-visible-tjuaecli", "tjuaecli"),
    ...
    assert!(types.contains(&"tjuaecli"));
...
async fn agent_logos_endpoint_returns_backend_to_logo_catalog() {
    ...
    // TjuaeCLI has no vendor `backend` (NULL); it must still be keyed by its
