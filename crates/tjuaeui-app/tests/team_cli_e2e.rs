//! E2E coverage for the agent-facing `tjuaecore team` CLI fallback.
...
async fn team_capabilities_prints_contract_without_runtime_env() {
    let output = team_command()
        ...
        .env_remove("TJUAE_RUNTIME_TOKEN")
        .output()
        .await
        .unwrap();
...
async fn tool_command_rejects_forged_identity_fields_before_http_call() {
    let mut child = team_command()
        .args(["send-message"])
        .env("TJUAE_BASE_URL", "http://127.0.0.1:9")
        .env("TJUAE_CONVERSATION_ID", "conv-1")
        .env("TJUAE_USER_ID", "user-1")
        .env("TJUAE_RUNTIME_TOKEN", "token-1")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
