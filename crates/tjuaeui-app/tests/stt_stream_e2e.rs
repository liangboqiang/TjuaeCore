...
//! mirroring `tjuaeui-shell/tests/stt_stream_deepgram_integration.rs`).
//!
...
use tjuaeui_app::{AppConfig, AppServices, create_router};
...
async fn seed_stt_prefs(app: &TestApp, config: Value) {
    let repo = Arc::new(tjuaeui_db::SqliteClientPreferenceRepository::new(
        app.services.database.pool().clone(),
    ));
    let service = tjuaeui_system::ClientPrefService::new(repo);
    let mut req = tjuaeui_api_types::UpdateClientPreferencesRequest::new();
...
// ---------------------------------------------------------------------------
// Mock Deepgram live server (minimal copy of the tjuaeui-shell integration
