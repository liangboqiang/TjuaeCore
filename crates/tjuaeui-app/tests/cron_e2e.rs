...
//! at the service integration level in `tjuaeui-cron/tests/service_integration.rs`.
...
async fn cj3b_create_accepts_workspace_with_whitespace_segment() {
    ...
    let (token, csrf) = setup_and_login(&mut app, &services, "admin", "StrongP@ss1").await;
    ensure_default_assistant(&mut app, &token, &csrf).await;
    let dir = std::env::temp_dir().join(format!("tjuaeui-cron-test-{}", tjuaeui_common::generate_short_id()));
...
async fn cj5b_run_now_legacy_workspace_with_whitespace_succeeds() {
    ...
    cron_repo
        .insert(&tjuaeui_db::models::CronJobRow {
