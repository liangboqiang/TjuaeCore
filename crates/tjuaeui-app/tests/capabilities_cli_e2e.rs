//! E2E coverage for the top-level agent-facing `tjuaecore capabilities` index.

use tokio::process::Command;

fn tjuaecore_command() -> Command {
    Command::new(env!("CARGO_BIN_EXE_tjuaecore"))
}

#[tokio::test]
async fn top_level_capabilities_prints_domain_index_without_runtime_env() {
    let output = tjuaecore_command()
        .arg("capabilities")
        .env_remove("TJUAE_BASE_URL")
        .env_remove("TJUAE_CONVERSATION_ID")
        .env_remove("TJUAE_USER_ID")
        .env_remove("TJUAE_HELPER_BIN")
        .output()
        .await
        .unwrap();
    ...
    assert_eq!(stdout["data"]["contract"], "agent-facing-tjuaecore-cli");
    assert_eq!(stdout["data"]["entrypoint"], "tjuaecore capabilities");
    assert_eq!(stdout["data"]["runtime_context"]["primary"], "TJUAE_CONVERSATION_ID");
    ...
    assert_eq!(config["invocation"], "tjuaecore config capabilities");
    ...
    assert_eq!(diagnose["invocation"], "tjuaecore diagnose capabilities");
}
