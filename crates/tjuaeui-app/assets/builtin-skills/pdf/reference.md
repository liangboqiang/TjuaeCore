...
# 解密。
qpdf --password=USER_PASSWORD --decrypt encrypted.pdf decrypted.pdf
```
...
- `pypdfium2`：Apache-2.0 / BSD-3-Clause，具体以安装版本为准。
- `pdf-lib`：MIT。
- `pdfjs-dist`：Apache-2.0。
- `pypdf`：BSD-3-Clause。
- `pdfplumber`：MIT。
- `reportlab`：BSD。
- Poppler、qpdf 和 OCR 工具的许可证以各自发行包为准。

分发应用或内置二进制文件前，应核对实际版本及其许可证义务。
