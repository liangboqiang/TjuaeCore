...
fn run_main() -> Result<ExitCode, MainError> {
    ...
    // parse a uniform CLI, but bypass `tjuaeui_runtime::init` (which would
    ...
    // detection path exactly. It must hit the same `tjuaeui_runtime::init`
    // before performing managed-runtime and PATH probing.
    let needs_runtime = cli.command.as_ref().is_none_or(Command::need_runtime);
    if needs_runtime {
        tjuaeui_runtime::set_managed_resources_mode(cli.managed_resources_mode.into());
        tjuaeui_runtime::init(&cli.data_dir);
    }
    ...
    let merged_path = unsafe { tjuaeui_runtime::enhance_process_path() };
...
async fn async_main(merged_path: String, cli: Cli) -> Result<ExitCode, MainError> {
    ...
    match cli.command {
        ...
        None => {
            ...
            // touching the DB, so a second tjuaecore yields structurally rather
            // than racing the assistant bootstrap over the same data directory
            // (concurrent-startup regression). Held (via `_instance_guard`, not a bare `_`,
            ...
            let _instance_guard = match tjuaeui_db::DataDirInstanceGuard::try_acquire(&db_path) {
                Ok(Some(guard)) => {
                    ...
                    match bootstrap::wait_for_instance_guard(&db_path) {
                        ...
                        Ok(None) => {
                            tracing::info!(
                                ...
                                "another tjuaecore owns the data directory; yielding"
                            );
                            return Err(MainError::Bootstrap(bootstrap::BootstrapError::new(
                                ...
                                "another tjuaecore already owns this data directory",
                            )));
                        }
                        Err(error) => {
                            tracing::warn!(
