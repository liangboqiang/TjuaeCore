//! `tjuaecore diagnose` subcommand: agent-facing read-only troubleshooting CLI.
...
const ENV_LOG_DIR: &str = "TJUAE_LOG_DIR";
    ...
    let log_dir = optional_string_field(&payload, "log_dir")
        ...
        .ok_or_else(|| {
            DiagnoseError::new(
                ...
                command,
                "missing log directory; set TJUAE_LOG_DIR or pass log_dir in stdin JSON",
            )
...
async fn request_json(
    ...
    if !status.is_success() {
        return Err(
            DiagnoseError::new(DiagnoseErrorCode::HttpStatusError, command, "TjuaeUI 后端返回错误状态")
                .field("path", path)
...
fn read_optional_stdin_payload(command: &str) -> Result<Value, DiagnoseError> {
    ...
    if !value.is_object() {
        return Err(
            DiagnoseError::new(DiagnoseErrorCode::PayloadInvalid, command, "JSON 载荷必须是对象")
                .field("field", "stdin"),
        );
    }
    Ok(value)
...
fn resolve_conversation_selector(
    ...
    let object = payload.as_object_mut().ok_or_else(|| {
        DiagnoseError::new(DiagnoseErrorCode::PayloadInvalid, command, "JSON 载荷必须是对象").field("field", "stdin")
    })?;
    if object
        .get("conversation_id")
...
fn tail_latest_log(
    ...
    collect_tjuaecore_logs(log_dir, 0, &mut files).map_err(|_| {
        DiagnoseError::new(
...
fn collect_tjuaecore_logs(dir: &Path, depth: usize, out: &mut Vec<PathBuf>) -> io::Result<()> {
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        let path = entry.path();
        if path.is_dir() {
            collect_tjuaecore_logs(&path, depth + 1, out)?;
        } else if path
            .file_name()
            .and_then(|name| name.to_str())
            .is_some_and(|name| name.ends_with(".tjuaecore.log"))
        {
            out.push(path);
        }
    }
...
mod tests {
    ...
    fn diagnose_error_renders_quoted_stable_contract() {
        ...
        assert_eq!(
            error.stderr_line(),
            "DIAGNOSE_ENV_MISSING command=\"diagnose context\" field=\"TJUAE_CONVERSATION_ID\": missing required environment variable"
        );
    }
