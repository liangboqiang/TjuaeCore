    ...
    json!({
        ...
        "domains": [
            ...
            domain("assistants", &[
                ...
                stdin_redacted(&["assistants", "file", "read"], "读取目录中声明的文本文件。", &["source", "namespace", "slug", "version", "path"], &[], false, false, &[]),
                stdin_redacted(&["assistants", "file", "write"], "写入当前可编辑目录文件并使旧激活结果失效。", &["source", "namespace", "slug", "path", "content"], &[], true, false, &["content"]),
                stdin(&["assistants", "prepare"], "只检查技能、MCP、模型和智能体依赖，返回分组且逐项的激活计划，不修改任何资源。", &["source", "namespace", "slug", "version"], &[], false, false),
                stdin(&["assistants", "activate"], "按 prepare 返回的指纹、已确认资源组及逐项选择原子激活助手。", &["source", "namespace", "slug", "plan_id", "fingerprint", "confirmed_groups", "choices"], &[], true, false),
                stdin(&["assistants", "export"], "导出不含用户偏好的纯助手目录包。", &["source", "namespace", "slug", "version", "output_path"], &[], true, false),
                stdin(&["assistants", "publish"], "由开发者提交并发布 TjuaeHub 助手工作区。", &["source", "namespace", "slug", "message"], &[], true, false),
            ]),
            domain("skills", &[
