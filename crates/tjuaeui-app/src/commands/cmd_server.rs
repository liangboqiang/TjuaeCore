//! `tjuaecore` (no subcommand): the main HTTP server.
...
use tjuaeui_api_types::{RuntimeStatusScope, RuntimeStatusScopeKind};
use tjuaeui_app::{AppConfig, AppServices, RouterBuildError, create_router_with_runtime};
use tjuaeui_system::RuntimePrepareService;
use tjuaeui_team::TeamIdleCleanupCoordinator;

use crate::bootstrap::{BootstrapError, BootstrapErrorCode, ParentExitSignal, ServerEnvironment};

const LISTENING_EVENT_PREFIX: &str = "TJUAECORE_LISTENING";
...
pub(crate) async fn run_server(
    ...
    // 10 min / 30 min / 60 s and are overridable via TJUAE_IDLE_TIMEOUT_SECS,
    // TJUAE_TEAM_IDLE_TIMEOUT_SECS, and TJUAE_IDLE_SCAN_INTERVAL_SECS. The
    ...
    let idle_cleanup_coordinator: Arc<dyn tjuaeui_ai_agent::IdleCleanupCoordinator> =
        Arc::new(TeamIdleCleanupCoordinator::new(
            router_runtime.team_service.clone(),
            services.active_lease_registry.clone(),
        ));
    let (solo_timeout_secs, team_timeout_secs, scan_interval_secs) = tjuaeui_ai_agent::resolve_idle_config_from_env();
    let idle_scanner_handle = tjuaeui_ai_agent::start_idle_scanner_with_coordinator(
...
mod tests {
    ...
    fn listening_event_line_is_machine_readable() {
        ...
        let payload = line
            .strip_prefix("TJUAECORE_LISTENING ")
