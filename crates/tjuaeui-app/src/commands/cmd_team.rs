...
const ENV_RUNTIME_TOKEN: &str = "TJUAE_RUNTIME_TOKEN";
...
impl RuntimeEnv {
    fn headers(&self) -> reqwest::header::HeaderMap {
        ...
        headers.insert("x-tjuaeui-user-id", self.user_id.parse().unwrap());
        headers.insert("x-tjuaeui-conversation-id", self.conversation_id.parse().unwrap());
        headers.insert("x-tjuaeui-runtime-token", self.runtime_token.parse().unwrap());
        headers
    }
}
...
fn validate_against_descriptor(command: &str, tool: TeamToolName, value: Value) -> Result<Value, ExitCode> {
    ...
    let descriptor = tjuaeui_api_types::team_tool_descriptor(tool.as_str()).expect("descriptor for canonical tool");
