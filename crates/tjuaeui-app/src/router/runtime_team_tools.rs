...
use tjuaeui_ai_agent::{RuntimeTokenScope, RuntimeTokenService, TEAM_RUNTIME_TOKEN_SESSION_GENERATION};
...
use tjuaeui_team::TeamSessionService;

const HEADER_USER_ID: &str = "x-tjuaeui-user-id";
const HEADER_CONVERSATION_ID: &str = "x-tjuaeui-conversation-id";
const HEADER_RUNTIME_TOKEN: &str = "x-tjuaeui-runtime-token";
...
async fn call(
    ...
    match state
        ...
        .execute_team_tool(
            &context,
            tjuaeui_api_types::TeamToolCall {
                tool: request.tool,
