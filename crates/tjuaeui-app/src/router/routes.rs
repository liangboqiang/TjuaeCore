...
use tjuaeui_channel::channel_routes;
#[cfg(feature = "weixin")]
use tjuaeui_channel::weixin_login_route;
...
use tjuaeui_shell::shell_routes;
use tjuaeui_system::{ClientPrefService, connection_test_routes, system_routes};
use tjuaeui_team::{TeamSessionService, team_routes};
...
fn boundary_error_for_status(status: StatusCode) -> Option<(&'static str, &'static str)> {
    match status {
        StatusCode::BAD_REQUEST => Some(("请求无效。", "BAD_REQUEST")),
        StatusCode::UNAUTHORIZED => Some(("未认证。", "UNAUTHORIZED")),
        StatusCode::FORBIDDEN => Some(("无权访问。", "FORBIDDEN")),
        StatusCode::NOT_FOUND => Some(("找不到路由。", "NOT_FOUND")),
        StatusCode::METHOD_NOT_ALLOWED => Some(("不允许此方法。", "METHOD_NOT_ALLOWED")),
        StatusCode::CONFLICT => Some(("发生冲突。", "CONFLICT")),
        StatusCode::GONE => Some(("资源已不可用。", "GONE")),
        StatusCode::PAYLOAD_TOO_LARGE => Some(("请求体过大。", "PAYLOAD_TOO_LARGE")),
        StatusCode::UNSUPPORTED_MEDIA_TYPE => Some(("不支持的媒体类型。", "UNSUPPORTED_MEDIA_TYPE")),
        StatusCode::UNPROCESSABLE_ENTITY => Some(("请求内容无法处理。", "UNPROCESSABLE_ENTITY")),
        StatusCode::TOO_MANY_REQUESTS => Some(("请求频率受限。", "RATE_LIMITED")),
        StatusCode::INTERNAL_SERVER_ERROR => Some(("服务器内部错误。", "INTERNAL_ERROR")),
        StatusCode::BAD_GATEWAY => Some(("上游服务不可用。", "BAD_GATEWAY")),
        StatusCode::GATEWAY_TIMEOUT => Some(("请求超时。", "GATEWAY_TIMEOUT")),
        _ => None,
    }
}
