//! Bounded wait for the data-dir process-level instance guard
//! (concurrent-startup option A).
//!
...
use tjuaeui_db::DataDirInstanceGuard;
...
mod tests {
    ...
    fn yields_after_bounded_wait_when_peer_holds_the_guard() {
        let dir = std::env::temp_dir().join(format!("tjuaeui-wait-guard-test-{}", std::process::id()));
    ...
    fn acquires_immediately_when_data_dir_is_free() {
        let dir = std::env::temp_dir().join(format!("tjuaeui-wait-guard-free-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let db_path = dir.join("tjuaeui-backend.db");
