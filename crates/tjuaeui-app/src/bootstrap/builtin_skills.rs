...
/// the same binary. When `TJUAE_BUILTIN_SKILLS_PATH` is set, skip
...
pub(super) async fn materialize_builtin_skills(data_dir: &Path) -> Result<()> {
    let skip = std::env::var(tjuaeui_extension::BUILTIN_SKILLS_ENV_VAR)
        .map(|v| !v.is_empty())
        .unwrap_or(false);
    if skip {
        return Ok(());
    ...
    let corpus = tjuaeui_extension::builtin_skills_corpus();
    let marker = tjuaeui_extension::builtin_skills_materialize_marker(corpus, env!("CARGO_PKG_VERSION"));

    tjuaeui_extension::materialize_if_needed(data_dir, corpus, &marker)
        .await
...
mod tests {
    ...
    fn materialize_marker_includes_builtin_skill_corpus_fingerprint() {
        let marker = tjuaeui_extension::builtin_skills_materialize_marker(
            tjuaeui_extension::builtin_skills_corpus(),
