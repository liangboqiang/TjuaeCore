...
impl AppConfig {
    ...
    pub fn database_path(&self) -> PathBuf {
        self.data_dir.join("tjuaeui-backend.db")
    }
}
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            host: tjuaeui_common::constants::DEFAULT_HOST.to_string(),
            port: tjuaeui_common::constants::DEFAULT_PORT,
...
pub fn derive_encryption_key(jwt_secret: &str) -> [u8; 32] {
    ...
    hasher.update(b"tjuaeui-encryption-key:");
...
mod tests {
    ...
    fn test_app_config_database_path() {
        let config = AppConfig {
            data_dir: PathBuf::from("/tmp/tjuaeui"),
            ..Default::default()
        };
        assert_eq!(config.database_path(), PathBuf::from("/tmp/tjuaeui/tjuaeui-backend.db"));
    }
}
