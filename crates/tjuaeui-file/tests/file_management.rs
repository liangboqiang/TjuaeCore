...
use tjuaeui_file::{FileService, IFileService};
...
async fn remove_entry_path_traversal_rejected() {
    ...
    assert!(err.contains("目录穿越"), "got: {err}");
}
...
async fn rename_entry_rejects_path_separator_in_name() {
    ...
    assert!(result.is_err());
    let err = result.unwrap_err().to_string();
    assert!(err.contains("路径分隔符"), "got: {err}");
}
...
#[tokio::test]
async fn create_temp_file_path_in_tjuaeui_dir() {
    let dir = tempfile::tempdir().unwrap();

    let svc = make_service(dir.path());
    let path = svc.create_temp_file("check.txt").await.unwrap();

    assert!(path.contains("tjuaeui"), "temp path should be under tjuaeui dir");
}
...
async fn create_temp_file_rejects_traversal() {
    ...
    assert!(result.is_err());
    let err = result.unwrap_err().to_string();
    assert!(err.contains("目录穿越"), "预期目录穿越错误，实际为：{err}");
}
...
async fn create_temp_file_rejects_path_separator() {
    ...
    assert!(result.is_err());
    let err = result.unwrap_err().to_string();
    assert!(err.contains("路径分隔符"), "预期路径分隔符错误，实际为：{err}");
}
