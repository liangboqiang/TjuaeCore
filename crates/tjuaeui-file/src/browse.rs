...
use tjuaeui_api_types::{BrowseDirectoryResponse, BrowseEntry};
...
pub fn resolve_browse_path(raw: &str, allowed_roots: &[PathBuf]) -> Result<PathBuf, FileError> {
    if raw.contains('\0') {
        return Err(FileError::BadRequest("路径包含空字节".into()));
    }

    let expanded = expand_tilde(raw.trim());
    let canonical = fs::canonicalize(&expanded).map_err(|e| match e.kind() {
        std::io::ErrorKind::NotFound => FileError::NotFound(format!("找不到路径：{}", raw)),
        _ => FileError::BadRequest(format!("无法解析路径 '{}'：{}", raw, e)),
    })?;
    ...
    if !allowed {
        return Err(FileError::PathOutsideSandbox {
            message: format!("路径 '{}' 位于允许的沙箱之外", raw),
...
/// projects (liangboqiang/TjuaeUI#3191) — so strip the prefix before serializing.
...
pub fn list_directory(
    ...
    let metadata = fs::metadata(dir).map_err(|e| FileError::NotFound(format!("无法访问目录：{}", e)))?;
    if !metadata.is_dir() {
        return Err(FileError::BadRequest("该路径不是目录".into()));
    }
...
mod tests {
    ...
    // Regression tests for liangboqiang/TjuaeUI#3191: responses must never carry
