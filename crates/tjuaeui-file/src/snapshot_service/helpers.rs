...
pub(super) const SNAPSHOT_DIR_PREFIX: &str = "tjuaeui-snapshot-";
...
const SNAPSHOT_SIG_NAME: &str = "tjuaeui";
/// Signature email used for snapshot commits.
const SNAPSHOT_SIG_EMAIL: &str = "snapshot@tjuaeui.local";
...
pub(super) struct WorkspaceState {
    ...
    /// - snapshot mode: `/tmp/tjuaeui-snapshot-{hash}` (bare-style git dir).
...
pub(super) fn resolve_workspace(workspace: &str) -> Result<PathBuf, FileError> {
    ...
    if !path.exists() {
        return Err(FileError::NotFound(format!("找不到工作区：{}", workspace)));
    }
