...
fn validate_tjuae_manifest_write(path: &Path, data: &[u8]) -> Result<(), FileError> {
    ...
    let is_tjuae_json = file_name.starts_with(".tjuae-") && file_name.ends_with(".json");
    if file_name != "_meta.json" && !is_tjuae_json {
        return Ok(());
    }
    ...
    if digest.len() != 64 || !digest.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Err(FileError::BadRequest(
            "TJUAE_MANIFEST_INVALID: contentHash 必须是 sha256 摘要".to_owned(),
        ));
    }
    let _ = (manifest.compatibility, manifest.requirements, manifest.extensions);
    Ok(())
}

/// 将提供给前端和 WebSocket 的相对路径统一为正斜杠格式。
/// 绝对文件系统路径仍保留平台原生格式。
fn api_relative_path(path: &Path) -> String {
...
fn build_dir_tree_sync(dir: &Path, root: &Path) -> Result<Vec<DirOrFile>, FileError> {
    let entries =
        std::fs::read_dir(dir).map_err(|e| FileError::BadRequest(format!("无法读取目录 '{}'：{e}", dir.display())))?;
    ...
    for entry in entries {
        ...
        let relative_path = api_relative_path(path.strip_prefix(root).unwrap_or(&path));
...
fn get_file_metadata_sync(path: &Path) -> Result<FileMetadata, FileError> {
    let metadata = std::fs::metadata(path)
        .map_err(|e| FileError::NotFound(format!("无法读取 '{}' 的元数据：{e}", path.display())))?;
...
fn remove_entry_sync(path: &Path) -> Result<(), FileError> {
    let metadata =
        std::fs::metadata(path).map_err(|e| FileError::NotFound(format!("无法删除 '{}'：{e}", path.display())))?;
...
fn rename_entry_sync(path: &Path, new_name: &str) -> Result<PathBuf, FileError> {
    let parent = path
        .parent()
        .ok_or_else(|| FileError::BadRequest(format!("路径 '{}' 没有父目录", path.display())))?;

    let new_path = parent.join(new_name);

    if new_path.exists() {
        return Err(FileError::BadRequest(format!("目标 '{}' 已存在", new_path.display())));
    }
    ...
    for attempt in 0..MAX_ATTEMPTS {
        match std::fs::rename(path, &new_path) {
            ...
            Err(err) if err.kind() == std::io::ErrorKind::PermissionDenied && attempt + 1 < MAX_ATTEMPTS => {
...
fn get_image_base64_sync(path: &Path) -> Result<String, FileError> {
    let bytes =
        std::fs::read(path).map_err(|e| FileError::NotFound(format!("无法读取图片 '{}'：{e}", path.display())))?;
...
impl crate::traits::IFileService for FileService {
    ...
    async fn read_file(&self, path: &str, extra_root: Option<&Path>) -> Result<Option<String>, FileError> {
        if has_traversal(path) {
            return Err(FileError::BadRequest(format!("路径 '{}' 包含无效的目录穿越片段", path)));
        }
    ...
    async fn write_file(&self, path: &str, data: &[u8], workspace: &str) -> Result<bool, FileError> {
        ...
        let relative_path = api_relative_path(
            canonical
    ...
    async fn create_temp_file(&self, file_name: &str) -> Result<String, FileError> {
        ...
        tokio::task::spawn_blocking(move || {
            let tmp_dir = std::env::temp_dir().join("tjuaeui");
    ...
    async fn create_upload_file(
        ...
        if file_name.is_empty() {
            return Err(FileError::BadRequest("文件名不能为空".to_owned()));
        }
        if has_traversal(file_name) {
            return Err(FileError::BadRequest(format!(
                "文件名 '{}' 包含无效的目录穿越片段",
        ...
        let conv_id = match conversation_id {
            Some(id) if !id.is_empty() => {
                if has_traversal(id) || id.contains('/') || id.contains('\\') {
                    return Err(FileError::BadRequest(format!("对话 ID '{}' 包含无效字符", id)));
                }
                Some(id.to_owned())
            }
            _ => None,
        ...
        tokio::task::spawn_blocking(move || {
            let mut dir = std::env::temp_dir().join("tjuaeui");
...
mod tests {
    ...
    fn list_workspace_files_sync_skips_directory_symlinks() {
        ...
        std::os::unix::fs::symlink(&skill_dir, workspace.join("tjuaeui-skills")).unwrap();

        let files = list_workspace_files_sync(&dir.path().join("workspace")).unwrap();

        assert!(
            files.iter().all(|f| f.name != "tjuaeui-skills"),
    ...
    impl tjuaeui_realtime::EventBroadcaster for NullBroadcaster {
        fn broadcast(&self, _msg: tjuaeui_api_types::WebSocketMessage<serde_json::Value>) {}
    }
    ...
    fn tjuae_skill_manifest_accepts_the_canonical_schema() {
        let path = Path::new("test-skill/_meta.json");
    ...
    #[test]
    fn tjuae_skill_manifest_rejects_runtime_preferences() {
        let path = Path::new("test-skill/_meta.json");
        let mut manifest = valid_skill_manifest();
        manifest["autoInject"] = serde_json::json!(true);
        let error = validate_tjuae_manifest_write(path, &serde_json::to_vec(&manifest).unwrap()).unwrap_err();
        assert!(error.to_string().contains("字段无效"));
    }
