...
pub fn validate_path(path: &str, allowed_roots: &[&Path]) -> Result<PathBuf, FileError> {
    let canonical =
        std::fs::canonicalize(path).map_err(|e| FileError::BadRequest(format!("无法解析路径 '{}'：{}", path, e)))?;
    ...
    if is_allowed {
        ...
        Err(FileError::PathOutsideSandbox {
            message: format!("路径 '{}' 位于允许的沙箱之外", path),
            field: Some("path"),
...
pub fn validate_path_for_write(path: &str, allowed_roots: &[&Path]) -> Result<PathBuf, FileError> {
    ...
    let parent = p
        .parent()
        .ok_or_else(|| FileError::BadRequest(format!("路径 '{}' 没有父目录", path)))?;

    let file_name = p
        .file_name()
        .ok_or_else(|| FileError::BadRequest(format!("路径 '{}' 不含文件名", path)))?;

    let canonical_parent = std::fs::canonicalize(parent)
        .map_err(|e| FileError::BadRequest(format!("无法解析路径 '{}' 的父目录：{}", path, e)))?;
...
mod tests {
    ...
    fn validate_path_rejects_nonexistent() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("无法解析"));
    }
