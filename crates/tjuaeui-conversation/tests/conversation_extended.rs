...
use tjuaeui_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_prefixed_id, now_ms};
...
use tjuaeui_db::{IConversationRepository, SqliteConversationRepository, init_database_memory};
...
fn ensure_test_workspace_path() -> String {
    ensure_named_workspace_path("tjuaeui-conversation-extended-test-project")
}
...
async fn t8_6_rejects_before_and_after_together() {
    ...
    assert!(
        matches!(err, ConversationError::BadRequest { reason } if reason == "before、after 与 anchor_message_id 不能同时使用")
    );
}
...
async fn t8_8_rejects_invalid_cursor() {
    ...
    assert!(matches!(err, ConversationError::BadRequest { reason } if reason == "消息游标无效"));
}
...
async fn t9_6_search_result_includes_conversation_model() {
    ...
    // Search surfaces conversation.model only for tjuae_cli (the only type that
    // carries a top-level model under the tjuae_cli-only rule).
    let tjuae_cli_req: CreateConversationRequest = serde_json::from_value(json!({
        "type": "tjuaecli",
        "model": { "provider_id": "p1", "model": "claude-sonnet-4-20250514" },
        "extra": { "workspace": workspace }
    }))
    .unwrap();
    let conv = svc.create(USER_ID, tjuae_cli_req).await.unwrap();
...
async fn t10_1_same_workspace() {
    let (svc, _repo, _b) = setup().await;
    let shared_workspace = ensure_named_workspace_path("tjuaeui-conversation-extended-shared-workspace");
    let other_workspace = ensure_named_workspace_path("tjuaeui-conversation-extended-other-workspace");
...
async fn t10_2_no_associated() {
    let (svc, _repo, _b) = setup().await;
    let workspace = ensure_named_workspace_path("tjuaeui-conversation-extended-unique-workspace");
