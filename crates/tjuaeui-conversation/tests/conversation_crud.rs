...
use serde_json::json;
use std::sync::Mutex;
use tjuaeui_ai_agent::{AgentError, IWorkerTaskManager};
use tjuaeui_api_types::{
    CreateConversationRequest, ListConversationsQuery, UpdateConversationRequest, WebSocketMessage,
};
use tjuaeui_common::{AgentKillReason, AgentType, ConversationSource, ConversationStatus, TimestampMs};
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("tjuaeui-conversation-crud-test-project");
...
async fn t1_1_create_with_defaults() {
    ...
    // Non-tjuae_cli: top-level model is None.
...
async fn t1_2_create_each_agent_type() {
    ...
    let types = vec![("acp", AgentType::Acp), ("tjuaecli", AgentType::TjuaeCli)];

    for (type_str, expected_type) in types {
        let body = if type_str == "tjuaecli" {
            json!({
                "type": type_str,
        ...
        if type_str == "tjuaecli" {
            assert!(resp.model.is_some(), "tjuaecli should keep top-level model");
        } else {
    ...
    for type_str in ["openclaw-gateway", "nanobot", "remote", "gemini"] {
        ...
        match err {
            ConversationError::BadRequest { reason } => {
                assert!(reason.contains("不再支持"), "{type_str} 的错误不符合预期：{reason}");
            }
            other => panic!("期望 {type_str} 返回 BadRequest，实际为 {other:?}"),
        }
    }
}
...
async fn t4_4_extra_merge_preserves_existing_keys() {
    let (svc, _, task_mgr) = setup().await;
    let dir = std::env::temp_dir().join("tjuaeui-conversation-crud-extra-merge");
...
async fn full_lifecycle_create_get_update_delete() {
    let (svc, broadcaster, task_mgr) = setup().await;
    let updated_workspace = std::env::temp_dir().join("tjuaeui-conversation-crud-updated-workspace");
...
async fn create_rejects_deprecated_remote_runtime() {
    ...
    match err {
        ConversationError::BadRequest { reason } => {
            assert!(reason.contains("不再支持"), "意外错误：{reason}");
        }
        other => panic!("期望 BadRequest，实际为 {other:?}"),
    }
}

#[tokio::test]
async fn create_accepts_top_level_model_for_tjuae_cli() {
    let (svc, _, _task_mgr) = setup().await;
    ...
    let model = resp.model.expect("tjuaecli response should carry top-level model");
    assert_eq!(model.provider_id, "p1");
    assert_eq!(model.model, "gpt-4o");
}

#[tokio::test]
async fn create_tjuae_cli_strips_extra_model_field() {
    let (svc, _, _task_mgr) = setup().await;
    let workspace = ensure_test_workspace_path();
    ...
    assert!(
        ...
        "tjuaecli create must strip extra.model to avoid dual source of truth; got {:?}",
        resp.extra
    );
...
#[tokio::test]
async fn update_accepts_top_level_model_for_tjuae_cli() {
    let (svc, _, task_mgr) = setup().await;
...
#[tokio::test]
async fn update_tjuae_cli_strips_extra_model_from_patch() {
    let (svc, _, task_mgr) = setup().await;
    ...
    // Client mistakenly sends extra.model on an tjuae_cli PATCH. It should be
    ...
    assert!(
        ...
        "tjuaecli PATCH must strip extra.model; got {:?}",
        updated.extra
    );
...
async fn create_acp_seeds_acp_session_runtime_from_extra() {
    use tjuaeui_db::{SqliteAcpSessionRepository, init_database_memory};

    let db = init_database_memory().await.unwrap();
    let repo = Arc::new(tjuaeui_db::SqliteConversationRepository::new(db.pool().clone()));
    let broadcaster = Arc::new(TestBroadcaster::new());
    let agent_metadata_repo: Arc<dyn tjuaeui_db::IAgentMetadataRepository> =
        Arc::new(tjuaeui_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    let acp_session_repo: Arc<dyn tjuaeui_db::IAcpSessionRepository> =
    ...
    let task_mgr: Arc<dyn IWorkerTaskManager> = Arc::new(NoopTaskManager);
    let svc = tjuaeui_conversation::ConversationService::new(
        std::env::temp_dir(),
        broadcaster.clone(),
        Arc::new(EmptySkillResolver),
        task_mgr,
