...
use tjuaeui_api_types::{ConversationArtifactResponse, ConversationResponse, MessageResponse, MessageSearchItem};
use tjuaeui_common::{
    AgentType, ConversationSource, ConversationStatus, MessagePosition, MessageStatus, MessageType, ProviderWithModel,
};
use tjuaeui_db::MessageSearchRow;
use tjuaeui_db::models::{ConversationArtifactRow, ConversationRow, MessageRow};
...
///
/// TjuaeUI stores the full provider object (`TProviderWithModel`) which includes
...
mod tests {
    use super::*;
    use serde_json::json;
    use tjuaeui_common::{AgentType, ConversationSource, ConversationStatus};
    ...
    #[test]
    fn parse_provider_with_model_tjuaeui_format() {
    ...
    fn row_to_response_marks_workspace_inside_data_dir_as_temporary() {
        let row = make_row(
            ...
            Some("tjuaeui"),
            None,
            r#"{"workspace":"/srv/tjuaeui-data/conversations/claude-temp-abc"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/tjuaeui-data")).unwrap();
    ...
    fn row_to_response_marks_missing_workspace_as_non_temporary() {
        let row = make_row("acp", "pending", Some("tjuaeui"), None, r#"{}"#);
        let resp = row_to_response(row, Path::new("/srv/tjuaeui-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn test_search_row_to_item_builds_nested_conversation() {
        let row = MessageSearchRow {
            ...
            conversation_source: Some("tjuaeui".into()),
        ...
        assert_eq!(item.conversation.source, Some(ConversationSource::TjuaeUI));
