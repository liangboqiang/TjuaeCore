use base64::Engine;
use tjuaeui_db::MessagePageCursor;
...
pub fn decode_message_cursor(raw: &str) -> Result<MessagePageCursor, ConversationError> {
    let encoded = raw
        .strip_prefix("v1.")
        .ok_or_else(|| ConversationError::bad_request("消息游标无效"))?;
    let bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD
        .decode(encoded)
        .map_err(|_| ConversationError::bad_request("消息游标无效"))?;
    let cursor: MessageCursorV1 =
        serde_json::from_slice(&bytes).map_err(|_| ConversationError::bad_request("消息游标无效"))?;
    if cursor.id.is_empty() {
        return Err(ConversationError::bad_request("消息游标无效"));
    }
