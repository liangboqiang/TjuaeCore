...
//! given conversation; otherwise an tjuae_cli job that runs fine
//! interactively can fail under cron with `Provider '<vendor>' not
//! found` (agent-options lookup regression). Centralising the lookup here forces
...
//! we return an empty `ProviderWithModel`; non-tjuae_cli factory branches
//! ignore the field, and the tjuae_cli branch surfaces a clear "provider
...
/// `provider_id: ""`, `model: ""`, `use_model: None`. Non-tjuae_cli factories
/// ignore the field, while the tjuae_cli factory surfaces a clear "Provider
...
mod tests {
    ...
    /// label `"tjuaecli"` when the conversation's model JSON was an older
    /// shape) and `send_message` parsed the row directly, so the cron
    /// path would emit `Provider 'tjuaecli' not found` while the
    ...
    fn interactive_and_cron_paths_agree_on_provider_id() {
        ...
        assert_ne!(canonical_resolved.provider_id, "tjuaecli");
        assert_ne!(legacy_resolved.provider_id, "tjuaecli");
    }
}
