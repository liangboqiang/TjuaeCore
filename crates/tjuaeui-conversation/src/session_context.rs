...
use tjuaeui_api_types::{AcpBuildExtra, TeamSessionBinding, TjuaeCliBuildExtra};
use tjuaeui_common::{AgentType, WorkspacePathValidationError, validate_workspace_path_availability};
use tjuaeui_db::models::ConversationRow;
use tjuaeui_db::{IAcpSessionRepository, IAgentMetadataRepository};
...
impl<'a> SessionContextBuilder<'a> {
    ...
    pub(crate) async fn build_options(
        &self,
        row: &ConversationRow,
        seed: Option<TjuaeCliRuntimePermissionSeed>,
    ...
    async fn resolve_acp_identity(
        ...
        extra: &serde_json::Value,
        session_row: Option<&tjuaeui_db::models::AcpSessionRow>,
        ...
        if agent_source != "builtin" {
            return Err(ConversationError::BadRequest {
                reason: "非内置 ACP Agent 必须在 extra 中提供 agent_id".to_owned(),
            });
        }
...
/// Runtime permission gate inputs for an tjuae_cli rebuild, loaded from the
/// conversation's persisted assistant snapshot in the service layer
/// (`ConversationService::load_tjuae_cli_permission_seed`) and threaded down so
...
///   sessions.
#[derive(Debug, Clone)]
pub(crate) struct TjuaeCliRuntimePermissionSeed {
    pub default_permission_mode: String,
    pub resolved_permission_value: Option<String>,
}

fn build_tjuae_cli_context(
    row: &ConversationRow,
    extra: serde_json::Value,
    team: Option<TeamSessionBinding>,
    permission_seed: Option<TjuaeCliRuntimePermissionSeed>,
) -> TjuaeCliSessionBuildContext {
    let mut config: TjuaeCliBuildExtra = match serde_json::from_value(extra.clone()) {
        Ok(config) => config,
        Err(err) => {
            warn!(
                conversation_id = %row.id,
                error = %err,
                "session_context: invalid tjuaecli extra; using defaults"
            );
            TjuaeCliBuildExtra::default()
        }
    };
    config.user_id.get_or_insert_with(|| row.user_id.clone());
    apply_team_seed_to_tjuae_cli_config(&team, &mut config);
    let belongs_to_team = team.is_some();
...
fn apply_runtime_permission_seed(
    seed: TjuaeCliRuntimePermissionSeed,
    row: &ConversationRow,
    config: &mut TjuaeCliBuildExtra,
) {
    ...
    if let Some(resolved) = seed.resolved_permission_value.filter(|value| !value.is_empty()) {
        debug!(
            conversation_id = %row.id,
            "session_context: tjuaecli rebuild seeded from resolved runtime permission"
        );
...
fn apply_team_seed_to_tjuae_cli_config(team: &Option<TeamSessionBinding>, config: &mut TjuaeCliBuildExtra) {
    let Some(team) = team else {
        return;
    };
    if config.backend.as_deref().is_none_or(str::is_empty) {
...
fn decode_persisted_session_state(state: tjuaeui_db::PersistedSessionState) -> PersistedSessionState {
...
mod tests {
    ...
    fn tjuae_cli_context(context: AgentSessionContext) -> TjuaeCliSessionBuildContext {
        match context.kind {
            AgentSessionKind::TjuaeCli(tjuae_cli) => *tjuae_cli,
            other => panic!("expected TjuaeCLI context, got {other:?}"),
        }
    }
    ...
    async fn acp_non_builtin_without_agent_id_is_rejected() {
        ...
        assert!(err.to_string().contains("必须在 extra 中提供 agent_id"));
    }
    ...
    #[tokio::test]
    async fn tjuae_cli_team_extra_is_exposed_as_typed_context() {
        let repos = setup().await;
        let row = row(
            "tjuaecli",
            serde_json::json!({
        ...
        assert_eq!(team.runtime_seed.backend.as_deref(), Some("tjuaecli"));
        assert_eq!(team.mcp.as_ref().unwrap().stdio.port, 5252);

        let tjuae_cli = tjuae_cli_context(context);
        assert!(tjuae_cli.belongs_to_team);
        assert_eq!(tjuae_cli.config.team_mcp_stdio_config.unwrap().port, 5252);
    }

    #[tokio::test]
    async fn tjuae_cli_uses_conversation_model_and_ignores_legacy_extra_model() {
        let repos = setup().await;
        let row = row(
            "tjuaecli",
            serde_json::json!({
    ...
    async fn workspace_empty_uses_auto_path_and_is_not_custom() {
        let repos = setup().await;
        let row = row("tjuaecli", serde_json::json!({}), None);

        let context = repos.builder().build(&row).await.unwrap();
        assert!(!context.workspace.is_custom);
        assert!(context.workspace.stored_path.is_empty());
        assert!(context.workspace.path.ends_with("tjuaecli-temp-conv-1"));
    }
    ...
    fn workspace_validation_failure_logs_redacted_runtime_check() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        let raw_path = "/tmp/tjuaeui-secret-workspace-token-12345";
        let captured = capture_logs(Level::WARN, || {
            runtime.block_on(async {
                let repos = setup().await;
                let row = row("tjuaecli", serde_json::json!({ "workspace": raw_path }), None);
        ...
        assert!(captured.contains("tjuaeui_feedback_diagnostics"), "{captured}");
    ...
    fn assert_archived(err: ConversationError, expected_id: &str) {
        match err {
            ConversationError::Archived { id, reason } => {
                ...
                assert!(reason.contains("该历史对话已无法继续"), "意外的归档原因：{reason}");
            }
            other => panic!("期望 ConversationError::Archived，实际为 {other:?}"),
        }
    }
    ...
    async fn legacy_agent_types_are_archived_before_runtime_context() {
        ...
        for (agent_type, extra) in [
            ...
            (
                "codex",
                serde_json::json!({ "workspace": "/tmp/tjuaeui-codex-history" }),
            ),
            (
                "openclaw-gateway",
    ...
    fn tjuae_cli_seed(mode: &str, resolved: Option<&str>) -> TjuaeCliRuntimePermissionSeed {
        TjuaeCliRuntimePermissionSeed {
    ...
    #[test]
    fn tjuae_cli_auto_mode_rebuild_adopts_resolved_permission_value() {
        // AC#1: auto happy path — runtime yolo survives rebuild.
        let row = row("tjuaecli", serde_json::json!({ "session_mode": "default" }), None);
        let ctx = build_tjuae_cli_context(
            &row,
            serde_json::json!({ "session_mode": "default" }),
            None,
            Some(tjuae_cli_seed("auto", Some("yolo"))),
        );
        assert_eq!(ctx.config.session_mode.as_deref(), Some("yolo"));
    }

    #[test]
    fn tjuae_cli_auto_mode_resolved_overrides_create_time_seed() {
        // AC#2: existing-data compat — create-time non-yolo seed is overridden.
        let row = row("tjuaecli", serde_json::json!({ "session_mode": "auto_edit" }), None);
        let ctx = build_tjuae_cli_context(
            &row,
    ...
    #[test]
    fn tjuae_cli_fixed_mode_ignores_resolved_permission_value() {
        ...
        let ctx = build_tjuae_cli_context(
            ...
            serde_json::json!({ "session_mode": "default" }),
            None,
            Some(tjuae_cli_seed("fixed", Some("yolo"))),
        );
        assert_eq!(ctx.config.session_mode.as_deref(), Some("default"));
    }

    #[test]
    fn tjuae_cli_team_bound_session_ignores_resolved_permission_value() {
