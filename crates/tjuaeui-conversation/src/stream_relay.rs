...
use tjuaeui_ai_agent::protocol::events::{ErrorEventData, TipType};
use tjuaeui_ai_agent::{AgentSendError, AgentStreamEvent, protocol::events::ThinkingEventData};
...
use tjuaeui_api_types::{AgentErrorCode, WebSocketMessage};
use tjuaeui_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let mut outcome = if deleting {
                        ...
                        self.finalize(
                            &full_text_buffer,
                            &text_segments,
                            &AgentStreamEvent::Finish(tjuaeui_ai_agent::protocol::events::FinishEventData::default()),
...
mod tests {
    ...
    use tjuaeui_ai_agent::protocol::events::{ErrorEventData, FinishEventData, TextEventData, ThinkingEventData};
    ...
    async fn needs_auth_tip_sets_summary_flag_on_finish() {
        use tjuaeui_ai_agent::protocol::events::{TipType, TipsEventData};
    ...
    async fn run_warning_tip_with_finish_persists_warning_tip() {
        ...
        tx.send(AgentStreamEvent::Tips(
            tjuaeui_ai_agent::protocol::events::TipsEventData {
                content: String::new(),
                tip_type: tjuaeui_ai_agent::protocol::events::TipType::Warning,
                code: Some("ACP_EMPTY_TURN".into()),
                params: None,
            },
        ))
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(tjuaeui_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
        ...
        assert_eq!(content["content"], "模型提供商拒绝了请求");
        assert_eq!(content["type"], "error");
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use tjuaeui_ai_agent::protocol::events::tool_call::{
    ...
    async fn run_tool_group_persists_message() {
        use tjuaeui_ai_agent::protocol::events::tool_call::{ToolCallStatus, ToolGroupEntry};
    ...
    async fn complete_conversation_ignores_not_found_after_delete() {
        ...
        let bus: Arc<dyn EventBroadcaster> = Arc::new(tjuaeui_realtime::BroadcastEventBus::new(64));
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, tjuaeui_db::MessageRowUpdate)>>,
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, tjuaeui_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _id: &str) -> Result<Option<tjuaeui_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn create(&self, _row: &tjuaeui_db::models::ConversationRow) -> Result<(), DbError> {
            Ok(())
        }
        async fn update(&self, _id: &str, _updates: &tjuaeui_db::ConversationRowUpdate) -> Result<(), DbError> {
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &tjuaeui_db::ConversationFilters,
        ) -> Result<tjuaeui_common::PaginatedResult<tjuaeui_db::models::ConversationRow>, DbError> {
            Ok(tjuaeui_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<tjuaeui_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<tjuaeui_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn list_messages_page(
            &self,
            _conv_id: &str,
            _params: &tjuaeui_db::MessagePageParams,
        ) -> Result<tjuaeui_db::MessagePageResult, DbError> {
            Ok(tjuaeui_db::MessagePageResult {
                items: vec![],
        ...
        async fn update_message(&self, id: &str, updates: &tjuaeui_db::MessageRowUpdate) -> Result<(), DbError> {
            if self.not_found.load(Ordering::Acquire) {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<tjuaeui_common::PaginatedResult<tjuaeui_db::MessageSearchRow>, DbError> {
            Ok(tjuaeui_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
