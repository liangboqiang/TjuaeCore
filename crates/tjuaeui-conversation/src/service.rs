...
use tjuaeui_ai_agent::session_context::{AgentSessionContext, AgentSessionKind};
...
fn parse_agent_type_from_metadata(value: &str) -> Result<AgentType, ConversationError> {
    ...
    serde_json::from_str::<AgentType>(&quoted).map_err(|_| ConversationError::BadRequest {
        reason: format!("agent_metadata 中的助手 Agent 类型不受支持：{value}"),
    })
}
...
fn resolve_create_agent_type(
    ...
    explicit_type.ok_or_else(|| ConversationError::BadRequest {
        reason: "创建对话时必须提供 `type` 或 `assistant.id`。".into(),
    })
}
...
impl ConversationService {
    ...
    /// `delete()` removes the conversation row. Used by `tjuaeui-app` to wire up `WorkerTaskManagerImpl`
...
impl ConversationService {
    ...
    ) -> Result<tjuaeui_api_types::ConversationAssistantIdentityResponse, ConversationError> {
    ...
    /// to `tjuaeui`, and broadcasts `conversation.listChanged(created)`.
    ...
    pub async fn create(
        ...
        let now = now_ms();
        let source = req.source.unwrap_or(ConversationSource::TjuaeUI);
        ...
        // Type-aware rule: top-level `model` is tjuae_cli-only. Other agent types
        ...
        if effective_type != AgentType::TjuaeCli && req.model.is_some() {
            return Err(ConversationError::BadRequest {
                reason: format!(
                    "仅 tjuaecli 对话接受顶层 `model`；{} 类型请通过 `extra` 传递模型",
                    effective_type.serde_name()
                ),
            });
        }

        // tjuae_cli source-of-truth rule: top-level `model` wins. If an older client
        // still packs `extra.model`, strip it before persist so the stored row
        // has a single canonical model representation.
        if effective_type == AgentType::TjuaeCli
            && let Some(obj) = extra.as_object_mut()
            && obj.remove("model").is_some()
        {
            warn!("tjuaecli create: stripped legacy `extra.model`; top-level `model` is canonical");
        }
            ...
            Some(obj) => match obj.remove("selected_session_mcp_servers") {
                Some(value) => Some(serde_json::from_value::<Vec<SessionMcpServer>>(value).map_err(|e| {
                    ConversationError::BadRequest {
                        reason: format!("selected_session_mcp_servers 无效：{e}"),
                    }
                })?),
                None => None,
            },
    ...
    pub async fn update(
        ...
        {
            return Err(ConversationError::BadRequest {
                reason: "创建后不可修改 extra.skills 和 MCP 快照".into(),
            });
        }
            ...
            && (incoming.get("current_model_id").is_some() || incoming.get("current_mode_id").is_some())
        {
            ...
            return Err(ConversationError::BadRequest {
                reason: "必须通过 /config-options 修改 ACP 运行时当前模式或模型，不可修改 conversation.extra".into(),
            });
        }

        // Type-aware rule: top-level `model` is tjuae_cli-only. For non-tjuae_cli
        // conversations, model/mode must be updated via `extra` (see spec
        // 2026-05-12).
        if existing_type != AgentType::TjuaeCli && req.model.is_some() {
            return Err(ConversationError::BadRequest {
                reason: format!(
                    "仅 tjuaecli 对话接受顶层 `model`；{} 类型请通过 `extra` 传递模型",
                    existing.r#type
        ...
        // Merge extra if provided. For tjuae_cli, strip `extra.model` post-merge
        ...
        let merged_extra = if let Some(new_extra) = &req.extra {
            ...
            if existing_type == AgentType::TjuaeCli
                && let Some(obj) = existing_extra.as_object_mut()
                && obj.remove("model").is_some()
            {
                warn!("tjuaecli update: stripped legacy `extra.model` from merged extra");
            }
    ...
    pub async fn list_messages(
        ...
        if message_locator_count(&query) > 1 {
            return Err(ConversationError::bad_request(
                "before、after 与 anchor_message_id 不能同时使用",
            ));
        }
    ...
    pub async fn send_message(
        ...
        if let Some(team_id) = team_id_from_extra(&row.extra) {
            ...
            return Err(ConversationError::Forbidden {
                reason: "团队对话必须通过团队 API 发送消息".into(),
            });
        }
...
impl ConversationService {
    /// Loads the persisted runtime permission gate inputs for an tjuae_cli
    /// rebuild. Returns `None` for non-tjuae_cli conversations and for tjuae_cli
    ...
    async fn load_tjuae_cli_permission_seed(
        &self,
        row: &tjuaeui_db::models::ConversationRow,
    ) -> Result<Option<TjuaeCliRuntimePermissionSeed>, ConversationError> {
        if row.r#type != AgentType::TjuaeCli.serde_name() {
            return Ok(None);
        }
        let snapshot = self
            .conversation_repo
            ...
            .map_err(|e| {
                ConversationError::internal(format!(
                    "Failed to load assistant snapshot for tjuaecli permission seed: {e}"
                ))
            })?;
        Ok(snapshot.map(|snapshot| TjuaeCliRuntimePermissionSeed {
            default_permission_mode: snapshot.default_permission_mode,
...
/// name (e.g. `"tjuaecli"`). Falls back to the agent type's serde name
...
impl ConversationService {
    async fn resolve_mcp_support_policy(
        ...
        match agent_type {
            ...
            AgentType::TjuaeCli => Ok(McpSupportPolicy::TJUAE_CLI),
            _ => Ok(McpSupportPolicy::TJUAE_CLI),
        }
    }
}
...
fn classify_repo_mcp_status(
    row: &tjuaeui_db::models::McpServerRow,
...
mod tests {
    ...
    fn enum_to_db_source() {
        assert_eq!(enum_to_db(&ConversationSource::TjuaeUI).unwrap(), "tjuaeui");
    ...
    fn response_with_type(agent_type: tjuaeui_common::AgentType) -> ConversationResponse {
    ...
    #[test]
    fn assistant_lineage_extracts_tjuae_cli_preset_id() {
        use tjuaeui_common::AgentType;
        let response = response_with_type(AgentType::TjuaeCli);
        let extra = json!({ "preset_assistant_id": "preset-xyz" });
        let lineage = AssistantLineage::from_response_and_extra(&response, &extra);
        assert_eq!(lineage.agent_type, "tjuaecli");
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_tjuaeui_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
            McpSupportPolicy::TJUAE_CLI,
        );
