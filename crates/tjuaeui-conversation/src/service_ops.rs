...
use tjuaeui_common::{AgentKillReason, ErrorChain};
use tracing::warn;
...
impl ConversationService {
    ...
    pub async fn browse_workspace(
        ...
        let canonical_browse = browse_path
            .canonicalize()
            .map_err(|_| ConversationError::not_found_reason("找不到目录"))?;
        ...
        if depth > MAX_DIR_DEPTH {
            return Err(ConversationError::BadRequest {
                reason: format!("目录深度超过上限 {MAX_DIR_DEPTH}"),
            });
        }
