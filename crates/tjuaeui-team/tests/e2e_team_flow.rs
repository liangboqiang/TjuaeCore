...
use tjuaeui_ai_agent::protocol::events::{AgentStreamEvent, FinishEventData};
use tjuaeui_ai_agent::shared_kernel::approval_key;
use tjuaeui_ai_agent::types::{BuildTaskOptions, SendMessageData};
use tjuaeui_api_types::AgentModeResponse;
use tjuaeui_api_types::WebSocketMessage;
use tjuaeui_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs, now_ms};
use tjuaeui_db::ITeamRepository;
use tjuaeui_realtime::EventBroadcaster;
use tjuaeui_team::event_loop::AgentLoopContext;
...
use tjuaeui_team::service::TeamSessionService;
use tjuaeui_team::{TeamAgent, TeamProjectionMessageStore, TeamSession, TeammateRole};
...
impl TeamProjectionMessageStore for NoopProjectionStore {
    ...
    async fn find_projected_message(
        ...
        _msg_id: &str,
        _msg_type: &str,
    ) -> Result<Option<tjuaeui_db::models::MessageRow>, tjuaeui_team::TeamError> {
        Ok(None)
    }

    async fn insert_projected_message(
        &self,
        _row: &tjuaeui_db::models::MessageRow,
...
impl IAgentTask for RecordingAgent {
    ...
    async fn send_message(&self, data: SendMessageData) -> Result<(), tjuaeui_ai_agent::AgentSendError> {
        ...
        match &self.fail_with {
            Some(msg) => Err(tjuaeui_ai_agent::AgentSendError::from_agent_error(
...
#[async_trait]
impl tjuaeui_ai_agent::IWorkerTaskManager for StubTaskManager {
...
fn backend_path() -> Arc<PathBuf> {
    Arc::new(PathBuf::from("/tmp/tjuaecore-e2e-test"))
}
...
async fn setup_session_with_turn_recorder_inner(
    ...
    let task_manager_dyn: Arc<dyn tjuaeui_ai_agent::IWorkerTaskManager> = task_manager.clone();
    ...
    let team = tjuaeui_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
...
async fn setup_session_with_runtime_ports(
    ...
    let task_manager_dyn: Arc<dyn tjuaeui_ai_agent::IWorkerTaskManager> = task_manager;
    let team = tjuaeui_team::types::Team {
        id: "e2e-team".into(),
        name: "E2E Team".into(),
        workspace: "/tmp/e2e-team".into(),
...
async fn s2a_mcp_team_send_message_rejects_without_live_service() {
    ...
    assert!(mcp_text(&resp).contains("团队服务不可用"));
...
async fn s3a_on_agent_finish_writes_idle_notification_to_lead() {
    ...
    session
        .scheduler()
        .set_status("worker-1", tjuaeui_team::TeammateStatus::Working)
        .await
        .unwrap();
    ...
    assert_eq!(
        worker_status,
        tjuaeui_team::TeammateStatus::Idle,
...
async fn s3b_lead_finish_does_not_write_idle_notification() {
    ...
    session
        .scheduler()
        .set_status("lead-1", tjuaeui_team::TeammateStatus::Working)
        .await
        .unwrap();
...
async fn s3c_finish_triggers_lead_wake_with_idle_notification() {
    ...
    session
        ...
        .write(
            ...
            "worker-1",
            "lead-1",
            tjuaeui_team::MailboxMessageType::Message,
            "do the work",
            None,
        )
        .await
...
async fn s8b_worker_cannot_call_spawn_agent() {
    ...
    assert!(
        text.contains("只有团队负责人"),
        "error must explain the lead-only restriction; got {text}"
    );
...
async fn one_notify_drains_five_intents_after_busy_turn() {
    ...
    for index in 1..=5 {
        ...
        assert_eq!(ack.enqueue_status, tjuaeui_api_types::TeamMessageEnqueueStatus::Queued);
...
async fn s10_error_finish_sets_agent_status_to_error() {
    ...
    assert_eq!(
        status,
        tjuaeui_team::TeammateStatus::Error,
