...
use tjuaeui_team::{Mailbox, TaskBoard, TeamAgent, TeamMcpServer, TeamPromptDumpConfig, TeammateManager, TeammateRole};
...
async fn sp2_legacy_backend_alias_rejected() {
    ...
    assert!(is_error_response(&resp));
    let text = extract_text(&resp);
    assert!(text.contains("backend 不再接受"), "unexpected error: {text}");
...
async fn ttl6_task_list_rejects_invalid_filter_arguments() {
    ...
    for (id, args, expected) in [
        (2, json!({"slot_id": "worker-1"}), "参数无效"),
        (3, json!({"status": "blocked"}), "参数无效"),
        (4, json!({"status": []}), "参数无效"),
        (5, json!({"limit": 0}), "参数无效"),
    ] {
...
async fn sb1_bridge_config_generation() {
    use tjuaeui_team::{TeamMcpStdioConfig, TeamMcpStdioServerSpec};
    ...
    let config = TeamMcpStdioConfig {
        ...
        slot_id: "lead-1".into(),
        binary_path: "/bin/tjuaecore".into(),
    };

    let spec = TeamMcpStdioServerSpec::from_config("/bin/tjuaecore", &config);
