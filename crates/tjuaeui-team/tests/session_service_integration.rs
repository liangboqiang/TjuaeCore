...
use tjuaeui_ai_agent::task_manager::AgentFactory;
use tjuaeui_ai_agent::types::BuildTaskOptions;
use tjuaeui_ai_agent::{ActiveLeaseRegistry, AgentError, IWorkerTaskManager, WorkerTaskManagerImpl};
...
use tjuaeui_team::session::SpawnAgentRequest;
use tjuaeui_team::{
...
impl FakeConversationPorts {
    fn new(repo: Arc<MockConversationRepo>) -> Self {
        let workspace_root = std::env::temp_dir().join(format!(
            "tjuaeui-team-fake-workspaces-{}",
            tjuaeui_common::generate_id()
        ));
        Self {
            repo,
            workspace_root,
...
impl TeamConversationProvisioningPort for FakeConversationPorts {
    async fn create_team_conversation(
    ...
    ) -> Result<TeamConversationCreateResult, tjuaeui_team::TeamError> {
        ...
        let id = tjuaeui_common::generate_id();
    ...
    async fn conversation_workspace(&self, conversation_id: &str) -> Result<Option<String>, tjuaeui_team::TeamError> {
    ...
    async fn conversation_assistant_id(
        &self,
        conversation_id: &str,
    ) -> Result<Option<String>, tjuaeui_team::TeamError> {
        Ok(self.repo.get_extra(conversation_id).and_then(|extra| {
            extra
                .get("assistant_id")
                .or_else(|| extra.get("preset_assistant_id"))
    ...
    async fn create_team_temp_workspace(&self, team_id: &str) -> Result<String, tjuaeui_team::TeamError> {
        if self
        ...
        self.repo
            .update(
                ...
                &ConversationRowUpdate {
                    ...
                    status: None,
                    updated_at: Some(tjuaeui_common::now_ms()),
                    project_id: None,
                    folder_id: None,
                },
            )
    ...
    async fn save_acp_runtime_mode(&self, conversation_id: &str, mode: &str) -> Result<(), tjuaeui_team::TeamError> {
    ...
    async fn get_config_options(
        &self,
        conversation_id: &str,
    ) -> Result<GetConfigOptionsResponse, tjuaeui_team::TeamError> {
        let extra = self
            .repo
            .get_extra(conversation_id)
            .ok_or_else(|| tjuaeui_team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
        let model = extra
    ...
    async fn warmup_agent_process(
        ...
        let row = self
            ...
            .ok_or_else(|| {
                tjuaeui_team::TeamError::InvalidRequest(format!("conversation not found: {conversation_id}"))
            })?;
        let extra: serde_json::Value = serde_json::from_str(&row.extra)?;
        let team = tjuaeui_api_types::TeamSessionBinding::from_extra_value(&extra)?;
        ...
        task_manager
            ...
            .await
            .map_err(|error| {
                tjuaeui_team::TeamError::InvalidRequest(format!("failed to warm up agent process: {error}"))
            })?;
        Ok(())
    }
...
impl TeamProjectionMessageStore for FakeConversationPorts {
    ...
    async fn find_projected_message(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, tjuaeui_team::TeamError> {
        Ok(self
            .repo
    ...
    async fn insert_projected_message(&self, row: &MessageRow) -> Result<(), tjuaeui_team::TeamError> {
...
impl TeamConversationLookupPort for FakeConversationPorts {
    async fn lookup_team_binding_by_conversation(
        &self,
        conversation_id: &str,
    ) -> Result<Option<TeamConversationBindingLookup>, tjuaeui_team::TeamError> {
...
struct FullMockTeamRepo {
    inner: MockTeamRepo,
    teams: std::sync::Mutex<Vec<tjuaeui_db::models::TeamRow>>,
...
impl ITeamRepository for FullMockTeamRepo {
    async fn create_team(&self, row: &tjuaeui_db::models::TeamRow) -> Result<(), DbError> {
        self.teams.lock().unwrap().push(row.clone());
        Ok(())
    }
    async fn list_teams(&self) -> Result<Vec<tjuaeui_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().clone())
    }
    async fn list_teams_by_user(&self, user_id: &str) -> Result<Vec<tjuaeui_db::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn get_team(&self, id: &str) -> Result<Option<tjuaeui_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().iter().find(|t| t.id == id).cloned())
    }
    async fn update_team(&self, id: &str, params: &tjuaeui_db::UpdateTeamParams) -> Result<(), DbError> {
        ...
        team.updated_at = tjuaeui_common::now_ms();
        Ok(())
    }
    ...
    async fn write_message(&self, row: &tjuaeui_db::models::MailboxMessageRow) -> Result<(), DbError> {
    ...
    async fn read_unread_and_mark(
        ...
        team_id: &str,
        to_agent_id: &str,
    ) -> Result<Vec<tjuaeui_db::models::MailboxMessageRow>, DbError> {
    ...
    async fn create_task(&self, row: &tjuaeui_db::models::TeamTaskRow) -> Result<(), DbError> {
    ...
    async fn find_task_by_id(
        ...
        team_id: &str,
        task_id: &str,
    ) -> Result<Option<tjuaeui_db::models::TeamTaskRow>, DbError> {
        self.inner.find_task_by_id(team_id, task_id).await
    }
    async fn update_task(&self, task_id: &str, params: &tjuaeui_db::UpdateTaskParams) -> Result<(), DbError> {
        self.inner.update_task(task_id, params).await
    }
    async fn list_tasks(&self, team_id: &str) -> Result<Vec<tjuaeui_db::models::TeamTaskRow>, DbError> {
...
impl IWorkerTaskManager for CountingTaskManager {
    fn get_task(&self, conversation_id: &str) -> Option<tjuaeui_ai_agent::AgentInstance> {
    ...
    fn collect_idle(&self, idle_threshold_ms: tjuaeui_common::TimestampMs) -> Vec<String> {
...
mod mock_agent {
    use tjuaeui_ai_agent::AgentError;
    use tjuaeui_ai_agent::agent_task::{IAgentTask, IMockAgent};
    use tjuaeui_ai_agent::protocol::events::AgentStreamEvent;
    use tjuaeui_ai_agent::types::SendMessageData;
    use tjuaeui_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs};
    ...
    impl IAgentTask for MockAgent {
        ...
        async fn send_message(&self, _data: SendMessageData) -> Result<(), tjuaeui_ai_agent::AgentSendError> {
            Ok(())
        }
...
fn success_factory() -> AgentFactory {
    ...
    Arc::new(|opts: BuildTaskOptions| {
        async move {
            Ok(tjuaeui_ai_agent::AgentInstance::Mock(Arc::new(
...
fn confirmations_factory(count: usize) -> AgentFactory {
    use futures_util::FutureExt;
    use tjuaeui_common::Confirmation;
    Arc::new(move |opts: BuildTaskOptions| {
...
struct TestTeamAssistantCatalog {
    rows: Vec<TeamAssistantCatalogEntry>,
}
...
fn assistant_catalog_entry(
    ...
    TeamAssistantCatalogEntry {
        ...
        recommended_prompts_i18n: Default::default(),
    }
}

fn assistant_catalog(rows: Vec<TeamAssistantCatalogEntry>) -> Arc<dyn TeamAssistantCatalogPort> {
    Arc::new(TestTeamAssistantCatalog { rows })
}

fn single_assistant_catalog(entry: TeamAssistantCatalogEntry) -> Arc<dyn TeamAssistantCatalogPort> {
    assistant_catalog(vec![entry])
}
...
impl IProviderRepository for EmptyProviderRepo {
    async fn list(&self) -> Result<Vec<tjuaeui_db::models::Provider>, DbError> {
        Ok(vec![])
    }
    async fn find_by_id(&self, _id: &str) -> Result<Option<tjuaeui_db::models::Provider>, DbError> {
        Ok(None)
    }
    async fn create(
        &self,
        _params: tjuaeui_db::CreateProviderParams<'_>,
    ) -> Result<tjuaeui_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
    async fn update(
        &self,
        _id: &str,
        _params: tjuaeui_db::UpdateProviderParams<'_>,
    ) -> Result<tjuaeui_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
...
fn setup_with_factory_metadata_team_repo_and_conversation_repo(
    ...
    let backend_binary_path = Arc::new(std::path::PathBuf::from("/tmp/tjuaecore-test"));
...
async fn recovery_creates_system_run_intents_without_restoring_old_memory_run() {
    ...
    team_repo
        .write_message(&tjuaeui_db::models::MailboxMessageRow {
...
async fn teammate_first_wake_uses_canonical_prompt_at_service_boundary() {
    ...
    let created = svc
        .create_team(
            ...
            CreateTeamRequest {
                ...
                agents: tjuae_cli_two_agent_input(),
                workspace: None,
            },
        )
        .await
    ...
    assert!(first_message.contains("## 团队治理"));
    assert!(first_message.contains("所有团队协调都必须使用 `team_*` MCP 工具"));
    assert!(first_message.contains("用 `team_send_message` 向负责人的 `slot_id` 汇报结果"));
    assert!(first_message.contains("停止生成并结束回合"));
...
fn word_creator_assistant() -> TeamAssistantCatalogEntry {
    let mut assistant = assistant_catalog_entry("word-creator", "Word Creator", "claude", Vec::new());
    assistant.description = "Drafts Word documents".into();
    assistant
}
...
fn tjuae_cli_two_agent_input() -> Vec<TeamAgentInput> {
    two_agent_input()
        .into_iter()
        .map(|mut agent| {
            agent.backend = Some("tjuaecli".into());
            agent
        })
        .collect()
}
...
async fn renew_active_lease_allows_empty_team_without_unrelated_lease() {
    ...
    team_repo
        .create_team(&tjuaeui_db::models::TeamRow {
            id: "team-empty".into(),
            user_id: "user1".into(),
            name: "Empty".into(),
            ...
            created_at: tjuaeui_common::now_ms(),
            updated_at: tjuaeui_common::now_ms(),
            project_id: None,
            folder_id: None,
        })
        .await
...
async fn create_team_with_workspace_writes_same_workspace_to_team_and_initial_agents() {
    ...
    let workspace_dir =
        std::env::temp_dir().join(format!("tjuaeui-team-user-workspace-{}", tjuaeui_common::generate_id()));
...
async fn create_team_side_branch_backfills_project_binding_when_injected() {
    ...
    let store: Arc<dyn tjuaeui_db::IProjectStore> = Arc::new(tjuaeui_db::SqliteProjectStore::new(db.pool().clone()));
    std::mem::forget(db);
...
async fn create_team_without_workspace_uses_leader_auto_workspace_for_all_initial_agents() {
    ...
    let got = svc.get_team("user1", &created.id).await.unwrap();
    assert!(!got.workspace.trim().is_empty(), "teams.workspace 必须设置");
    assert!(
        got.workspace.replace('\\', "/").contains("/conversations/acp-temp-"),
        "意外的自动工作区：{}",
        got.workspace
    );
...
async fn tc_create_team_prefers_assistant_avatar_over_backend_logo() {
    ...
    let mut assistant = assistant_catalog_entry("assistant-lead", "Lead Assistant", "claude", Vec::new());
    assistant.avatar = Some("/api/assistant-assets/assistant-lead/avatar".into());
    let (svc, _team_repo, _task_manager, _conv_repo) = setup_with_factory_metadata_assistants_and_conversation_repo(
        success_factory(),
        agent_metadata_repo,
        single_assistant_catalog(assistant),
    );
    ...
    assert_eq!(
        ...
        Some("/api/assistant-assets/assistant-lead/avatar")
    );
}
...
async fn tc_create_team_carries_assistant_identity_into_lead_conversation_extra() {
    ...
    let (svc, _team_repo, _task_manager, conv_repo) = setup_with_factory_metadata_assistants_and_conversation_repo(
        success_factory(),
        agent_metadata_repo,
        single_assistant_catalog(assistant_catalog_entry(
            "assistant-lead",
            "Lead Assistant",
            "claude",
            Vec::new(),
...
async fn team_preset_assistant_snapshot_is_frozen() {
    let (svc, _team_repo, conversation_ports, conv_repo) = setup_with_ports_metadata_assistants_and_conversation_repo(
        ...
        single_assistant_catalog(word_creator_assistant()),
    );
...
async fn tg3_get_team_rejects_cross_user_access() {
    ...
    assert!(matches!(result, Err(tjuaeui_team::TeamError::Forbidden(_))));
}
...
async fn aa_add_agent_inherits_team_workspace() {
    ...
    let (svc, _, conv_repo) =
        setup_with_factory_and_metadata_and_conversation_repo(success_factory(), agent_metadata_repo);
    let workspace = std::env::temp_dir().join(format!("tjuaeui-team-workspace-{}", tjuaeui_common::generate_id()));
...
async fn add_agent_uses_team_temp_workspace_when_team_and_leader_workspaces_are_unusable() {
    ...
    conv_repo
        .patch_extra(
            ...
            serde_json::json!({ "workspace": "/tmp/tjuaeui-team-missing-leader-workspace" }),
        )
        .unwrap();
...
async fn d31b2_ensure_session_broadcasts_failed_loading_team_for_missing_team() {
    ...
    assert!(matches!(err, tjuaeui_team::TeamError::TeamNotFound(_)));
