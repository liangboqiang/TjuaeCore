...
use tjuaeui_team::events::TeamEventEmitter;
use tjuaeui_team::message_projection::{
    ProjectedTeamMessage, TeamMessageProjection, TeamProjectionMessageStore, TeamProjectionRequest,
    TeamProjectionSource,
};
use tjuaeui_team::prompts::{AvailableAssistant, build_lead_prompt, build_teammate_prompt, build_wake_payload};
use tjuaeui_team::types::{
    MailboxMessage, MailboxMessageType, TaskStatus, TeamAgent, TeamTask, TeammateRole, TeammateStatus,
};
use tjuaeui_team::visibility::TeamVisibilityPolicy;
use tjuaeui_team::{Mailbox, TaskBoard, TeammateManager};
...
fn lp2_lead_prompt_contains_tool_descriptions() {
    ...
    // TjuaeUI lead prompt references the `team_*` coordination tools that the
...
fn lp3_lead_prompt_contains_task_management_guidance() {
    ...
    assert!(prompt.contains("使用 `team_task_create` 拆分任务"), "缺少任务拆分指引");
    assert!(prompt.contains("分配任务"), "缺少任务分配指引");
    assert!(prompt.contains("依赖"), "缺少依赖关系指引");
    assert!(prompt.contains("队员汇报后检查结果"), "缺少队员结果检查指引");
}
...
fn tp1_teammate_prompt_contains_execution_guidance() {
    ...
    assert!(prompt.contains("## 团队治理"), "缺少团队治理规则");
    assert!(
        prompt.contains("所有团队协调都必须使用 `team_*` MCP 工具"),
        "缺少规范协调规则"
    );
    assert!(prompt.contains("## 工作方式"), "缺少执行指引");
    assert!(prompt.contains("team_send_message"), "缺少沟通工具");
    assert!(prompt.contains("team_task_update"), "缺少任务更新工具");
    assert!(prompt.contains("shutdown_request"), "缺少下线协议");
    assert!(prompt.contains("shutdown_approved"), "缺少 shutdown_approved");
    assert!(prompt.contains("停止生成并结束回合"), "缺少停止协议");
    assert!(prompt.contains("槽位 ID：w1"), "缺少队员槽位 ID");
    assert!(!prompt.contains("Teammates:"), "不得注入静态队员列表");
}
...
fn tp2_teammate_prompt_contains_team_name() {
    ...
    assert!(prompt.contains("团队：Project Falcon"));
}
...
fn wp1_wake_payload_includes_unread_messages() {
    ...
    assert!(payload.contains("摘要：Finished task Y"));
}
