...
所有端点前缀 `/api/teams`，全部需要 JWT。响应统一 `ApiResponse<T>` / `ErrorResponse`。DTO 定义在 `crates/tjuaeui-api-types/src/team.rs`。
...
- `backend`：`acp / claude / gemini / qwen / nanobot / tjuae_cli / remote / openclaw-gateway`
...
走 `tjuaeui-conversation` 模块的单聊端点，跟普通单聊**完全一致**：
