...
mod tests {
    ...
    fn team_run_event_has_correct_shape() {
        ...
        emitter.broadcast_team_run(
            ...
            tjuaeui_api_types::TeamRunPayload {
                team_id: "team-1".into(),
                team_run_id: "run-1".into(),
                source: tjuaeui_api_types::TeamRunSource::UserMessage,
                has_user_intervention: true,
                target_slot_id: "lead-1".into(),
                target_role: tjuaeui_api_types::TeamRunTargetRole::Lead,
                status: tjuaeui_api_types::TeamRunStatus::Accepted,
        ...
        let payload: tjuaeui_api_types::TeamRunPayload = serde_json::from_value(events[0].data.clone()).unwrap();
        assert_eq!(payload.team_id, "team-1");
        assert_eq!(payload.team_run_id, "run-1");
        assert_eq!(payload.target_role, tjuaeui_api_types::TeamRunTargetRole::Lead);
        assert_eq!(payload.status, tjuaeui_api_types::TeamRunStatus::Accepted);
    ...
    fn child_turn_event_has_correct_shape() {
        ...
        emitter.broadcast_child_turn(
            ...
            tjuaeui_api_types::TeamChildTurnPayload {
                team_id: "team-1".into(),
                team_run_id: "run-1".into(),
                slot_id: "worker-1".into(),
                role: tjuaeui_api_types::TeamRunTargetRole::Teammate,
                conversation_id: "conv-1".into(),
                turn_id: "turn-1".into(),
                status: tjuaeui_api_types::TeamRunStatus::Running,
            },
        );
        ...
        let payload: tjuaeui_api_types::TeamChildTurnPayload = serde_json::from_value(events[0].data.clone()).unwrap();
        assert_eq!(payload.team_id, "team-1");
        assert_eq!(payload.team_run_id, "run-1");
        assert_eq!(payload.slot_id, "worker-1");
        assert_eq!(payload.status, tjuaeui_api_types::TeamRunStatus::Running);
    }

    #[test]
    fn slot_work_changed_event_has_correct_shape() {
        let (emitter, bc) = make_emitter();
        emitter.broadcast_slot_work(tjuaeui_api_types::TeamSlotWorkChangedPayload {
            team_id: "team-1".into(),
            slot_work: tjuaeui_api_types::TeamSlotWorkPayload {
                slot_id: "lead-1".into(),
                role: tjuaeui_api_types::TeamRunTargetRole::Lead,
                state: tjuaeui_api_types::TeamSlotWorkState::Idle,
        ...
        let payload: tjuaeui_api_types::TeamSlotWorkChangedPayload =
            serde_json::from_value(events[0].data.clone()).unwrap();
        assert_eq!(payload.team_id, "team-1");
        assert_eq!(payload.slot_work.slot_id, "lead-1");
        assert_eq!(payload.slot_work.state, tjuaeui_api_types::TeamSlotWorkState::Idle);
