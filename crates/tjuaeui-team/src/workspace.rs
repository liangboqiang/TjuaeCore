...
use tjuaeui_common::{WorkspacePathValidationError, validate_workspace_path_availability};
use tjuaeui_db::models::TeamRow;
use tjuaeui_db::{ITeamRepository, UpdateTeamParams};
use tracing::warn;
...
pub(crate) fn validate_create_workspace_path(workspace: &str) -> Result<String, TeamError> {
    validate_workspace_path_availability(workspace).map_err(|error| match error {
        WorkspacePathValidationError::Empty => TeamError::InvalidRequest("工作区目录为空".into()),
...
fn validate_runtime_workspace_path(workspace: &str) -> Result<String, TeamError> {
    validate_workspace_path_availability(workspace).map_err(|error| match error {
        WorkspacePathValidationError::Empty => TeamError::InvalidRequest("团队工作区为空".into()),
        WorkspacePathValidationError::DoesNotExist(path)
        | WorkspacePathValidationError::NotDirectory(path)
