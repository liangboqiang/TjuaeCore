...
use dashmap::DashMap;
use tjuaeui_ai_agent::{ActiveLeaseRegistry, AgentError, AgentInstance, IWorkerTaskManager, IdleCleanupCoordinator};
...
use tjuaeui_db::{
    IAgentMetadataRepository, IAssistantDefinitionRepository, IProviderRepository, ITeamRepository, UpdateTeamParams,
};
...
impl TeamSessionService {
    ...
    pub async fn create_team(&self, user_id: &str, req: CreateTeamRequest) -> Result<TeamResponse, TeamError> {
        if req.agents.is_empty() {
            return Err(TeamError::InvalidRequest("至少需要一个 Agent".into()));
        }
        if req
            .agents
            .iter()
    ...
    pub async fn add_agent(
        ...
        if let Some(session) = self.sessions.get(team_id).map(|e| Arc::clone(&e.session)) {
            ...
            let service = self
                .self_ref
                .upgrade()
                .ok_or_else(|| TeamError::InvalidRequest("add_agent 需要活动的 TeamSessionService".into()))?;
    ...
    pub async fn remove_agent(&self, user_id: &str, team_id: &str, slot_id: &str) -> Result<(), TeamError> {
        ...
        let (removed, session, removal_lease) = {
            ...
            if removed.role == crate::types::TeammateRole::Lead {
                return Err(TeamError::InvalidRequest("无法移除团队负责人".into()));
            }
                ...
                session
                    ...
                    .restore_attach_required_after_remove_persist_error(
                        ...
                        MemberRuntimeFailure {
                            ...
                            public_reason: "成员信息更新失败，Agent 运行时需要重启".to_owned(),
                        },
                    );
    ...
    async fn ensure_session_inner(&self, team_id: &str) -> Result<(), TeamError> {
        ...
        let service = self
            .self_ref
            .upgrade()
            .ok_or_else(|| TeamError::InvalidRequest("团队服务正在关闭".to_owned()))?;
        ...
        let Some(leader) = agents_snapshot
            ...
            let error = TeamError::InvalidRequest("团队没有负责人 Agent".to_owned());
            self.broadcast_session_status(
                team_id,
                TeamSessionStatus::Failed,
                Some(TeamSessionPhase::AttachingAgents),
    ...
    pub async fn get_run_state(&self, user_id: &str, team_id: &str) -> Result<TeamRunStateResponse, TeamError> {
        ...
        let active_run = session.team_run_manager().current_payload(&snapshot).filter(|run| {
            matches!(
                run.status,
                tjuaeui_api_types::TeamRunStatus::Accepted
                    | tjuaeui_api_types::TeamRunStatus::Running
                    | tjuaeui_api_types::TeamRunStatus::Cancelling
            )
        });
    ...
    pub async fn resolve_team_tool_context(
        ...
        let allowed_tools = tjuaeui_api_types::team_tool_descriptors_for_role(role_to_tool_role(agent.role))
            .into_iter()
...
async fn set_active_agent_session_mode(instance: &AgentInstance, mode: &str) -> Result<(), AgentError> {
    ...
    match instance {
        ...
        AgentInstance::TjuaeCli(manager) => manager.set_mode(mode).await,
...
mod tests {
    ...
    use tjuaeui_api_types::{AddAgentRequest, ConfigOptionConfirmation, SetConfigOptionResponse};
    use tjuaeui_common::{AgentKillReason, AgentType, ConversationStatus, TimestampMs, now_ms};
    use tjuaeui_db::{IConversationRepository, ITeamRepository};
    ...
    impl ModeSettingAgent {
        ...
        fn idle_pending_tjuae_cli(conversation_id: &str) -> Self {
            Self::accepts_mode(conversation_id)
    ...
    impl MutableTaskManager {
        ...
        fn insert_idle_pending_tjuae_cli_agent(&self, conversation_id: &str) {
            self.tasks.lock().unwrap().insert(
                conversation_id.to_owned(),
                AgentInstance::Mock(Arc::new(ModeSettingAgent::idle_pending_tjuae_cli(conversation_id))),
            );
        }
    ...
    fn two_agent_team_request(name: &str) -> tjuaeui_api_types::CreateTeamRequest {
        tjuaeui_api_types::CreateTeamRequest {
            name: name.into(),
            agents: vec![
                tjuaeui_api_types::TeamAgentInput {
                    name: "Lead".into(),
                    role: "lead".into(),
    ...
    fn team_with_tjuae_cli_worker_request(name: &str) -> tjuaeui_api_types::CreateTeamRequest {
        let mut request = two_agent_team_request(name);
        request.agents.push(tjuaeui_api_types::TeamAgentInput {
            name: "Butler".into(),
            role: "teammate".into(),
    ...
    #[test]
    fn idle_collectable_team_member_accepts_idle_pending_tjuae_cli_runtime() {
        let task = AgentInstance::Mock(Arc::new(ModeSettingAgent::idle_pending_tjuae_cli("tjuaecli-idle")));

        assert!(super::is_idle_collectable_team_member(&task, now_ms(), 300_000));
    }

    #[test]
    fn idle_collectable_team_member_rejects_running_tjuae_cli_runtime() {
        let task = AgentInstance::Mock(Arc::new(
            ModeSettingAgent::accepts_mode("tjuaecli-running")
                .with_agent_type(AgentType::TjuaeCli)
    ...
    #[tokio::test]
    async fn idle_cleanup_stops_team_session_when_tjuae_cli_member_is_idle_pending() {
        let task_manager = Arc::new(MutableTaskManager::new());
        let (svc, _repo, _task_manager, _conv_repo, _broadcaster) =
            setup_with_factory_metadata_team_repo_conversation_repo_broadcaster_and_task_manager(task_manager.clone());
        let created = svc
            .create_team("user-test", team_with_tjuae_cli_worker_request("Idle TjuaeCLI Cleanup"))
            .await
            .unwrap();
        let lead = created.assistants.iter().find(|agent| agent.role == "lead").unwrap();
        let acp_worker = created.assistants.iter().find(|agent| agent.name == "Worker").unwrap();
        let tjuae_cli_worker = created.assistants.iter().find(|agent| agent.name == "Butler").unwrap();
        task_manager.insert_idle_finished_agent(&lead.conversation_id);
        task_manager.insert_idle_finished_agent(&acp_worker.conversation_id);
        task_manager.insert_idle_pending_tjuae_cli_agent(&tjuae_cli_worker.conversation_id);
