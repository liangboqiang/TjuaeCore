...
use tjuaeui_ai_agent::ActiveLeaseRegistry;
...
impl From<TeamError> for ApiError {
    fn from(err: TeamError) -> Self {
        match err {
            ...
            TeamError::DuplicateAgentName(msg) => ApiError::BadRequest(format!("Agent 名称已被占用：{msg}")),
            TeamError::RuntimeNotReady { conversation_id } => ApiError::coded(
                StatusCode::CONFLICT,
                "TEAM_RUNTIME_NOT_READY",
                format!("对话的团队 Agent 运行时尚未就绪：{conversation_id}"),
...
mod tests {
    ...
    fn invalid_request_maps_missing_assistant_identity_to_coded_api_error() {
        let err: ApiError = TeamError::InvalidRequest("spawn_agent.assistant_id 为必填项".into()).into();
    ...
    fn invalid_request_maps_unknown_assistant_to_coded_api_error() {
        let err: ApiError = TeamError::InvalidRequest("找不到预设助手：bare:deadbeef".into()).into();
    ...
    fn invalid_request_maps_legacy_identity_field_to_coded_api_error() {
        let err: ApiError = TeamError::InvalidRequest("backend 不再接受；请使用 assistant_id".into()).into();
    ...
    fn member_runtime_failure_maps_to_sanitized_coded_conflict() {
        let err: ApiError = TeamError::MemberRuntimeFailed {
            ...
            public_reason: "Agent 运行时启动失败".into(),
        }
        .into();
        assert_eq!(err.status_code(), StatusCode::CONFLICT);
