...
pub(crate) fn parse_agent_type(backend: &str) -> Result<AgentType, TeamError> {
    ...
    Err(TeamError::InvalidRequest(format!("不支持的后端：{backend}")))
}
...
impl TeamSessionService {
    ...
    /// Returns a flat list of model IDs for use by internal agents (tjuae_cli).
...
mod tests {
    ...
    use std::sync::Arc;
    use tjuaeui_db::models::{AgentMetadataRow, Provider};
    use tjuaeui_db::{
        DbError, IAgentMetadataRepository, IProviderRepository, UpdateAgentHandshakeParams, UpsertAgentMetadataParams,
    };
    ...
    impl IProviderRepository for SingleProviderRepo {
        ...
        async fn create(&self, _params: tjuaeui_db::CreateProviderParams<'_>) -> Result<Provider, DbError> {
            Err(DbError::NotFound("not implemented".into()))
        }

        async fn update(&self, _id: &str, _params: tjuaeui_db::UpdateProviderParams<'_>) -> Result<Provider, DbError> {
            Err(DbError::NotFound("not implemented".into()))
        }
    ...
    async fn create_team_rejects_assistant_missing_from_team_selectable_catalog() {
        let svc = service_with_selectable_catalog(vec![]);
        ...
        let err = svc
            ...
            .await
            .expect_err("不可用于团队模式的助手必须被拒绝");

        assert!(
            matches!(&err, TeamError::InvalidRequest(msg) if msg.contains("不可用于团队模式")),
            "期望团队助手可用性错误，实际为 {err:?}"
        );
    }
    ...
    async fn add_agent_rejects_assistant_missing_from_team_selectable_catalog() {
        ...
        let err = svc
            .add_agent(
                "user1",
                &created.id,
                tjuaeui_api_types::AddAgentRequest {
                    name: "Worker".into(),
                    role: "teammate".into(),
                    backend: None,
    ...
    async fn resolve_spawn_backend_and_model_prefers_assistant_identity_over_caller_backend() {
        ...
        let svc = TeamSessionService::new(
            ...
            Arc::new(RowsTeamAssistantCatalog {
                rows: vec![team_assistant_entry("word-creator", "Word Creator", "tjuaecli")],
            }),
            Arc::new(SingleProviderRepo {
                rows: vec![provider_row("openai", &["gpt-5-mini"])],
            }),
