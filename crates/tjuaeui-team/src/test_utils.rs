...
use tjuaeui_db::models::{MailboxMessageRow, TeamRow, TeamTaskRow};
use tjuaeui_db::{DbError, ITeamRepository, UpdateTaskParams, UpdateTeamParams};
...
pub(crate) mod workspace_harness {
    ...
    impl ITeamRepository for FullMockTeamRepo {
        ...
        async fn write_message(&self, _row: &tjuaeui_db::models::MailboxMessageRow) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn update_task(&self, _task_id: &str, _params: &tjuaeui_db::UpdateTaskParams) -> Result<(), DbError> {
            Ok(())
        }
        ...
        fn new(repo: Arc<MockConversationRepo>) -> Self {
            Self {
                ...
                workspace_root: std::env::temp_dir().join(format!(
                    "tjuaeui-team-workspace-harness-{}",
    ...
    impl IWorkerTaskManager for NoopTaskManager {
        fn get_task(&self, _conversation_id: &str) -> Option<tjuaeui_ai_agent::AgentInstance> {
            None
        }
        ...
        async fn get_or_build_task(
            &self,
            _conversation_id: &str,
            _options: tjuaeui_ai_agent::types::BuildTaskOptions,
        ...
        fn collect_idle(&self, _idle_threshold_ms: tjuaeui_common::TimestampMs) -> Vec<String> {
            vec![]
        }
    }
    ...
    pub(crate) fn setup_with_factory_metadata_team_repo_conversation_repo_broadcaster_and_task_manager(
        ...
        let svc = TeamSessionService::new(
            ...
            Arc::new(std::path::PathBuf::from("/tmp/tjuaecore-test")),
        );
    ...
    pub(crate) fn single_agent_team_request(name: &str) -> CreateTeamRequest {
        CreateTeamRequest {
            name: name.into(),
            agents: vec![tjuaeui_api_types::TeamAgentInput {
                name: "Lead".into(),
                role: "lead".into(),
