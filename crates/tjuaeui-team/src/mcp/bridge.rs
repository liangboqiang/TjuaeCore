...
pub use tjuaeui_api_types::{TEAM_MCP_SERVER_NAME, TeamMcpStdioConfig};

/// Stdio MCP server description ready to be handed to `session/new`.
///
/// Field shapes:
/// - `name` = `"tjuaeui-team"` (fixed; team routing is done via `port` + `token`,
...
impl TeamMcpStdioServerSpec {
    ...
    ///
    /// `backend_binary_path` is the absolute path to the `tjuaecore`
...
mod tests {
    ...
    fn sample_cfg() -> TeamMcpStdioConfig {
        TeamMcpStdioConfig {
            ...
            token: "tok-abc".into(),
            slot_id: "slot-1".into(),
            binary_path: "/usr/bin/tjuaecore".into(),
        }
    }

    #[test]
    fn from_config_fills_all_fields() {
        let spec = TeamMcpStdioServerSpec::from_config("/usr/bin/tjuaecore", &sample_cfg());

        assert_eq!(spec.name, TEAM_MCP_SERVER_NAME);
        assert_eq!(spec.command, "/usr/bin/tjuaecore");
    ...
    fn into_sdk_serializes_as_stdio_variant() {
        let spec = TeamMcpStdioServerSpec::from_config("/bin/tjuaecore", &sample_cfg());
        ...
        assert_eq!(json["command"], "/bin/tjuaecore");
