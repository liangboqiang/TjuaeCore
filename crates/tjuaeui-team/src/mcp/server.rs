...
use tjuaeui_api_types::TeamSendMessageQueuedResponse;
...
impl TeamMcpServer {
    ...
    pub async fn start_with_prompt_dump(
        ...
        let listener = match TcpListener::bind("127.0.0.1:0").await {
            Ok(l) => l,
            Err(e) => {
                return Err(TeamError::InvalidRequest(format!("绑定 TCP 失败：{e}")));
            }
        };
        let addr = listener
            .local_addr()
            .map_err(|e| TeamError::InvalidRequest(format!("获取本地地址失败：{e}")))?;
        ...
        let http_listener = TcpListener::bind("127.0.0.1:0")
            .await
            .map_err(|e| TeamError::InvalidRequest(format!("绑定 HTTP 失败：{e}")))?;
        let http_addr = http_listener
            .local_addr()
            .map_err(|e| TeamError::InvalidRequest(format!("获取 HTTP 地址失败：{e}")))?;
...
async fn handle_method(
    ...
    match request.method.as_str() {
        ...
        _ => JsonRpcResponse::error(request.id, METHOD_NOT_FOUND, format!("未知方法：{}", request.method)),
    }
}
...
async fn handle_tools_list(
    ...
    let context = TeamToolContext {
        ...
        transport: tjuaeui_api_types::TeamToolTransport::Mcp,
    };
...
async fn exec_describe_assistant(args: &Value, service: &Weak<TeamSessionService>) -> Result<String, ToolCallError> {
    ...
    let assistant_id = args
        ...
        .ok_or_else(|| ToolCallError::from_message("缺少必填字段：assistant_id"))?;
...
async fn exec_send_message(
    ...
    let input: SendMessageInput =
        serde_json::from_value(args.clone()).map_err(|e| ToolCallError::from_message(format!("参数无效：{e}")))?;
    ...
    let service = service
        .upgrade()
        .ok_or_else(|| ToolCallError::from_message("团队服务不可用，无法唤醒目标"))?;
    ...
    serde_json::to_string(&response).map_err(|e| ToolCallError::from_message(format!("序列化错误：{e}")))
}
...
fn parse_task_list_filters(args: &Value) -> Result<TaskListFilters, ToolCallError> {
    ...
    let statuses = match input.status {
        ...
        Some(TaskListStatusInput::Many(statuses)) => {
            if statuses.is_empty() {
                return Err(ToolCallError::from_message("参数无效：status 不能为空"));
            }
            let parsed = statuses
                .iter()
    ...
    let limit = match input.limit {
        Some(value) if value <= 0 => {
            return Err(ToolCallError::from_message("参数无效：limit 必须大于 0"));
        }
...
fn parse_task_status_arg(status: &str) -> Result<TaskStatus, ToolCallError> {
    TaskStatus::parse(status)
        .ok_or_else(|| ToolCallError::from_message(format!("参数无效：不支持的任务状态“{status}”")))
}
...
async fn exec_spawn_agent(
    ...
    // keeps a clear user-visible lead-only message for the MCP client.
    if caller_role != TeammateRole::Lead {
        return Err(ToolCallError::from_message("只有团队负责人可以创建 Agent"));
    }
    if args.get("backend").is_some() {
        return Err(ToolCallError::from_message("backend 不再接受；请使用 assistant_id"));
    }
    if args.get("agent_type").is_some() {
        return Err(ToolCallError::from_message("agent_type 不再接受；请使用 assistant_id"));
    }
    if args.get("model").is_some() {
        return Err(ToolCallError::from_message(
            "model 不再接受；请使用助手配置或 UI 模型选择器",
        ));
    }
    ...
    let service = service
        .upgrade()
        .ok_or_else(|| ToolCallError::from_message("团队服务不可用，无法创建 Agent"))?;
...
async fn exec_task_list(args: &Value, scheduler: &TeammateManager) -> Result<String, ToolCallError> {
    ...
    serde_json::to_string_pretty(&output).map_err(|e| ToolCallError::from_message(format!("序列化错误：{e}")))
}
...
async fn exec_rename_agent(
    ...
    if let Some(svc) = service.upgrade() {
        let user_id = svc
            ...
            .await
            .ok_or_else(|| ToolCallError::from_message(format!("团队 {team_id} 没有活动会话")))?;
...
async fn exec_shutdown_agent(
    ...
    if caller_role != TeammateRole::Lead {
        return Err(ToolCallError::from_message("只有团队负责人可以关闭 Agent"));
    }
    ...
    let service = service
        .upgrade()
        .ok_or_else(|| ToolCallError::from_message("团队服务不可用，无法唤醒待关闭目标"))?;
...
mod tests {
    use super::*;
    use tjuaeui_api_types::{TeamRunTargetRole, TeamSlotWorkPayload, TeamSlotWorkState};
    ...
    /// Non-lead callers are rejected at the dispatch layer. Service weak is
    /// never upgraded because the early role gate short-circuits.
    #[tokio::test]
    async fn exec_spawn_agent_rejects_non_lead() {
        ...
        assert!(
            err.message.contains("只有团队负责人"),
            "error must explain the lead-only restriction, got {err:?}"
        );
    }
    ...
    async fn exec_spawn_agent_rejects_malformed_args() {
        ...
        assert!(
            err.message.contains("参数无效"),
            "must surface a localized invalid-parameter error for JSON deserialize failure, got {err:?}"
        );
    }
    ...
    async fn exec_spawn_agent_rejects_model_override() {
        ...
        assert!(
            err.message.contains("model 不再接受"),
    ...
    async fn exec_spawn_agent_rejects_legacy_backend_alias() {
        ...
        assert!(
            err.message.contains("backend 不再接受"),
    ...
    async fn exec_spawn_agent_rejects_legacy_agent_type_alias() {
        ...
        assert!(
            err.message.contains("agent_type 不再接受"),
    ...
    async fn exec_spawn_agent_requires_assistant_identity() {
        ...
        assert!(
            err.message.contains("缺少必填字段：assistant_id"),
