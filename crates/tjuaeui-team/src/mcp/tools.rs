...
pub use tjuaeui_api_types::{
    TEAM_DESCRIBE_ASSISTANT_DESCRIPTION, TEAM_LIST_ASSISTANTS_DESCRIPTION, TEAM_SPAWN_AGENT_DESCRIPTION,
};
use tjuaeui_api_types::{TeamToolPermission, TeamToolRole};

// ---------------------------------------------------------------------------
// Tool descriptors (returned by tools/list)
// ---------------------------------------------------------------------------

pub type ToolDescriptor = tjuaeui_api_types::TeamToolDescriptor;
...
pub fn all_tool_descriptors_for_role(caller_role: TeammateRole) -> Vec<ToolDescriptor> {
    ...
    tjuaeui_api_types::team_tool_descriptors_for_role(role)
}
...
pub fn authorize_tool(caller_role: TeammateRole, tool_name: &str) -> Result<(), String> {
    let Some(spec) = tjuaeui_api_types::team_tool_descriptor(tool_name) else {
        return Err(format!("未知工具：{tool_name}"));
    };
    if spec.permission == TeamToolPermission::LeadOnly && caller_role != TeammateRole::Lead {
        return Err(format!("只有团队负责人可以使用 {tool_name}"));
    }
    Ok(())
}
...
pub fn is_builtin_mcp_backend(backend: &str) -> bool {
    backend == tjuaeui_common::constants::TJUAE_CLI_RUNTIME_BACKEND
}
    ...
    match tool_name {
        "team_send_message" => {
            let input: SendMessageInput =
                serde_json::from_value(arguments.clone()).map_err(|e| format!("team_send_message 参数无效：{e}"))?;
        ...
        "team_spawn_agent" => Err("由服务器直接处理".into()),
        "team_task_create" => {
            let input: TaskCreateInput =
                serde_json::from_value(arguments.clone()).map_err(|e| format!("team_task_create 参数无效：{e}"))?;
        ...
        "team_task_update" => {
            let input: TaskUpdateInput =
                serde_json::from_value(arguments.clone()).map_err(|e| format!("team_task_update 参数无效：{e}"))?;
        ...
        | "team_describe_assistant" => Err("由服务器直接处理".into()),
...
mod tests {
    ...
    #[test]
    fn team_spawn_agent_description_is_tjuaeui_original() {
        ...
        assert!(
            desc.contains("调用本工具前必须"),
            "description must include the complete TjuaeUI staffing workflow"
        );
        assert!(
            desc.contains("明确批准"),
            "description must retain the approval precondition clause"
        );
    }
    ...
    fn parse_spawn_agent_is_handled_directly_by_server() {
        ...
        assert!(result.unwrap_err().contains("由服务器直接处理"));
    }
    ...
    fn builtin_mcp_backend_check() {
        assert!(is_builtin_mcp_backend("tjuaecli"));
    ...
    fn team_describe_assistant_descriptor_text_matches() {
        ...
        assert!(desc.description.starts_with("创建队员前获取助手详情"));
        assert!(desc.description.contains("确认匹配后，使用同一 assistant_id"));
    }
    ...
    fn team_list_assistants_descriptor_guides_real_assistant_ids() {
        ...
        assert!(
            desc.description.starts_with("列出可用于创建团队队员的助手。"),
            "unexpected descriptor text: {}",
            desc.description
        );
        assert!(desc.description.contains("真实助手目录"));
    }
    ...
    fn team_spawn_agent_schema_requires_assistant_id_only() {
        ...
        assert!(assistant_desc.starts_with("用于创建队员的助手 ID"));
    ...
    fn team_spawn_agent_description_uses_assistant_first_staffing_language() {
        let desc = all_tool_descriptors()
            ...
            .find(|d| d.name == "team_spawn_agent")
            .unwrap();
        assert!(desc.description.contains("推荐助手"));
        assert!(!desc.description.contains("推荐模型"));
        assert!(!desc.description.contains("模型参数"));
        assert!(!desc.description.contains("推荐助手或后端"));
    }
    ...
    fn team_describe_assistant_description_uses_assistant_only_wording() {
        ...
        let props = desc.input_schema["properties"].as_object().unwrap();
        let assistant_desc = props["assistant_id"]["description"].as_str().unwrap();
        assert!(desc.description.contains("获取助手详情"));
        assert!(!desc.description.contains("预设助手"));
        assert!(assistant_desc.starts_with("可用助手目录中的助手 ID"));
    }
}
