...
use tjuaeui_api_types::{AddAgentRequest, GetConfigOptionsResponse, TeamAgentInput, TeamToolTransport};
use tjuaeui_common::{AgentKillReason, AgentType, ProviderWithModel, generate_id};
use tjuaeui_db::models::{AgentMetadataRow, TeamRow};
...
impl TeamAgentProvisioner {
    fn normalized_role(input: &TeamAgentInput) -> Result<TeammateRole, TeamError> {
        TeammateRole::parse(input.role.trim())
            .ok_or_else(|| TeamError::InvalidRequest(format!("团队 Agent 角色无效：{}", input.role)))
    }
    ...
    pub(crate) async fn add_agent(
        ...
        let role = TeammateRole::parse(req.role.trim())
            .ok_or_else(|| TeamError::InvalidRequest(format!("团队 Agent 角色无效：{}", req.role)))?;
    ...
    pub(crate) async fn attach_agent_process(
        ...
        self.conversation_port
            ...
            .await
            .map_err(|e| TeamError::InvalidRequest(format!("预热重建后的 Agent {} 失败：{e}", agent.slot_id)))?;
        info!(
            team_id = %team_id,
    ...
    pub(crate) async fn update_session_mode_seed(&self, agent: &TeamAgent, mode: &str) -> Result<(), TeamError> {
        self.conversation_port
            ...
            .await
            .map_err(|e| TeamError::InvalidRequest(format!("持久化 {} 的 session_mode 失败：{e}", agent.slot_id)))?;
        self.conversation_port
            .save_acp_runtime_mode(&agent.conversation_id, mode)
            .await
            .map_err(|e| TeamError::InvalidRequest(format!("持久化 {} 的 ACP 运行时模式失败：{e}", agent.slot_id)))?;
        Ok(())
    }
    ...
    async fn create_team_conversation_for_agent(
        ...
        let provider_id = if agent_type == AgentType::TjuaeCli {
        ...
        let (top_level_model, extra) = if agent_type == AgentType::TjuaeCli {
            (
    ...
    fn build_team_extra(
        ...
        if agent_type != AgentType::TjuaeCli {
            extra["current_model_id"] = serde_json::Value::String(model.to_owned());
        }
        if let Some(assistant_id) = assistant_id {
...
mod tests {
    ...
    use tjuaeui_ai_agent::{AgentError, AgentInstance};
    ...
    fn test_mcp_config() -> TeamMcpStdioConfig {
        TeamMcpStdioConfig {
            ...
            token: "token".into(),
            slot_id: "slot-1".into(),
            binary_path: "/tmp/tjuaecore".into(),
        }
    }

    #[tokio::test]
    async fn team_tool_transport_prefers_mcp_for_builtin_tjuae_cli_backend() {
        let provisioner = test_provisioner(Arc::new(Mutex::new(Vec::new())));
        let mut agent = test_agent();
        agent.backend = "tjuaecli".into();
