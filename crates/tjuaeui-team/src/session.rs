...
use tjuaeui_ai_agent::IWorkerTaskManager;
...
use tjuaeui_common::{AgentKillReason, generate_id};
use tjuaeui_db::ITeamRepository;
...
pub struct WakeInput {
    ...
    /// as left bubbles (matches TjuaeUI `TeammateManager.wake()`). These are
...
pub struct TeamSession {
    ...
    /// agent's conversation as left bubbles (TjuaeUI parity: see
...
impl TeamSession {
    ...
    pub async fn start_with_prompt_dump(
        ...
        let auth_token = tjuaeui_common::generate_id();
    ...
    pub async fn on_agent_finish(&self, conversation_id: &str, is_error: bool) -> Result<Option<String>, TeamError> {
        ...
        let slot_id = {
            ...
            agents
                ...
                .map(|a| a.slot_id)
                .ok_or_else(|| {
                    TeamError::AgentNotFound(format!("找不到 conversation_id={conversation_id} 对应的 Agent"))
                })?
        };

        // The event loop's `finalize_turn` handles most cases, but
        // `on_agent_finish` remains callable for tjuae_cli resume and test scenarios.
    ...
    pub async fn send_message(
        ...
        let lead_slot_id = self
            ...
            .find_lead_slot_id()
            .await
            .ok_or_else(|| TeamError::AgentNotFound("团队中没有负责人 Agent".into()))?;
    ...
    async fn enqueue_user_message(
        ...
        let run = self
            ...
            .ok_or_else(|| TeamError::InvalidRequest("用户可见入队操作丢失了团队运行".into()))?;
    ...
    pub(crate) async fn shutdown_agent(
        ...
        if target.role == TeammateRole::Lead {
            return Err(TeamError::InvalidRequest("无法关闭团队负责人".into()));
        }
    ...
    pub async fn cancel_child_turn(
        ...
        let batch = snapshot
            .active_batch
            .ok_or_else(|| TeamError::InvalidRequest(format!("槽位没有活动批次：{slot_id}")))?;
    ...
    pub(crate) async fn add_manual_agent(&self, agent: &TeamAgent) -> Result<(), TeamError> {
        ...
        let lead_slot_id = self
            ...
            .find_lead_slot_id()
            .await
            .ok_or_else(|| TeamError::InvalidRequest("找不到团队负责人".into()))?;
        let welcome = self
            .mailbox
            .write(
                &self.team.id,
    ...
    pub async fn spawn_agent(&self, caller_slot_id: &str, req: SpawnAgentRequest) -> Result<TeamAgent, TeamError> {
        ...
        if requested_name.is_empty() {
            return Err(TeamError::InvalidRequest("spawn_agent.name 不能为空".into()));
        }
        ...
        let assistant_id = req
            ...
            .ok_or_else(|| TeamError::InvalidRequest("spawn_agent.assistant_id 为必填项".into()))?;

        let service = self
            .service
            .upgrade()
            .ok_or_else(|| TeamError::InvalidRequest("spawn_agent 需要活动的 TeamSessionService".into()))?;
...
pub(crate) async fn attach_member_runtime(
    ...
    if let Err(error) = attach_result {
        ...
        // cause (the timing regression was only diagnosable by timing forensics).
        ...
        match service.register_event_loop(&session, &agent.slot_id) {
            ...
            Err(error) => {
                ...
                let failure = MemberRuntimeFailure {
                    ...
                    public_reason: "无法注册 Agent 运行时事件循环".to_owned(),
                };
...
fn sanitize_member_runtime_failure(error: &TeamError) -> MemberRuntimeFailure {
    ...
    MemberRuntimeFailure {
        classification,
        public_reason: "Agent 运行时启动失败".to_owned(),
    }
}
...
mod tests {
    ...
    impl TeamProjectionMessageStore for NoopProjectionStore {
        ...
        async fn find_projected_message(
            ...
            _msg_id: &str,
            _msg_type: &str,
        ) -> Result<Option<tjuaeui_db::models::MessageRow>, TeamError> {
            Ok(None)
        }

        async fn insert_projected_message(&self, _row: &tjuaeui_db::models::MessageRow) -> Result<(), TeamError> {
            Ok(())
        }
    }
    ...
    fn backend_path() -> Arc<PathBuf> {
        Arc::new(PathBuf::from("/tmp/tjuaecore-test"))
    }
    ...
    async fn stdio_spec_uses_fixed_name_and_binary_path() {
        ...
        assert_eq!(spec.command, "/tmp/tjuaecore-test");
    ...
    async fn recovery_scan_creates_one_wake_per_slot_with_non_self_unread() {
        ...
        assert_eq!(payload.source, tjuaeui_api_types::TeamRunSource::SystemLifecycle);
    ...
    async fn message_to_runtime_starting_member_returns_blocked_runtime_starting() {
        ...
        assert_eq!(
            ...
            Some(tjuaeui_api_types::TeamSlotBlockedReason::RuntimeStarting)
        );
        session.stop();
    }
    ...
    fn assert_reached_db_step(err: TeamError) {
        match err {
            TeamError::InvalidRequest(msg) if msg.contains("活动的 TeamSessionService") => {}
            other => panic!("期望服务不可用错误，实际为 {other:?}"),
        }
    }
    ...
    async fn spawn_agent_requires_assistant_identity() {
        ...
        let err = session
            ...
            .await
            .expect_err("assistant_id 必须为必填项");
        assert!(
            matches!(&err, TeamError::InvalidRequest(msg) if msg.contains("assistant_id 为必填项")),
            "期望缺少 assistant_id 的无效请求错误，实际为 {err:?}"
        );
        session.stop();
    }
    ...
    async fn spawn_agent_rejects_empty_name() {
        ...
        let mut req = spawn_req(Some("word-creator"));
        req.name = "   ".into();
        let err = session.spawn_agent("lead-1", req).await.expect_err("空名称必须被拒绝");
        assert!(
            matches!(&err, TeamError::InvalidRequest(msg) if msg.contains("不能为空")),
            "期望空名称的无效请求错误，实际为 {err:?}"
        );
        session.stop();
    }
