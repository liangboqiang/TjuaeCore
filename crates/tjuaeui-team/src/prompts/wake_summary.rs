...
pub(super) fn render_task_board_summary(
    ...
    output.push_str("## 当前任务看板摘要\n\n");

    if tasks.is_empty() {
        output.push_str("任务看板中没有任务。\n\n");
        return output;
    }
    ...
    output.push_str(&format!("显示 {} / {} 个任务。\n", selected.len(), tasks.len()));
    output.push_str(&format!(
        "已隐藏：进行中={hidden_active}，已完成={hidden_completed}，已删除={hidden_deleted}。\n"
    ));
    if hidden_active + hidden_completed + hidden_deleted > 0 {
        output.push_str("可通过 `team_task_list` 查看更多任务；需要时请使用筛选条件。\n");
    }
    output.push('\n');
    output.push_str("| ID | 主题 | 状态 | 负责人 | 前置依赖 |\n");
    output.push_str("|---|---|---|---|---|\n");
...
fn render_blocked_by(blocked_by: &[String]) -> String {
    ...
    if blocked_by.len() > MAX_BLOCKED_BY_IDS {
        parts.push(format!("另有 {} 项", blocked_by.len() - MAX_BLOCKED_BY_IDS));
    }
    parts.join(", ")
}
...
mod tests {
    ...
    fn active_tasks_under_limit_all_render() {
        ...
        assert!(payload.contains("已隐藏：进行中=0，已完成=0，已删除=0。"));
    ...
    fn completed_tasks_are_limited_to_recent_rows() {
        ...
        assert!(payload.contains("已隐藏：进行中=0，已完成=2，已删除=0。"));
        assert!(payload.contains("可通过 `team_task_list` 查看更多任务；需要时请使用筛选条件。"));
    ...
    fn deleted_tasks_are_hidden_and_counted() {
        ...
        assert!(payload.contains("已隐藏：进行中=0，已完成=0，已删除=1。"));
    }
    ...
    fn active_tasks_over_limit_are_truncated_and_counted() {
        ...
        assert!(payload.contains("显示 40 / 45 个任务。"));
        assert!(payload.contains("已隐藏：进行中=5，已完成=0，已删除=0。"));
    ...
    fn teammate_summary_keeps_owned_and_related_active_tasks_only() {
        ...
        assert!(payload.contains("已隐藏：进行中=1，已完成=1，已删除=0。"));
    }
    ...
    fn hidden_counts_are_mutually_exclusive() {
        ...
        let payload = render_task_board_summary(&agent("worker-1"), &tasks, &default_roster());
        assert!(payload.contains("显示 40 / 44 个任务。"));
        assert!(payload.contains("已隐藏：进行中=2，已完成=1，已删除=1。"));
    }
    ...
    fn subject_and_blocked_by_values_are_truncated() {
        ...
        assert!(payload.contains("另有 1 项"));
    ...
    fn removed_owner_active_task_is_hidden_when_unrelated() {
        ...
        assert!(!payload.contains("worker-2（已移除）"));
        assert!(payload.contains("显示 1 / 2 个任务。"));
        assert!(payload.contains("已隐藏：进行中=1，已完成=0，已删除=0。"));
    }
    ...
    fn removed_owner_active_task_is_shown_when_it_blocks_visible_work() {
        ...
        assert!(payload.contains("| blockera… | Removed member blocker | in_progress | worker-2（已移除） | - |"));
    ...
    fn removed_owner_completed_and_deleted_tasks_are_hidden() {
        ...
        assert!(payload.contains("已隐藏：进行中=0，已完成=1，已删除=1。"));
    }
}
