...
pub use tjuaeui_team_prompts::AvailableAssistant;

use crate::types::{MailboxMessage, MailboxMessageType, TeamAgent, TeamTask};

fn to_prompt_role(role: crate::types::TeammateRole) -> tjuaeui_team_prompts::TeamPromptRole {
    match role {
        crate::types::TeammateRole::Lead => tjuaeui_team_prompts::TeamPromptRole::Lead,
        crate::types::TeammateRole::Teammate => tjuaeui_team_prompts::TeamPromptRole::Teammate,
    }
}

fn to_prompt_agent(agent: &TeamAgent) -> tjuaeui_team_prompts::TeamPromptAgent {
    tjuaeui_team_prompts::TeamPromptAgent {
...
///
/// Delegates to `tjuaeui-team-prompts`, the canonical Team role prompt crate.
...
pub fn build_lead_prompt_for_transport(
    ...
    let body = tjuaeui_team_prompts::build_lead_prompt(&tjuaeui_team_prompts::LeadPromptParams {
        agent: &prompt_agent,
        team_name,
...
pub fn build_teammate_prompt_for_transport(
    ...
    let leader = prompt_members
        .iter()
        .find(|candidate| candidate.role == tjuaeui_team_prompts::TeamPromptRole::Lead)
        .cloned()
        .unwrap_or_else(|| tjuaeui_team_prompts::TeamPromptAgent {
            slot_id: "lead".to_owned(),
            name: "负责人".to_owned(),
            role: tjuaeui_team_prompts::TeamPromptRole::Lead,
            backend: agent.backend.clone(),
            model: agent.model.clone(),
            status: None,
        });
    ...
    tjuaeui_team_prompts::build_teammate_prompt(&tjuaeui_team_prompts::TeammatePromptParams {
        agent: &prompt_agent,
        team_name,
        leader: &leader,
        teammates: &teammates,
    ...
    if !unread_messages.is_empty() {
        ...
        for msg in unread_messages {
            ...
            if let Some(ref summary) = msg.summary {
                payload.push_str(&format!("  摘要：{summary}\n"));
            }
        }
        payload.push('\n');
    } else {
...
mod tests {
    ...
    fn lead_prompt_contains_core_sections() {
        ...
        assert!(prompt.contains("## 工作流程"));
        assert!(prompt.contains("第一次团队回合必须调用 `team_members`"));
        assert!(prompt.contains("调用 `team_list_assistants`"));
        assert!(prompt.contains("使用 `team_spawn_agent` 前等待明确确认"));
        assert!(prompt.contains("提案后结束回合"));

        // Assistant selection, not model selection.
        assert!(prompt.contains("## 助手选择"));
        assert!(!prompt.contains("team_list_models"));
        assert!(prompt.contains("不要向 `team_spawn_agent` 传模型"));

        // Conversation Style — don't pitch proposals up-front
        assert!(prompt.contains("## 对话风格"));
        assert!(prompt.contains("自然友好地回复"));

        // Idle, sequencing, shutdown, important rules
        assert!(prompt.contains("## 队员空闲状态"));
        assert!(prompt.contains("## 有依赖的任务顺序"));
        assert!(prompt.contains("## 下线队员"));
        assert!(prompt.contains("team_shutdown_agent"));
        assert!(prompt.contains("## 重要规则"));
    ...
    fn teammate_prompt_contains_agent_identity() {
        ...
        assert!(prompt.contains("名称：Worker1"));
        assert!(prompt.contains("槽位 ID：w1"));
        assert!(prompt.contains("团队：Alpha"));
        assert!(prompt.contains("负责人：Lead"));
    ...
    fn teammate_prompt_contains_team_name() {
        ...
        assert!(prompt.contains("团队：Beta Team"));
    }
    ...
    fn wake_payload_with_idle_notification() {
        ...
        assert!(payload.contains("摘要：Finished feature X"));
    }
    ...
    fn wake_payload_with_tasks() {
        ...
        assert!(payload.contains("当前任务看板摘要"));
        assert!(payload.contains("显示 2 / 2 个任务。"));
    ...
    fn wake_payload_empty() {
        ...
        assert!(payload.contains("没有新消息"));
        assert!(payload.contains("任务看板中没有任务"));
