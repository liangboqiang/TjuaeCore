pub use tjuaeui_common::constants::TJUAE_CLI_RUNTIME_BACKEND;
use tjuaeui_common::constants::{is_team_capable, supports_team_cli_fallback, supports_team_mcp};
...
mod tests {
    ...
    fn builtin_backend_is_mcp_capable_regardless_of_capabilities() {
        assert!(is_team_capable_backend("tjuaecli", None));
        assert!(supports_team_mcp_backend("tjuaecli", None));
        assert!(supports_team_mcp_backend("tjuaecli", Some(&json!({}))));
        assert_eq!(TJUAE_CLI_RUNTIME_BACKEND, "tjuaecli");
    }
