...
use tjuaeui_api_types::TeamRunTargetRole;
...
pub(super) struct ActiveBatch {
    ...
    pub(super) started_at_ms: Option<tjuaeui_common::TimestampMs>,
}
...
impl SlotWorkCoordinator {
    ...
    pub(crate) fn commit_enqueue(
        ...
        let record = state
            ...
            .get(&lease.lease_id)
            .cloned()
            .ok_or_else(|| TeamError::InvalidRequest("入队租约已不再活动".into()))?;
        if record.lease != *lease || lease.session_generation != self.session_generation {
            return Err(TeamError::InvalidRequest("入队租约所有者已过期".into()));
        }
    ...
    fn validate_enqueue_constraint(slot_id: &str, slot: &SlotState) -> Result<(), TeamError> {
        if slot.removed {
            return Err(TeamError::InvalidRequest(format!("团队槽位已移除：{slot_id}")));
        }
        match slot.runtime_constraint {
