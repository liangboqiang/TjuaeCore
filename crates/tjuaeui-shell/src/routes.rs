...
use tjuaeui_system::ClientPrefService;
...
impl From<ShellError> for ApiError {
    fn from(err: ShellError) -> Self {
        match err {
            ShellError::FileNotFound(path) => ApiError::BadRequest(format!("找不到文件：{path}")),
            ShellError::DirectoryNotFound(path) => ApiError::BadRequest(format!("找不到目录：{path}")),
            ShellError::InvalidUrl(msg) => ApiError::BadRequest(format!("URL 无效：{msg}")),
            ShellError::ToolNotInstalled(tool) => ApiError::BadRequest(format!("尚未安装工具：{tool}")),
            ShellError::CommandFailed(msg) => ApiError::Internal(format!("命令执行失败：{msg}")),
            ShellError::Io(e) => ApiError::Internal(format!("IO 错误：{e}")),
        }
    }
}
...
async fn extract_stt_multipart(mut multipart: Multipart) -> Result<SttMultipartFields, ApiError> {
            ...
            "fileName" => {
                file_name = Some(
                    field
                        .text()
                        .await
                        .map_err(|e| ApiError::BadRequest(format!("读取 fileName 失败：{e}")))?,
                );
            }
            "mimeType" => {
                mime_type = Some(
                    field
                        .text()
                        .await
                        .map_err(|e| ApiError::BadRequest(format!("读取 mimeType 失败：{e}")))?,
                );
            }
            "languageHint" => {
                let text = field
                    .text()
                    .await
                    .map_err(|e| ApiError::BadRequest(format!("读取 languageHint 失败：{e}")))?;
                if !text.is_empty() {
    ...
    let file_name = file_name.ok_or_else(|| ApiError::BadRequest("缺少 'fileName' 字段".to_owned()))?;
    let mime_type = mime_type.ok_or_else(|| ApiError::BadRequest("缺少 'mimeType' 字段".to_owned()))?;
...
///
/// Key mismatch fix: the TjuaeUI frontend stores the config under
...
pub(crate) async fn load_stt_config(
...
) -> Result<SpeechToTextConfig, tjuaeui_system::SystemError> {
    ...
    Ok(prefs
        ...
        .unwrap_or(SpeechToTextConfig {
            enabled: false,
            provider: tjuaeui_api_types::SpeechToTextProvider::Openai,
            auto_send: None,
            openai: None,
            deepgram: None,
        }))
...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(tjuaeui_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = tjuaeui_system::ClientPrefService::new(repo);
    ...
    fn stt_disabled_maps_to_bad_request() {
        ...
        assert!(matches!(err, ApiError::BadRequest(msg) if msg.contains("未启用")));
    }
    ...
    fn stt_stream_unsupported_maps_to_bad_request() {
        ...
        assert!(matches!(err, ApiError::BadRequest(msg) if msg.contains("不支持")));
    }
