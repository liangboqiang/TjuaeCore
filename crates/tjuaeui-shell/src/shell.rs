...
impl ShellService {
    ...
    async fn try_linux_terminal(&self, path: &str) -> Result<(), ShellError> {
        ...
        Err(ShellError::ToolNotInstalled("终端模拟器".to_owned()))
    }
}
...
fn validate_url(url: &str) -> Result<(), ShellError> {
    ...
    if !ALLOWED_URL_SCHEMES.contains(&parsed.scheme()) {
        return Err(ShellError::InvalidUrl(format!("不允许使用 '{}' 协议", parsed.scheme())));
    }
    Ok(())
}
...
mod tests {
    ...
    fn validate_url_rejects_file_scheme() {
        ...
        assert!(matches!(result, Err(ShellError::InvalidUrl(msg)) if msg.contains("协议")));
    }
    ...
    fn linux_show_item_uses_filemanager1_when_gdbus_available() {
        let path = Path::new("/home/user/Downloads/TjuaeUI.deb");
        ...
        assert!(args.contains(&"['file:///home/user/Downloads/TjuaeUI.deb']".to_owned()));
    ...
    fn linux_show_item_percent_encodes_spaces_in_uri() {
        let path = Path::new("/home/user/My Downloads/TjuaeUI.deb");
        let (program, args) = linux_show_item_command(path, true);
        assert_eq!(program, "gdbus");
        assert!(
            args.contains(&"['file:///home/user/My%20Downloads/TjuaeUI.deb']".to_owned()),
