...
pub const TJUAE_TIMESTAMP_SEPARATOR: &str = "_tjuaeui_";
pub const TJUAE_FILES_MARKER: &str = "[[TJUAE_FILES]]";
...
pub const COOKIE_NAME: &str = "tjuaeui-session";
pub const COOKIE_MAX_AGE_DAYS: u32 = 30;
pub const CSRF_COOKIE_NAME: &str = "tjuaeui-csrf-token";
...
pub const TJUAE_CLI_RUNTIME_BACKEND: &str = "tjuaecli";
...
pub fn supports_team_mcp(backend: &str, agent_capabilities: Option<&serde_json::Value>) -> bool {
    if backend == TJUAE_CLI_RUNTIME_BACKEND {
        return true;
    }
...
mod tests {
    ...
    fn team_mcp_builtin_backend_does_not_require_capability_metadata() {
        assert!(supports_team_mcp("tjuaecli", None));
        assert!(supports_team_mcp("tjuaecli", Some(&json!({}))));
    }
