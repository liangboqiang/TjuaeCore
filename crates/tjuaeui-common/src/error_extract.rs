...
//! important error context as `tracing` events to stderr **without** including
//! it in the JSON-RPC error response or the result frame we receive. This
//! module turns the last few stderr lines into a single human-readable message,
//! but only if the content matches an allowlisted error keyword — stderr is not
//! a trusted source.
//!
...
//! backends. It lives in `tjuaeui-common` (below every consumer: `tjuaeui-session`,
//! `tjuaeui-conversation`, the agent-* crates all depend on common) so the
//! consuming backends gain no upward crate dependency. Ported verbatim from the
//! deleted `tjuaeui-ai-agent::manager::acp::stderr_error_extractor` (git
...
pub fn extract_error_message(stderr_tail: &str) -> Option<String> {
...
mod tests {
    ...
    /// Security regression: a stderr chunk that mixes a secret-bearing line with
    /// an allowlisted line must surface ONLY the allowlisted line, never the
    /// token. The non-allowlisted secret line (`ANTHROPIC_API_KEY=sk-...`) does
    /// not contain any keyword, so it can never be chosen — and the chosen line
    /// must not carry the secret either.
    #[test]
    fn never_surfaces_secret_from_non_allowlisted_line() {
        let stderr = "DEBUG bootstrap: loaded ANTHROPIC_API_KEY=sk-ant-0123456789abcdef secret token\n\
                      ERROR upstream: rate limit exceeded, retry later";
        let result = extract_error_message(stderr).expect("the rate-limit line must surface");
        assert!(
            result.contains("rate limit"),
            "expected the allowlisted line; got {result}"
        );
        assert!(
            !result.contains("sk-ant"),
            "secret token must never leak into the user message; got {result}"
        );
        assert!(
            !result.to_lowercase().contains("anthropic_api_key"),
            "env-var name carrying a secret must not leak; got {result}"
        );
    }
}
