...
use super::cli_helpers::{DETECT_TIMEOUT, is_cli_installed, run_cli};

const CLI_NAME: &str = "tjuae-cli";

/// MCP Agent adapter for TjuaeCli.
///
/// TjuaeCli stores MCP configuration in a TOML config file. The config path
/// is obtained via `tjuae-cli config path`.
///
/// # Config Format (TOML)
///
/// ```toml
...
/// ```
pub struct TjuaeCliAdapter;

#[async_trait::async_trait]
impl McpAgentAdapter for TjuaeCliAdapter {
...
/// Resolve the TOML config file path from TjuaeCLI.
async fn get_config_path() -> Result<String, McpError> {
    let (stdout, _stderr) = run_cli(CLI_NAME, &["config", "path"], DETECT_TIMEOUT).await?;
    let path = stdout.trim().to_owned();
    if path.is_empty() {
        return Err(McpError::AgentOperationFailed(
            "TjuaeCLI config path returned empty output".into(),
        ));
    }
    Ok(path)
}
