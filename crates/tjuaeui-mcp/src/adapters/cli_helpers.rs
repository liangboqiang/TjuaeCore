...
use tjuaeui_runtime::resolve_command_path;
...
///
/// Uses `tjuaeui_runtime::resolve_command_path` so the lookup respects
...
mod tests {
    ...
    async fn is_cli_installed_returns_false_for_missing_binary() {
        let result = is_cli_installed("tjuaeui-definitely-not-a-real-binary-xyz")
            .await
            .unwrap();
        assert!(!result);
    }
