...
use tjuaeui_api_types::{OAuthLoginResponse, OAuthStatusResponse};
use tjuaeui_common::{TimestampMs, now_ms};
use tjuaeui_db::{IOAuthTokenRepository, UpsertOAuthTokenParams};
...
const DEFAULT_CLIENT_ID: &str = "tjuaeui";
...
mod tests {
    ...
    impl IOAuthTokenRepository for MockTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<tjuaeui_db::models::OAuthTokenRow>, tjuaeui_db::DbError> {
            Ok(None)
        }
        ...
        ) -> Result<tjuaeui_db::models::OAuthTokenRow, tjuaeui_db::DbError> {
            unimplemented!()
        }

        async fn delete(&self, _: &str) -> Result<(), tjuaeui_db::DbError> {
            Ok(())
        }

        async fn list_authenticated_urls(&self) -> Result<Vec<String>, tjuaeui_db::DbError> {
            Ok(vec![])
        }
    }
    ...
    impl IOAuthTokenRepository for IdempotentDeleteRepo {
        ...
        async fn delete(&self, url: &str) -> Result<(), tjuaeui_db::DbError> {
            Err(tjuaeui_db::DbError::NotFound(format!(
    ...
    impl IOAuthTokenRepository for ValidTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<tjuaeui_db::models::OAuthTokenRow>, tjuaeui_db::DbError> {
            Ok(Some(tjuaeui_db::models::OAuthTokenRow {
