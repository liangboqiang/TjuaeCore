...
use tjuaeui_api_types::{McpAuthMethod, McpConnectionTestErrorCode, McpConnectionTestResult, McpToolResponse};
...
const CLIENT_NAME: &str = "tjuaeui-mcp-test";
...
pub(super) fn timeout_result(duration: Duration) -> McpConnectionTestResult {
    error_result(
        ...
        format!("连接测试在 {} 秒后超时", duration.as_secs()),
...
fn command_not_found_message(command: &str) -> String {
    match missing_command_runtime(command) {
        "node" => format!(
            "找不到命令：{command}。TjuaeCore 通常会自动提供托管的 Node 运行时；请重试连接测试，若仍失败请检查后端日志，或为此 MCP 服务器配置绝对命令路径。"
        ),
        "bun" => format!(
            "找不到命令：{command}。请安装 Bun（包含 bun/bunx），然后重启 TjuaeUI，或为此 MCP 服务器配置绝对命令路径。"
        ),
        "uv" => format!("找不到命令：{command}。请安装 uv，然后重启 TjuaeUI，或为此 MCP 服务器配置绝对命令路径。"),
        "python" => {
            format!("找不到命令：{command}。请安装 Python，然后重启 TjuaeUI，或为此 MCP 服务器配置绝对命令路径。")
        }
        "deno" => format!("找不到命令：{command}。请安装 Deno，然后重启 TjuaeUI，或为此 MCP 服务器配置绝对命令路径。"),
        _ => format!("找不到命令：{command}。请安装该命令，或为此 MCP 服务器配置绝对命令路径。"),
    }
}
...
pub(super) fn rpc_error_result(method: &str, err: &JsonRpcError) -> McpConnectionTestResult {
    error_result(
        ...
        format!("{method} 出错：{}（代码 {}）", err.message, err.code),
        Some(serde_json::json!({
            "method": method,
            "rpc_code": err.code,
        })),
...
mod tests {
    ...
    fn timeout_result_message() {
        ...
        assert!(!result.success);
        assert!(result.error.as_deref().unwrap().contains("30 秒"));
    ...
    fn spawn_error_not_found() {
        ...
        assert!(error.contains("找不到命令：npx"));
        assert!(error.contains("托管的 Node 运行时"));
        assert!(error.contains("绝对命令路径"));
    ...
    fn spawn_error_not_found_generic_command() {
        ...
        let error = result.error.as_deref().unwrap();
        assert!(error.contains("找不到命令：missing-mcp"));
        assert!(error.contains("请安装该命令"));
        assert!(error.contains("绝对命令路径"));
    ...
    fn spawn_error_not_found_bun_command() {
        ...
        let error = result.error.as_deref().unwrap();
        assert!(error.contains("找不到命令：bunx"));
        assert!(error.contains("安装 Bun"));
    ...
    fn spawn_error_not_found_uv_command() {
        ...
        let error = result.error.as_deref().unwrap();
        assert!(error.contains("找不到命令：uvx"));
        assert!(error.contains("安装 uv"));
    ...
    fn spawn_error_not_found_python_command() {
        ...
        let error = result.error.as_deref().unwrap();
        assert!(error.contains("找不到命令：python3"));
        assert!(error.contains("安装 Python"));
    ...
    fn spawn_error_not_found_deno_command() {
        ...
        let error = result.error.as_deref().unwrap();
        assert!(error.contains("找不到命令：deno"));
        assert!(error.contains("安装 Deno"));
    ...
    fn spawn_error_permission_denied() {
        ...
        assert!(result.error.as_deref().unwrap().contains("没有执行权限"));
    ...
    fn spawn_error_other() {
        ...
        assert!(result.error.as_deref().unwrap().contains("无法启动"));
