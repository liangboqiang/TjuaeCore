...
mod tests {
    ...
    use std::sync::Mutex;
    use tjuaeui_api_types::{ImportMcpServerRequest, McpTransport};
    use tjuaeui_common::{McpServerStatus, TimestampMs};
    use tjuaeui_db::models::McpServerRow;
    use tjuaeui_db::{CreateMcpServerParams, DbError, UpdateMcpServerParams};
    ...
    async fn add_server_normalizes_shell_style_stdio_command() {
        ...
        let created = svc
            .add_server(CreateMcpServerRequest {
                ...
                description: None,
                transport: McpTransport::Stdio {
                    command: "npx @example/monitoring-mcp@latest --organization-slug=demo".into(),
                    args: vec![],
                    env: HashMap::new(),
                },
                original_json: None,
        ...
        match created.transport {
            McpTransport::Stdio { command, args, .. } => {
                ...
                assert_eq!(args, vec!["@example/monitoring-mcp@latest", "--organization-slug=demo"]);
            }
    ...
    async fn persist_test_result_records_success_status_and_tools() {
        ...
        let result = McpConnectionTestResult {
            success: true,
            tools: Some(vec![tjuaeui_api_types::McpToolResponse {
                name: "read_file".into(),
        ...
        assert_eq!(updated.last_test_status, tjuaeui_common::McpServerStatus::Connected);
    ...
    async fn persist_test_result_records_error_and_clears_tools() {
        ...
        let updated = svc.get_server(&created.id).await.unwrap();
        assert_eq!(updated.last_test_status, tjuaeui_common::McpServerStatus::Error);
