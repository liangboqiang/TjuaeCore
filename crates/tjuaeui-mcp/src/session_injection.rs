...
pub fn build_builtin_image_gen_server(
    ...
    Some(AcpSessionMcpServer::Stdio {
        name: "tjuaeui-image-generation".into(),
...
fn build_image_gen_env(config: &ImageGenConfig) -> Vec<NameValuePair> {
    let entries: [(&str, &Option<String>); 6] = [
        ("TJUAE_IMG_API_KEY", &config.api_key),
        ("TJUAE_IMG_API_URL", &config.api_url),
        ("TJUAE_IMG_MODEL", &config.model),
        ("TJUAE_IMG_QUALITY", &config.quality),
        ("TJUAE_IMG_SIZE", &config.size),
        ("TJUAE_IMG_STYLE", &config.style),
    ];
    ...
    fn builtin_image_gen_with_full_config() {
        ...
        match result.unwrap() {
            AcpSessionMcpServer::Stdio {
                ...
                assert_eq!(env.len(), 6);
                assert_eq!(env[0].name, "TJUAE_IMG_API_KEY");
    ...
    fn builtin_image_gen_with_partial_config() {
        ...
        match result.unwrap() {
            AcpSessionMcpServer::Stdio { env, .. } => {
                ...
                assert_eq!(env[0].name, "TJUAE_IMG_API_URL");
                assert_eq!(env[0].value, "https://api.openai.com");
                assert_eq!(env[1].name, "TJUAE_IMG_MODEL");
