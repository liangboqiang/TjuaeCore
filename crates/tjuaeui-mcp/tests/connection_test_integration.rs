...
use tjuaeui_mcp::McpServerTransport;
...
async fn stdio_nonexistent_command_returns_not_found_error() {
    ...
    assert!(error.contains("找不到命令"), "预期找不到命令错误，实际为：{error}");
...
async fn stdio_timeout_returns_timeout_error() {
    ...
    #[cfg(not(windows))]
    let (command, args) = ("sleep".to_owned(), vec!["60".into()]);
    let transport = McpServerTransport::Stdio {
        command,
        args,
        env: HashMap::new(),
    ...
    assert!(!result.success);
    let error = result.error.as_deref().unwrap();
    assert!(error.contains("超时"), "预期超时错误，实际为：{error}");
}
...
async fn stdio_with_args_spawns_correctly() {
    ...
    assert!(!result.success);
    let error = result.error.as_deref().unwrap();
    assert!(!error.contains("找不到命令"), "当前测试程序应当存在");
}
