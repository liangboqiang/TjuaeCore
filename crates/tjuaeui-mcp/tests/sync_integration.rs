...
use tjuaeui_db::SqliteMcpServerRepository;
use tjuaeui_mcp::{DetectedServer, McpAgentAdapter, McpError, McpServerTransport, McpSyncService};
...
async fn make_service(adapters: Vec<Arc<dyn McpAgentAdapter>>) -> McpSyncService {
    ...
    let repo: Arc<dyn tjuaeui_db::IMcpServerRepository> = Arc::new(SqliteMcpServerRepository::new(db.pool().clone()));
