//! `tjuaeui-process` — self-contained subprocess mechanism (feature 001).
//!
...
//! existing `CliAgentProcess` / process registry in `tjuaeui-ai-agent`.
//!
//! "Bytes not semantics": it never parses agent output, holds no session
//! state, and never mutates `std::env`. It depends only on `tjuaeui-common`
//! and `tjuaeui-runtime`.
//!
//! ## Isolation contract (why two mechanisms coexist without conflict)
//! All shared resources are namespaced under `{data_dir}/runtime/tjuaeui-process/`
