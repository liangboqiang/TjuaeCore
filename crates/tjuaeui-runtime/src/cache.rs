...
///
/// Lifecycle: written once by `tjuaeui-app`'s `main()` before
...
pub fn init(data_dir: impl AsRef<Path>) {
        ...
        && existing != path
    {
        tracing::warn!(
            ...
            "tjuaeui_runtime::init called twice with different paths; keeping first"
        );
    }
}

/// Returns the root cache directory used for all tjuaeui runtime artifacts.
///
/// Priority:
/// 1. Path supplied via [`init`] (`{data_dir}/runtime`) when the backend
///    started with `--data-dir`.
/// 2. Platform cache dir (via `dirs::cache_dir()`):
///    - macOS:   `~/Library/Caches/tjuaeui/runtime`
///    - Linux:   `$XDG_CACHE_HOME/tjuaeui/runtime` (fallback `~/.cache/tjuaeui/runtime`)
///    - Windows: `%LOCALAPPDATA%\tjuaeui\runtime`
///
...
pub fn runtime_root() -> Option<PathBuf> {
    ...
    dirs::cache_dir().map(|d| d.join("tjuaeui").join("runtime"))
}
...
mod tests {
    ...
    fn runtime_root_ends_with_expected_suffix() {
        ...
        let tail: Vec<_> = root
            ...
            .collect();
        assert_eq!(tail, vec!["runtime".to_string(), "tjuaeui".to_string()]);
    }
    ...
    fn node_runtime_root_appends_node_directory() {
        ...
        assert_eq!(
            tail,
            vec!["node".to_string(), "runtime".to_string(), "tjuaeui".to_string()]
        );
    }
}
