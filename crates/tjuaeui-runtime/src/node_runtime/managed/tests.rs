...
fn classify_error_detects_bundled_node_runtime_missing() {
    let err = NodeRuntimeError::managed_invalid(
        "bundled Node runtime missing under C:\\Program Files\\TjuaeUI\\resources\\bundled-tjuaecore\\win32-x64\\managed-resources\\node\\node-v24.11.0-win-x64",
    );
...
async fn bundled_runtime_missing_reports_bundled_resource_missing() {
    ...
    if !crate::test_support::run_in_env_child(
        ...
        |command| {
            command.env("TJUAE_BUNDLED_MANAGED_RESOURCES", &bundled_root);
        },
    ) {
        return;
    }
...
fn managed_runtime_install_lock_path_uses_runtime_root() {
    let root = PathBuf::from("/tmp/tjuaeui/runtime/node");
...
fn managed_runtime_injects_npm_state_under_runtime_root() {
    ...
    #[cfg(windows)]
    {
        std::fs::write(bin.join("node.exe"), b"").unwrap();
        std::fs::write(bin.join("npm.cmd"), b"").unwrap();
...
async fn bundled_runtime_validation_failure_does_not_fallback_to_remote_download() {
    ...
    let bundled_root = std::path::PathBuf::from(std::env::var_os("TJUAE_BUNDLED_MANAGED_RESOURCES").unwrap());
...
fn windows_managed_cli_paths_use_node_modules_under_archive_root() {
    let root = PathBuf::from(r"C:\TjuaeUI\node-v24.11.0-win-x64");
...
fn unix_managed_cli_paths_use_lib_node_modules() {
    let root = PathBuf::from("/opt/tjuaeui/node-v24.11.0-darwin-arm64");
