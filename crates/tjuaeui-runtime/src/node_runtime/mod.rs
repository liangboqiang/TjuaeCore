...
mod tests {
    ...
    fn fake_managed_runtime(root: &std::path::Path) -> ResolvedNodeRuntime {
        ...
        #[cfg(windows)]
        {
            ...
            let node_source = crate::resolve_command_path("node").expect("test environment must provide node");
            let node_path = root.join("node.exe");
            fs::copy(node_source, &node_path).expect("copy node executable");
            let npm_path = root.join("npm.cmd");
            let npx_path = root.join("npx.cmd");
            write_executable(&npm_path, "@echo off\r\necho 10.0.0\r\n");
            write_executable(&npx_path, "@echo off\r\necho 10.0.0\r\n");
    ...
    async fn ensure_explicit_path_requires_existing_file() {
        let missing = PathBuf::from("/tmp/tjuaeui-missing-node-runtime-command");
