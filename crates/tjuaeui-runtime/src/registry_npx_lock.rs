...
pub enum RegistryNpxLockError {
    #[error("内置 ACP Registry npx 锁文件无效：{0}")]
    InvalidLock(String),
    #[error("内置 npx 智能体“{backend}”没有固定到发布包")]
    MissingBackend { backend: String },
    #[error("内置 npx 智能体“{backend}”的参数不包含锁定包“{package}”")]
...
/// arguments with the exact version validated for this TjuaeCore release.
