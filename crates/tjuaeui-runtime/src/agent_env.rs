...
mod tests {
    ...
    const CHILD_MARKER: &str = "TJUAE_RUNTIME_AGENT_ENV_TEST_CHILD";
...
"#,
            ...
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("TJUAE_CURRENT_ONLY", "from-current")
                .env("TJUAE_OVERLAY", "from-current")
        ...
        assert_eq!(value("TJUAE_CURRENT_ONLY").as_deref(), Some("from-current"));
        assert_eq!(value("TJUAE_SHELL_ONLY").as_deref(), Some("from-shell"));
        assert_eq!(value("TJUAE_OVERLAY").as_deref(), Some("from-shell"));
