...
use tjuaeui_common::{generate_prefixed_id, now_ms};
use tjuaeui_db::{
...
impl AssistantService {
    ...
    ///   override when `TJUAE_BUILTIN_ASSISTANTS_PATH` is set).
...
mod tests {
    ...
    // T-B2 — bounded concurrent-startup retry policy.
    ...
    // duplicate definitions, no failure (concurrent-startup regression; spec §9).
    #[tokio::test]
    ...
    struct Fixture {
        ...
        override_repo: Arc<dyn IAssistantOverrideRepository>,
        provider_repo: Arc<dyn IProviderRepository>,
        agent_rows: Arc<Mutex<Vec<tjuaeui_api_types::AgentManagementRow>>>,
        _tmp: TempDir,
        _db: tjuaeui_db::Database,
    }

    #[derive(Clone, Default)]
    struct StubAgentCatalog {
        rows: Arc<Mutex<Vec<tjuaeui_api_types::AgentManagementRow>>>,
    }
    ...
    /// `resolve_default_agent_type` returns `"tjuaecli"`. Tests that need to
    ...
    struct FixtureOpts {
        ...
        /// Defaults to `"openai"` so existing tests get an `"tjuaecli"`
        /// default agent type.
        seed_platform: Option<&'static str>,
        agent_rows: Vec<tjuaeui_api_types::AgentManagementRow>,
    }
    ...
    fn mk_agent_row(
        id: &str,
        backend: &str,
        status: tjuaeui_api_types::AgentManagementStatus,
    ) -> tjuaeui_api_types::AgentManagementRow {
        tjuaeui_api_types::AgentManagementRow {
            id: id.into(),
            ...
            agent_source: tjuaeui_api_types::AgentSource::Builtin,
            ...
            behavior_policy: tjuaeui_api_types::BehaviorPolicy {
                supports_team: true,
                ..Default::default()
            },
            yolo_id: None,
            ...
            team_capable: true,
            status,
            last_check_status: Some(tjuaeui_api_types::AgentSnapshotCheckStatus::Online),
            last_check_kind: Some(tjuaeui_api_types::AgentSnapshotCheckKind::Manual),
    ...
    fn mk_uninstalled_agent_row(id: &str, backend: &str) -> tjuaeui_api_types::AgentManagementRow {
        let mut row = mk_agent_row(id, backend, tjuaeui_api_types::AgentManagementStatus::Unchecked);
        row.installed = false;
    ...
    async fn bootstrap_skips_dirty_generated_assistant_definitions() {
        let fx = fixture().await;
        {
            ...
            *rows = vec![
                mk_agent_row("agent-dirty", "dirty", tjuaeui_api_types::AgentManagementStatus::Online),
                mk_agent_row("agent-valid", "valid", tjuaeui_api_types::AgentManagementStatus::Online),
            ];
        }
    ...
    async fn builtin_listing_uses_manifest_default_enabled_and_sort_order() {
        ...
        let mut butler = mk_builtin("tjuaeui-assistant", "Butler");
        ...
        let butler_resp = list.iter().find(|a| a.id == "tjuaeui-assistant").unwrap();
        assert!(butler_resp.enabled, "butler defaults to enabled");
        assert_eq!(butler_resp.sort_order, 0);
        let butler_idx = list.iter().position(|a| a.id == "tjuaeui-assistant").unwrap();
    ...
    async fn list_maps_generated_definition_to_generated_source() {
        ...
        fx.agent_rows
            ...
            .push(mk_agent_row(
                "agent-claude",
                "claude",
                tjuaeui_api_types::AgentManagementStatus::Online,
            ));
        fx.definition_repo
            .upsert(&UpsertAssistantDefinitionParams {
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_agent() {
        ...
        assert_eq!(bare.agent_status, tjuaeui_api_types::AgentManagementStatus::Online);
    ...
    async fn bootstrap_materializes_generated_assistant_from_unchecked_agent() {
        let mut unchecked_row = mk_agent_row(
            "agent-cursor",
            "cursor",
            tjuaeui_api_types::AgentManagementStatus::Unchecked,
        );
        ...
        assert_eq!(bare.agent_status, tjuaeui_api_types::AgentManagementStatus::Unchecked);
        assert!(bare.team_selectable);
    ...
    async fn list_hides_existing_generated_assistant_when_agent_is_uninstalled_until_installed() {
        ...
        uninstalled_row.status = tjuaeui_api_types::AgentManagementStatus::Offline;
        let fx = fixture_with_options(FixtureOpts {
...

        {
            ...
            rows[0].status = tjuaeui_api_types::AgentManagementStatus::Unchecked;
        }
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_custom_agent() {
        ...
        custom_row.agent_source = tjuaeui_api_types::AgentSource::Custom;
    ...
    async fn bootstrap_falls_back_to_agent_type_when_backend_is_empty() {
        // Engines like TjuaeCLI carry their identity in `agent_type` and leave
        ...
        agent_row.agent_type = tjuaeui_common::AgentType::TjuaeCli;
        ...
        let bare = list
            .iter()
            .find(|assistant| assistant.id == "bare:agent-tjuaecli")
            .unwrap();
        assert_eq!(bare.agent_id, "agent-tjuaecli");
    }

    #[tokio::test]
    async fn tjuae_cli_assistant_resolves_agent_status_via_agent_type_not_backend() {
        // Regression: an assistant whose engine is tjuae_cli must match the tjuae_cli
        // agent row by `agent_type` ("tjuaecli"), since that row's `backend` is
        // NULL. Matching on `backend` alone left the row unresolved and
        // mislabelled every tjuae_cli assistant as Missing/unavailable.
        let mut tjuae_cli_row = mk_agent_row(
            "agent-tjuaecli",
            "tjuaecli",
            tjuaeui_api_types::AgentManagementStatus::Online,
        );
        tjuae_cli_row.backend = None;
        tjuae_cli_row.agent_type = tjuaeui_common::AgentType::TjuaeCli;

        let mut builtin = mk_builtin("builtin-tjuaecli", "Tjuae Assistant");
        builtin.agent_ref = "tjuaecli".into();

        let fx = fixture_with_options(FixtureOpts {
            builtins: vec![builtin],
            agent_rows: vec![tjuae_cli_row],
            ..Default::default()
        })
        .await;
        ...
        let assistant = list
            .iter()
            .find(|assistant| assistant.id == "builtin-tjuaecli")
            .expect("tjuaecli builtin assistant should be listed");
        assert_eq!(
            assistant.agent_status,
            tjuaeui_api_types::AgentManagementStatus::Online,
            "tjuaecli assistant should resolve to the online tjuaecli agent row, not Missing"
        );
    }
    ...
    async fn bootstrap_places_new_generated_assistants_before_existing_assistants() {
        let fx = fixture_with_options(FixtureOpts {
            ...
            agent_rows: vec![
                ...
                mk_agent_row("agent-codex", "codex", tjuaeui_api_types::AgentManagementStatus::Online),
            ],
            ..Default::default()
        })
        .await;
    ...
    async fn bootstrap_reactivates_soft_deleted_builtin_definition_by_source_ref() {
        let mut builtin = mk_builtin("tjuaeui-assistant", "TjuaeUI Butler");
        builtin.rule_file = Some("rules/tjuaeui-assistant.{locale}.md".into());
        let fx = fixture_with_builtins(vec![builtin]).await;

        let original = fx
            .definition_repo
            .get_by_assistant_id("tjuaeui-assistant")
            .await
            .unwrap()
        ...
        assert_eq!(restored.rule_resource_ref.as_deref(), Some("tjuaeui-assistant"));
    ...
    /// Anthropic provider routes to TjuaeCLI, not the Claude Code CLI:
    /// having an Anthropic API key does not imply the user has
    /// `claude` on `PATH`. CLI-based agents must be opted into
    /// explicitly.
    #[tokio::test]
    async fn resolve_default_agent_id_routes_anthropic_provider_to_tjuae_cli() {
        let fx = fixture_with_options(FixtureOpts {
    ...
    /// OpenAI / custom provider falls back to TjuaeCLI, the only TjuaeUI
    /// agent that doesn't require a third-party CLI.
    #[tokio::test]
    async fn resolve_default_agent_id_falls_back_to_tjuae_cli_for_openai_provider() {
        let fx = fixture_with_options(FixtureOpts {
    ...
    /// Custom (non-anthropic, non-openai) platform also routes to TjuaeCLI,
    /// which handles OpenAI-compatible custom URLs.
    #[tokio::test]
    async fn resolve_default_agent_id_handles_custom_platform_as_tjuae_cli() {
        let fx = fixture_with_options(FixtureOpts {
    ...
    /// or otherwise — should resolve to `"tjuaecli"`, the only built-in
    ...
    async fn create_without_agent_id_does_not_default_to_gemini_when_provider_exists() {
        for platform in ["anthropic", "openai"] {
            ...
            assert_eq!(
                ...
                "{platform} provider should resolve to tjuaecli"
            );
        }
    }
