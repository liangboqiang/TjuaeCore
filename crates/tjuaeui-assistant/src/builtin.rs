...
//! still can, via the `TJUAE_BUILTIN_ASSISTANTS_PATH` env var → disk
//! fallback path.
...
/// `crates/tjuaeui-app/assets/builtin-assistants/`.
static BUILTIN_ASSETS: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../tjuaeui-app/assets/builtin-assistants");
...
/// `TJUAE_BUILTIN_ASSISTANTS_PATH` at a fixture directory.
enum Source {
    Embedded,
    Disk(PathBuf),
}
...
impl BuiltinAssistantRegistry {
    ...
    ///
    /// If `TJUAE_BUILTIN_ASSISTANTS_PATH` is set and points to a readable
    /// directory, read from disk (test-only override). Otherwise use the
    /// assets embedded at compile time.
    pub fn load() -> Self {
        if let Ok(env) = std::env::var("TJUAE_BUILTIN_ASSISTANTS_PATH") {
            let p = PathBuf::from(env);
            if p.exists() {
                return Self::load_from_dir(p);
            }
            warn!(
                "TJUAE_BUILTIN_ASSISTANTS_PATH points to missing directory; \
    ...
    /// `TJUAE_BUILTIN_ASSISTANTS_PATH` E2E fixtures — the three
...
mod tests {
    ...
    // TJUAE_BUILTIN_ASSISTANTS_PATH). Graceful-degradation semantics must
    // stay intact.
    // -----------------------------------------------------------------------
    ...
    fn load_respects_env_var_disk_override() {
        ...
        let key = "TJUAE_BUILTIN_ASSISTANTS_PATH";
