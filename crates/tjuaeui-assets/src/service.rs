...
impl AssetService {
    ...
    pub fn get_logo(&self, asset_path: &str) -> Result<AssetFile, AssetError> {
        let normalized = normalize_logo_path(asset_path)
            .ok_or_else(|| AssetError::Forbidden(format!("资源路径越出徽标根目录：{asset_path}")))?;

        let file = LogoAssets::get(&normalized).ok_or_else(|| AssetError::NotFound("找不到徽标资源".into()))?;
...
mod tests {
    ...
    fn normalize_logo_path_rejects_traversal() {
        assert!(normalize_logo_path("../brand/tjuae-cli.svg").is_none());
