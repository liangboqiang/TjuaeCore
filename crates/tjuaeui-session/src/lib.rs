//! `tjuaeui-session` — server-authoritative SessionState control plane (feature
//! 002). Domain layer.
//!
//! ## What this crate is
//! It spawns the claude CLI directly (via 001 `tjuaeui-process`, no ACP/SDK),
...
//! `tjuaeui-conversation` + WebSocket — all Non-Goal here, untouched. This crate
...
//!
//! ## Layering
//! Domain layer, parallel to `tjuaeui-ai-agent`. Depends ONLY on
//! `tjuaeui-process` (001) + `tjuaeui-common`. NO production caller this
...
//!   acp/tjuae_cli impls in `backend/`) — the ONLY backend-aware code: startup+prompt
