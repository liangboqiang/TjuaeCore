...
use tjuaeui_process::{BoxedStdin, BoxedStdout, ManagedProcess, ProcessError, Spawner};
...
/// `tjuaeui-tjuae_cli` crate's own invariant test (tjuae_cli depends on session, so it
/// cannot live in session's `tests/` and be reached from tjuae_cli — it lives here,
