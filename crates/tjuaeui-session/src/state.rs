...
/// codex fills only label, and ACP/tjuae_cli have no workflow concept (empty roster).
...
///       queue affordance (MX-QUEUE-3). ACP/tjuae_cli lack the path → degrade to false.
///
/// Orthogonal to `can_send` (Idle): `can_send || can_queue` is the input-box gate.
/// Truth table (× backend `accepts_proactive_input`):
/// | state                              | claude(true) | codex/acp/tjuae_cli(false) |
/// |------------------------------------|--------------|-------------------------|
...
/// with a Workflow and was WRONG. Re-measured on claude 2.1.176 (TjuaeCore's exact
...
mod tests {
    ...
    fn can_queue_truth_table_capability_gated() {
        ...
        // codex/acp/tjuae_cli (accepts_proactive_input=false): degrades to false in
