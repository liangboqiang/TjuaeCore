...
pub enum Command {
    ...
    /// pure accept/decline approval (codex/tjuae_cli) ignore it.
    ///
...
impl BackendError {
    ...
    /// before.
    pub(crate) fn from_spawn(context: &'static str, e: tjuaeui_process::ProcessError) -> Self {
        match e {
            tjuaeui_process::ProcessError::WorkspaceUnavailable(path) => Self::WorkspaceUnavailable(path),
...
/// How a session is (re)opened. `session_id` is the STABLE TjuaeCore-minted
...
mod tests {
    ...
    fn from_spawn_keeps_workspace_unavailable_class() {
        let err = BackendError::from_spawn(
            "Codex 进程启动失败",
            tjuaeui_process::ProcessError::workspace_unavailable("/gone/workspace"),
        );
        assert_eq!(err, BackendError::WorkspaceUnavailable("/gone/workspace".into()));

        let err = BackendError::from_spawn("Codex 进程启动失败", tjuaeui_process::ProcessError::internal("boom"));
        assert_eq!(
            err,
            BackendError::Transport("Codex 进程启动失败: 内部错误：boom".into())
        );
    }
}
