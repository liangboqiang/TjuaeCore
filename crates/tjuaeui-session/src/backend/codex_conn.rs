...
impl BackendConnection for CodexConnection {
    async fn open_session(
        ...
        let cmd = tjuaeui_common::CommandSpec {
            ...
            // spawn env (per-agent overrides + TJUAE_* conversation runtime
        ...
        let proc = self
            .spawner
            .spawn(cmd, &[], "tjuaeui-session")
            .await
            .map_err(|e| BackendError::from_spawn("Codex 进程启动失败", e))?;
        ...
        // tjuae-probe transcripts in protocols/verification fixtures):
...
fn initialize_params() -> HandshakeParams {
    HandshakeParams(json!({
        "clientInfo": { "name": "tjuaeui-session", "version": "0.1.0" },
...
fn builtin_slash_commands() -> Vec<crate::capability::SlashCommandInfo> {
    ...
    vec![
        SlashCommandInfo {
            name: "review".into(),
            description: Some("审查当前更改并找出问题".into()),
        },
        SlashCommandInfo {
            name: "review-branch".into(),
            description: Some("对照指定分支审查代码变更".into()),
        },
        SlashCommandInfo {
            name: "review-commit".into(),
            description: Some("审查指定提交引入的代码变更".into()),
        },
        SlashCommandInfo {
            name: "init".into(),
            description: Some("创建包含 Codex 作业说明的 AGENTS.md 文件".into()),
        },
        SlashCommandInfo {
            name: "compact".into(),
            description: Some("压缩对话，避免达到上下文上限".into()),
        },
        SlashCommandInfo {
            name: "logout".into(),
            description: Some("退出 Codex 登录".into()),
        },
    ]
}
...
pub struct CodexSessionBackend {
    ...
    stdin: Arc<Mutex<Option<tjuaeui_process::BoxedStdin>>>,
...
fn start_codex_reader(
    ...
    stdout: Option<tjuaeui_process::BoxedStdout>,
    io: Arc<dyn AgentIo>,
...
impl CodexSessionBackend {
    ...
    async fn wake_handle(&self) -> Result<ProcHandle, BackendError> {
        ...
        let proc = spawner
            .spawn(cmd, &[], "tjuaeui-session")
            .await
            .map_err(|e| BackendError::from_spawn("Codex 恢复进程启动失败", e))?;
            ...
            Ok(Some(line)) => {
                ...
                match (method, has_id) {
                    ...
                    (Some(m), false) => {
                        ...
                        // terminal). See protocols/design/tjuaecore-codex-turn-no-terminal-hang-prompt.md.
                        if m == "error"
...
///
/// codex's permission tiers are DISCOVERED, not a fixed TjuaeUI enum — mirroring the
/// legacy ACP mechanism (`manager/acp/session.rs`: `availableModes[]` advertised by the
/// agent, `is_mode_valid` validates against that live list, TjuaeCore defines no values).
...
mod codex_perm {
    ...
    ///   - `agent-full-access` (canonical) / `full-access` / `yolo` → `:danger-full-access`
    ///   - `read-only`                               → `:read-only`
    ...
    pub(super) fn normalize_to_profile_id(mode: &str) -> String {
        ...
        match trimmed {
            ...
            // `normalize_requested_mode`); `full-access` and `yolo` map onto the
            // danger-full-access profile.
            "agent-full-access" | "full-access" | "yolo" => ":danger-full-access".to_owned(),
    ...
    pub(super) fn builtin_profile_display(profile_id: &str) -> Option<(&'static str, &'static str)> {
        match profile_id {
            ":read-only" => Some((
                "只读",
                "Codex 可以读取当前工作区中的文件；编辑文件或访问互联网需要批准。",
            )),
            ":workspace" => Some((
                "默认",
                "Codex 可以读写当前工作区中的文件并运行命令；访问互联网或编辑其他文件需要批准。（与智能体模式相同）",
            )),
            ":danger-full-access" => Some((
                "完全访问",
                "Codex 可以编辑当前工作区之外的文件，并且无需批准即可访问互联网。请谨慎使用。",
            )),
            _ => None,
        }
    }
...
fn fill_discovery(kind: DiscoveryKind, result: &Value, discovered: &Arc<std::sync::Mutex<Discovered>>) {
    ...
    match kind {
        ...
        DiscoveryKind::Permissions => {
            ...
            // longer translate to a fixed TjuaeUI enum or drop custom profiles: codex
            // defines the value set, TjuaeCore only transports it (parity with legacy ACP,
...
async fn handle_reverse_rpc(
    ...
    match method {
        ...
        _ => {
            write_reverse_error(stdin, &id, -32601, "method not handled by tjuaeui-session").await;
            emit(
                event_tx,
                session_id,
                cur,
...
fn map_notification(method: &str, params: &Value) -> Vec<SessionEvent> {
    match method {
        ...
        // collaboration selector in TjuaeUI, and that field would clobber the permission mode.
        ...
        match command {
            Command::Send { content, metadata } => {
                ...
                if matches!(route, Some(SlashRoute::Logout)) {
                    self.suspend
                        .ensure_awake(tjuaeui_common::now_ms(), || self.wake_handle())
                        .await?;
                ...
                // (verified against the tjuae-probe transcripts). Needs the bound
...
mod tests {
    ...
    async fn dispatch_send_writes_turn_start_with_thread_id() {
        ...
        // the tjuae-probe transcripts — NOT the fictional `sendUserTurn`). Requires
    ...
    async fn dispatch_set_mode_normalizes_legacy_persisted_value() {
        // A legacy persisted BARE value (`yolo`) — from an older TjuaeUI that stored the
    ...
    async fn dispatch_set_mode_passes_custom_profile_id_verbatim() {
        ...
        // heart of the legacy-ACP parity: codex owns the value set, TjuaeCore only transports
    ...
    async fn dispatch_set_mode_obsolete_and_unknown_bare_values_fall_to_workspace() {
        // Legacy-ACP parity: TjuaeCore never ships a frame codex would reject. Any unknown
    ...
    async fn r4_open_session_routes_through_injected_spawner_with_codex_app_server() {
            ...
            .open_session(
                ...
                SessionConfig {
                    ...
                    spawn_env: vec![tjuaeui_common::EnvVar {
                        name: "TJUAE_CONVERSATION_ID".into(),
                        value: "conv-1".into(),
                    }],
                    ..Default::default()
                },
        ...
        // (TJUAE_* runtime context + per-agent overrides) reaches the process.
        assert_eq!(
            spec.env
                .iter()
                .map(|e| (e.name.as_str(), e.value.as_str()))
                .collect::<Vec<_>>(),
            [("TJUAE_CONVERSATION_ID", "conv-1")],
    ...
    /// workspace-unavailable API error (workspace-availability follow-up).
    #[tokio::test]
    async fn open_session_classifies_workspace_unavailable_spawn_failure() {
        struct WorkspaceGoneSpawner;
        #[async_trait::async_trait]
        impl tjuaeui_process::Spawner for WorkspaceGoneSpawner {
            async fn spawn(
                &self,
                _spec: tjuaeui_common::CommandSpec,
                _extra_env: &[(String, String)],
                _opaque_owner_tag: &str,
            ) -> Result<std::sync::Arc<tjuaeui_process::ManagedProcess>, tjuaeui_process::ProcessError> {
                Err(tjuaeui_process::ProcessError::workspace_unavailable("/gone/workspace"))
            }
        }
    ...
    /// ACP agent's `availableModes[]`), TjuaeCore does not translate or whitelist.
    #[tokio::test]
    async fn fill_discovery_surfaces_permission_profiles_verbatim() {
        ...
        assert_eq!(workspace.name, "默认", "内置工作区级别应使用友好的中文名称");
        assert_eq!(
            workspace.description.as_deref(),
            Some("Codex 可以读写当前工作区中的文件并运行命令；访问互联网或编辑其他文件需要批准。（与智能体模式相同）"),
            "built-in workspace tier gets the legacy friendly description"
        );
        let read_only = d.modes.iter().find(|m| m.id == "read-only").unwrap();
        assert_eq!(read_only.name, "只读");
        let full = d.modes.iter().find(|m| m.id == "full-access").unwrap();
        assert_eq!(full.name, "完全访问");
    }

    /// UT-1b: `normalize_to_profile_id` — the ONE translation TjuaeCore still owns —
    ...
    fn codex_perm_normalize_to_profile_id_maps_legacy_and_passes_colon_ids() {
        ...
        // `agent-full-access`, `full-access` and `yolo` all select danger-full-access.
        for legacy in ["agent-full-access", "full-access", "yolo"] {
        ...
        // verbatim — codex, not TjuaeCore, owns the value set (legacy-ACP parity).
        for id in [
            ":workspace",
            ":danger-full-access",
            ":read-only",
    ...
    fn codex_perm_mode_to_catalog_value_lands_on_catalog_vocabulary() {
        ...
        for legacy in ["full-access", "yolo"] {
    ...
    async fn r8_send_during_never_arriving_handshake_times_out_and_errors() {
        ...
        let saved = std::env::var("TJUAE_HANDSHAKE_TIMEOUT_SECS").ok();
        unsafe { std::env::set_var("TJUAE_HANDSHAKE_TIMEOUT_SECS", "1") };
        let fake = FakeAgentIo::never_exits(Vec::new());
        ...
        let res = backend
            ...
            .await;
        match saved {
            Some(v) => unsafe { std::env::set_var("TJUAE_HANDSHAKE_TIMEOUT_SECS", v) },
            None => unsafe { std::env::remove_var("TJUAE_HANDSHAKE_TIMEOUT_SECS") },
        }
    ...
    async fn f4_suspend_then_wake_respawns_through_spawner() {
        ...
        assert!(
            backend
                .suspend
                .suspend_if_idle(tjuaeui_common::now_ms() + 10_000, false)
                .await
        );
        ...
        assert!(
            matches!(&err, BackendError::Transport(m) if m.contains("恢复进程启动失败")),
    ...
    /// legacy 30s (tjuaeui-agent-rest INIT_TIMEOUT_SECS), NOT the magic 2s the rewrite
    ...
    fn handshake_budget_is_legacy_30s_default_env_overridable() {
        ...
        let saved = std::env::var("TJUAE_HANDSHAKE_TIMEOUT_SECS").ok();
        unsafe { std::env::remove_var("TJUAE_HANDSHAKE_TIMEOUT_SECS") };
        assert!(
        ...
        unsafe { std::env::set_var("TJUAE_HANDSHAKE_TIMEOUT_SECS", "45") };
        assert_eq!(
