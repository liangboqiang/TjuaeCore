...
//! the upper `tjuaeui-ai-agent` Domain crate; `tjuaeui-session` must stay
...
pub struct AcpConnection {
    ...
    command: tjuaeui_common::CommandSpec,
}

impl AcpConnection {
    pub fn new(spawner: Arc<dyn Spawner>, command: tjuaeui_common::CommandSpec) -> Self {
...
impl BackendConnection for AcpConnection {
    async fn open_session(
        ...
        let proc = self
            ...
            .await
            .map_err(|e| BackendError::from_spawn("ACP 进程启动失败", e))?;
...
pub fn acp_capabilities() -> Capabilities {
    Capabilities {
        ...
        emits: SignalSet {
            ...
            // prompt response (no idle-timeout in TjuaeCore anyway, post-007).
            heartbeat: false,
            tool_lifecycle: true,
...
impl AcpSessionBackend {
    ...
    async fn bound_session(&self) -> Result<String, BackendError> {
        ...
        // (tjuaeui-agent-rest INIT_TIMEOUT_SECS). A cold start / untrusted project slows
    ...
    async fn wake_handle(&self) -> Result<ProcHandle, BackendError> {
        ...
        let proc = spawner
            .spawn(cmd, &[], "tjuaeui-session")
            .await
            .map_err(|e| BackendError::from_spawn("ACP 恢复进程启动失败", e))?;
...
struct AcpWakeRecipe {
    ...
    command: Option<tjuaeui_common::CommandSpec>,
    config: SessionConfig,
}
...
async fn handle_open_response(
    ...
    // protocols/design/tjuaecore-opencode-acp-configoptions-model-mode-prompt.md.
    let mut disc = discovered.lock().unwrap_or_else(|e| e.into_inner());
...
fn enrich_acp_error_message(raw: &str, stderr_tail: Option<&str>) -> String {
    ...
    if let Some(tail) = stderr_tail
        && let Some(cause) = tjuaeui_common::error_extract::extract_error_message(tail)
    {
...
mod tests {
    ...
    fn parse_permission_metadata_extracts_claude_title_prefix_server() {
        ...
        let params = json!({
            ...
            "toolCall": { "title": "mcp__tjuaeui-team__team_members", "rawInput": {} },
        });
        let meta = parse_permission_metadata(Some(&params)).expect("metadata present");
        assert_eq!(meta["server_name"], "tjuaeui-team");
    ...
    fn parse_permission_metadata_extracts_codex_raw_input_server() {
        ...
        let params = json!({
            ...
            "toolCall": { "title": "mcp__other__x", "rawInput": { "server_name": "tjuaeui-team-guide" } },
        });
        let meta = parse_permission_metadata(Some(&params)).expect("metadata present");
        assert_eq!(meta["server_name"], "tjuaeui-team-guide", "rawInput.server_name wins");
    }
    ...
    async fn f4_suspend_then_wake_respawns_through_spawner() {
        ...
        let command = tjuaeui_common::CommandSpec {
            command: "hermes".into(),
            args: vec!["acp".into()],
            env: Vec::new(),
            cwd: None,
