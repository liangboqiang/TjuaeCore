...
/// (tjuaeui-agent-rest/src/protocol/acp.rs:55, an `oneshot` + `tokio::time::timeout`)
...
/// Restore the legacy 30s budget (env-overridable via `TJUAE_HANDSHAKE_TIMEOUT_SECS`
...
pub(crate) fn handshake_budget() -> std::time::Duration {
    let secs = std::env::var("TJUAE_HANDSHAKE_TIMEOUT_SECS")
        .ok()
...
/// → `session/new`|`session/load` `mcpServers[]`; codex → `thread/start`; tjuae_cli →
/// its `McpServerConfig` map). Deliberately crate-local — `tjuaeui-session` stays
/// self-contained (no `tjuaeui-api-types`/ACP-SDK dep, §Cargo.toml). The app
...
pub struct SessionInit {
    ...
    /// Opaque persisted session snapshot for resume (tjuae_cli `SessionManager` JSON
    /// blob). Kept as `serde_json::Value` so `tjuaeui-session` need not depend on
    /// the `tjuae-agent` `Session` type; the tjuae_cli backend deserializes it.
    pub session_snapshot: Option<serde_json::Value>,
    /// Explicit resume flag. acp/codex resume via `SessionSpec::Resume`; this is the
    /// tjuae_cli equivalent + the single source of truth for "sanitize on resume".
    pub resume: bool,
}
...
pub struct SessionConfig {
    ...
    pub spawn_env: Vec<tjuaeui_common::EnvVar>,
