...
/// Pure `serde_json`, no ACP SDK — `tjuaeui-session` stays SDK-free.
...
impl BackendConnection for ClaudeConnection {
    async fn open_session(
        ...
        let io = adapter
            ...
            .await
            .map_err(|e| BackendError::from_spawn("Claude 进程启动失败", e))?;
...
impl ClaudeSessionBackend {
    ...
    async fn wake_handle(&self) -> Result<ProcHandle, BackendError> {
        ...
        let io = self
            ...
            .await
            .map_err(|e| BackendError::from_spawn("Claude 恢复进程启动失败", e))?;
        let io: Arc<dyn AgentIo> = Arc::from(io);
...
/// DIAGNOSTIC (env-gated, default OFF): when `TJUAE_CLAUDE_WIRE_DUMP` is set, log
...
fn claude_wire_dump_enabled() -> bool {
    std::env::var("TJUAE_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty())
}
...
fn dump_wire(direction: &str, session_id: &str, turn_gen: u64, bytes: &[u8]) {
    ...
    tracing::info!(
        target: "tjuaeui_session::claude_wire",
        direction,
                        ...
                        match tokio::time::timeout(std::time::Duration::from_millis(200), stdout.read(&mut chunk)).await {
                            ...
                            Ok(Ok(n)) if n > 0 => {
                                ...
                                match parsed {
                                    ...
                                    Err(_) => {
                                        tracing::error!(
                                            target: "tjuaeui_session::backend::claude_conn",
                                            conversation_id = %session_id,
...
mod tests {
    ...
    fn build_claude_init_args_invalid_mode_falls_back_to_default() {
        ...
        for invalid in ["yolo", "manual", "acceptedits", "danger", "Plan"] {
            assert_eq!(
    ...
    async fn f4_turn_in_flight_blocks_idle_suspend() {
        ...
        let suspended = backend
            .suspend
            .suspend_if_idle(tjuaeui_common::now_ms() + 10_000, true)
            .await;
