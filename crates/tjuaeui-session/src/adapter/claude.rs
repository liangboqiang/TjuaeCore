...
use tjuaeui_process::{BoxedStdin, ProcessError, Spawner};
...
pub(crate) fn claude_permission_modes() -> Vec<ModeInfo> {
    ...
    [
        ("default", "默认", "标准行为，执行危险操作前会请求确认"),
        ("acceptEdits", "接受编辑", "自动接受文件编辑操作"),
        ("plan", "计划模式", "仅制定计划，不实际执行工具"),
        ("dontAsk", "不询问", "不请求权限，未预先允许的操作将被拒绝"),
        ("bypassPermissions", "跳过权限检查", "跳过所有权限检查"),
    ]
    .into_iter()
...
/// leak), mirroring the ACP/tjuae_cli per-kind id convention. `Other` has no
        ...
        for b in &blocks {
            match b.get("type").and_then(Value::as_str).unwrap_or("") {
                ...
                "tool_use" => {
                    ...
                    // #486 (parity with tjuae_cli output_sink): DROP a malformed empty-name
...
impl BackendAdapter for ClaudeAdapter {
    async fn start_turn(
        ...
        let proc = spawner.spawn(spec, &[], "tjuaeui-session").await?;
...
async fn write_ndjson_line(stdin: &mut BoxedStdin, value: &Value) -> Result<(), ProcessError> {
    ...
    bytes.push(b'\n');
    // DIAGNOSTIC (env-gated, default OFF via TJUAE_CLAUDE_WIRE_DUMP): log the RAW
    ...
    if std::env::var("TJUAE_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty()) {
...
mod tests {
    ...
    async fn captured_frame_with_id(content: &[crate::ContentBlock], client_msg_id: Option<&str>) -> String {
        ...
        let mut stdin: tjuaeui_process::BoxedStdin = Box::new(CaptureStdin(buf.clone()));
        ClaudeAdapter::new()
    ...
    /// persists as a ghost `tool_step{name:""}` row. Mirrors tjuae_cli's empty-name
