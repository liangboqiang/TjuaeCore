...
mod tests {
    ...
    fn test_system_info_response_serialization() {
        let resp = SystemInfoResponse {
            cache_dir: "/home/user/.cache/tjuaeui".into(),
            work_dir: "/home/user/.local/share/tjuaeui".into(),
            log_dir: "/home/user/.local/state/tjuaeui/logs".into(),
            platform: "linux".into(),
            arch: "x64".into(),
        };
        let json = serde_json::to_value(&resp).unwrap();
        assert_eq!(json["cache_dir"], "/home/user/.cache/tjuaeui");
        assert_eq!(json["work_dir"], "/home/user/.local/share/tjuaeui");
        assert_eq!(json["log_dir"], "/home/user/.local/state/tjuaeui/logs");
    ...
    fn test_update_check_request_with_options() {
        let raw = json!({
            ...
            "repo": "liangboqiang/TjuaeUI"
        });
        let req: UpdateCheckRequest = serde_json::from_value(raw).unwrap();
        assert!(req.include_prerelease);
        assert_eq!(req.repo.as_deref(), Some("liangboqiang/TjuaeUI"));
    }
    ...
    fn test_update_check_result_with_update() {
        let result = UpdateCheckResult {
            ...
            latest: Some(UpdateReleaseInfo {
                ...
                html_url: "https://github.com/liangboqiang/TjuaeUI/releases/tag/v2.0.0".into(),
                published_at: Some("2026-04-01T00:00:00Z".into()),
                prerelease: false,
                draft: false,
                assets: vec![GitHubReleaseAsset {
                    name: "tjuaeui-2.0.0-darwin-arm64.dmg".into(),
                    url: "https://github.com/download/tjuaeui.dmg".into(),
                    size: 85_000_000,
        ...
        assert_eq!(json["latest"]["assets"][0]["name"], "tjuaeui-2.0.0-darwin-arm64.dmg");
