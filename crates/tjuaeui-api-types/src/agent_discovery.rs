...
use tjuaeui_common::{AgentType, TimestampMs};

/// How an agent row was sourced.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum AgentSource {
    /// Ships with the backend binary (no CLI install required — e.g. `tjuae_cli`).
    Internal,
...
pub struct AgentSourceInfo {
    ...
    /// 远程自定义 Agent 的 A2A Agent Card 或服务地址。
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// the frontend verbatim, and typed-decoded inside `tjuaeui-ai-agent`
...
pub struct AgentMetadata {
    ...
    /// Native mode id that TjuaeUI's generic `yolo` mode resolves to
    /// before calling `session/set_mode`. `None` means the backend has no
    /// "yolo" equivalent and the value should
