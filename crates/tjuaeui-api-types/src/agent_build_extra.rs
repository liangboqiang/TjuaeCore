...
/// TjuaeCli-specific fields extracted from `extra` in build task options.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct TjuaeCliBuildExtra {
    #[serde(default)]
...
mod tests {
    ...
    fn acp_build_extra_ignores_legacy_guide_config_field() {
        ...
        let parsed: AcpBuildExtra = serde_json::from_value(serde_json::json!({
            "backend": "claude",
            legacy_key: {"port": 1234, "token": "legacy", "binary_path": "/bin/tjuaecore"}
        }))
        .unwrap();
    ...
    #[test]
    fn tjuae_cli_build_extra_ignores_legacy_fields() {
        let legacy_key = concat!("guide", "_mcp_config");
        let parsed: TjuaeCliBuildExtra = serde_json::from_value(serde_json::json!({
            "backend": "tjuaecli",
            "max_tokens": 8192,
            legacy_key: {"port": 1234, "token": "legacy", "binary_path": "/bin/tjuaecore"}
        }))
        ...
        assert_eq!(parsed.backend.as_deref(), Some("tjuaecli"));
        let serialized = serde_json::to_value(&parsed).unwrap();
        assert!(
            serialized.get(legacy_key).is_none(),
            "legacy guide config must be ignored, not re-serialized"
