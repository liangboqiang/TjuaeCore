...
use tjuaeui_ai_agent::{AgentStreamEvent, IWorkerTaskManager};
use tjuaeui_api_types::{AssistantConversationRequest, CreateConversationRequest, SendMessageRequest};
use tjuaeui_common::{AgentType, ConversationSource};
...
impl ChannelMessageService {
    ...
    async fn create_conversation_for_session(
        ...
        // Top-level `model` is only accepted for tjuae_cli; other types pass via `extra`.
        let top_level_model = if agent_type == AgentType::TjuaeCli {
            Some(model)
        } else {
...
fn platform_to_source(platform: PluginType) -> ConversationSource {
    match platform {
        ...
        // Reserved variants default to TjuaeUI
        PluginType::Slack | PluginType::Discord => ConversationSource::TjuaeUI,
    }
}
...
mod tests {
    use super::*;
    use tjuaeui_ai_agent::protocol::events::{
    ...
    #[test]
    fn platform_to_source_reserved_defaults_to_tjuaeui() {
        assert_eq!(platform_to_source(PluginType::Slack), ConversationSource::TjuaeUI);
        assert_eq!(platform_to_source(PluginType::Discord), ConversationSource::TjuaeUI);
    }
    ...
    fn parse_known_agent_types() {
        ...
        assert_eq!(parse_agent_type("tjuaecli").unwrap(), AgentType::TjuaeCli);
    }
    ...
    // ── model placement by agent_type (regression: non-tjuae_cli must not
    //    use top-level model) ──────────────────────────────────────────
    ...
    #[test]
    fn tjuae_cli_model_stays_at_top_level() {
        let agent_type = AgentType::TjuaeCli;
        let model = ProviderWithModel {
        ...
        assert!(top_level_model.is_some(), "tjuaecli must use top-level model");
    ...
    #[test]
    fn conv_name_telegram_tjuae_cli() {
        let name = channel_conversation_name(PluginType::Telegram, "tjuaecli", None, Some("70880480"));
        assert_eq!(name, "tg-tjuaecli-70880480");
    }
    ...
    fn conv_name_non_acp_ignores_backend() {
        let name = channel_conversation_name(PluginType::Telegram, "tjuaecli", Some("claude"), Some("70880480"));
        assert_eq!(name, "tg-tjuaecli-70880480");
    }
}
