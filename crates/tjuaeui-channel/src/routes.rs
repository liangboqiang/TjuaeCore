...
use tjuaeui_db::{DbError, IChannelRepository};
use tjuaeui_extension::{ExtensionRegistry, ResolvedChannelPlugin};
...
async fn get_channel_settings(
    ...
    let platform =
        PluginType::from_str_opt(&platform).ok_or_else(|| ApiError::BadRequest(format!("平台无效：{}", platform)))?;
...
async fn sync_channel_settings(
    ...
    let _platform = PluginType::from_str_opt(&req.platform)
        .ok_or_else(|| ApiError::BadRequest(format!("平台无效：{}", req.platform)))?;
...
mod tests {
    use super::*;
    use tjuaeui_api_types::TestPluginExtraConfig;
