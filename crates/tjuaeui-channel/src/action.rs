    ...
    async fn handle_platform_action(&self, action: &UnifiedAction) -> Result<ActionResponse, ChannelError> {
        match action.action.as_str() {
            ...
            "pairing.check" => {
                ...
                if authorized {
                    Ok(ActionResponse {
                        text: Some("授权成功！发送消息即可开始聊天。".into()),
                        parse_mode: None,
                        buttons: None,
                        keyboard: None,
                    ...
                    Ok(ActionResponse {
                        text: Some("仍在等待授权，请管理员前往“设置 → 渠道”查看。".into()),
                        parse_mode: None,
...
fn build_pairing_response(code: &str) -> ActionResponse {
    ActionResponse {
        text: Some(format!(
            "欢迎使用！使用此机器人前需要完成授权。\n\n\
             你的配对码：*{code}*\n\n\
             请将配对码告知管理员，由管理员在“设置 → 渠道 → 配对请求”中批准。\n\
             配对码将在 10 分钟后过期。"
        )),
        parse_mode: None,
        buttons: Some(vec![vec![
...
mod tests {
    ...
    use tjuaeui_db::{DbError, IChannelRepository, IClientPreferenceRepository, UpdatePluginStatusParams};
    ...
    async fn session_new_creates_session() {
        ...
        match result {
            MessageResult::Action(resp) => {
                let text = resp.text.unwrap();
                assert!(text.contains("新会话"));
                // With no client_preferences configured, defaults to "tjuaecli"
                assert!(text.contains("tjuaecli"));
            }
            _ => panic!("Expected Action result"),
        }
    }
