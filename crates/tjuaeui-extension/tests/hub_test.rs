...
use tjuaeui_extension::hub::{HubIndexManager, HubInstaller};
use tjuaeui_extension::registry::ExtensionRegistry;
use tjuaeui_extension::state::ExtensionStateStore;
use tjuaeui_extension::types::HubExtensionStatus;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![tjuaeui_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: tjuaeui_extension::types::ExtensionSource::Env,
    }];
...
async fn verify_installation_catches_invalid_manifest() {
    ...
    std::fs::create_dir_all(&ext_dir).unwrap();
    std::fs::write(ext_dir.join("tjuae-extension.json"), b"not valid json").unwrap();
...
async fn verify_installation_catches_reserved_name() {
    let h = setup();
    let ext_dir = h.hub_dir.path().join("tjuae-bad");
    write_extension_manifest(&ext_dir, "tjuae-bad", "1.0.0");
