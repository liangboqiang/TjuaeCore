...
use tjuaeui_extension::{ExtensionSource, ScanPath, filter_by_engine_compatibility, load_all};
...
fn write_manifest_full(dir: &Path, name: &str, version: &str, engine_tjuae: Option<&str>, api_version: Option<&str>) {
...
fn el2_engine_incompatible_extension_filtered_out() {
    ...
    // Extension requires tjuaeui ^2.0.0 but app is 1.5.0.
...
fn el3_invalid_manifest_skipped_others_load() {
    ...
    fs::write(bad.join("tjuae-extension.json"), b"{ broken json").unwrap();

    // Missing required fields.
    let incomplete = create_ext_dir(tmp.path(), "incomplete");
    fs::write(
        incomplete.join("tjuae-extension.json"),
...
//
// Since TJUAE_E2E_TEST=1 changes the behaviour of resolve_scan_paths()
