...
use tjuaeui_api_types::{ApiResponse, AssistantSource, SkillImportLimitsResponse};
use tjuaeui_extension::classifier::{AssistantClassifier, AssistantRuleDispatcher};
use tjuaeui_extension::error::ExtensionError;
use tjuaeui_extension::external_paths::ExternalPathsManager;
use tjuaeui_extension::skill_routes::{SkillRouterState, skill_routes};
use tjuaeui_extension::skill_service::SkillPaths;
...
async fn router_with_dispatcher(dispatcher: Arc<FakeDispatcher>) -> axum::Router {
    ...
    let skill_repo = Arc::new(tjuaeui_db::SqliteSkillRepository::new(db.pool().clone()));
    std::mem::forget(tmp);
