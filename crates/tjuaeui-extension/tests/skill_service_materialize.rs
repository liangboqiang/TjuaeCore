...
use tjuaeui_extension::{resolve_skill_paths, skill_service};
...
async fn materialize_returns_only_listed_skill_source_paths() {
    ...
    unsafe {
        std::env::set_var(tjuaeui_extension::BUILTIN_SKILLS_ENV_VAR, &builtin_root);
    }
    ...
    unsafe {
        std::env::remove_var(tjuaeui_extension::BUILTIN_SKILLS_ENV_VAR);
    }
}
