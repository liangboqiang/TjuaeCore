...
/// 1. `$TJUAE_EXTENSIONS_PATH`
/// 2. Platform application-data directory
///
/// In E2E test mode (`TJUAE_E2E_TEST=1`), only the environment variable
/// paths are returned to ensure test isolation.
pub fn resolve_scan_paths() -> Vec<ScanPath> {
    let env_path = std::env::var("TJUAE_EXTENSIONS_PATH").ok();
    let e2e_mode = is_e2e_test_mode();
    resolve_scan_paths_inner(env_path.as_deref(), e2e_mode, None)
}

/// Resolve scan paths for the provided desktop `data_dir`.
///
/// Priority (highest first):
/// 1. `$TJUAE_EXTENSIONS_PATH`
...
///
/// Extensions that declare `engine.tjuae` with a version range incompatible
...
/// Check whether the extension's `engine.tjuae` requirement is satisfied.
fn is_engine_compatible(ext: &LoadedExtension, app_version: &semver::Version) -> bool {
    ...
    let Some(required) = &engine.tjuae else {
        return true; // no tjuaeui constraint
    };
    ...
    match semver::VersionReq::parse(required) {
        ...
        Ok(_) => {
            warn!(
                ...
                required = %required,
                actual = %app_version,
                "extension filtered out: engine.tjuae incompatible"
            );
            false
        }
        Err(e) => {
            warn!(
                ...
                required = %required,
                error = %e,
                "extension filtered out: invalid engine.tjuae version requirement"
            );
            false
        }
    }
...
fn is_e2e_test_mode() -> bool {
    std::env::var("TJUAE_E2E_TEST").map(|v| v == "1").unwrap_or(false)
}
...
mod tests {
    ...
    fn write_manifest_full(
        ...
        if let Some(eng) = engine_tjuae {
            manifest["engine"] = serde_json::json!({ "tjuae": eng });
        }
    ...
    fn make_loaded_ext(
        ...
        LoadedExtension {
            manifest: ExtensionManifest {
                ...
                homepage: None,
                icon: None,
                engine: engine_tjuae.map(|v| EngineConfig {
                    tjuae: Some(v.to_string()),
                }),
    ...
    #[test]
    fn resolve_scan_paths_for_data_dir_prefers_env_then_data_dir() {
        let tmp = tempfile::TempDir::new().unwrap();
        let app_root = tmp.path().join("TjuaeUI-Dev");
        let data_dir = app_root.join("tjuaeui");
    ...
    #[test]
    fn resolve_scan_paths_for_data_dir_uses_only_canonical_local_path() {
        let tmp = tempfile::TempDir::new().unwrap();
