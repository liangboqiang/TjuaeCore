        ...
        Ok(Ok(output)) => {
            if output.status.success() {
                ...
                Err(ExtensionError::HookFailed {
                    extension_name: extension_name.to_owned(),
                    hook: label.to_owned(),
                    reason: format!("退出码 {code}：{}", stderr.trim()),
                })
            }
        }
    }
...
mod tests {
    ...
    #[cfg(windows)]
    #[tokio::test]
    async fn test_execute_hook_windows_cmd_success() {
        let dir = tempfile::tempdir().unwrap();
        let marker = dir.path().join("cmd_marker.txt");
        let script_path = dir.path().join("hook.cmd");
        std::fs::write(&script_path, "@echo off\r\necho ok>cmd_marker.txt\r\n").unwrap();

        let result = execute_hook(dir.path(), "hook.cmd", HookKind::OnActivate, "test-ext").await;
