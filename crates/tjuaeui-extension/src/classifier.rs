...
//!
//! These traits live in `tjuaeui-extension` (not `tjuaeui-assistant`) so
//! `skill_routes` can depend on them without pulling `tjuaeui-assistant` into
//! the dependency graph; the concrete implementation ships from
//! `tjuaeui-assistant::AssistantService`.

use tjuaeui_api_types::AssistantSource;
...
///
/// Implemented by `tjuaeui_assistant::AssistantService`; depended on by
