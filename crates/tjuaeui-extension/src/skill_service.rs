...
use tjuaeui_db::{CreateSkillImportRecordParams, ISkillRepository, SkillRow, UpsertSkillParams};
...
///
/// Mirrors the strategy used by `tjuaeui-assistant::builtin`: the corpus is
/// authoritative at build time; an optional on-disk override
/// (`TJUAE_BUILTIN_SKILLS_PATH`) is consulted at runtime for rapid
/// iteration and E2E fixtures.
static BUILTIN_SKILLS: Dir<'static> = include_dir!("$CARGO_MANIFEST_DIR/../tjuaeui-app/assets/builtin-skills");
...
pub const BUILTIN_SKILLS_ENV_VAR: &str = "TJUAE_BUILTIN_SKILLS_PATH";
...
pub struct SkillPaths {
    ...
    /// Assistant-level rules directory (~/.tjuaeui/assistant-rules/).
    pub assistant_rules_dir: PathBuf,
    /// Assistant-level skills directory (~/.tjuaeui/assistant-skills/).
...
///
/// `data_dir` is the user-level data root (e.g. `~/.tjuaeui/`) and
...
async fn copy_skill_into_user_dir(paths: &SkillPaths, skill_path: &Path) -> Result<CopiedSkill, ExtensionError> {
    ...
    tokio::fs::create_dir_all(&paths.user_skills_dir)
        .await
        .map_err(|err| skill_import_io_error("创建用户技能目录", &paths.user_skills_dir, err))?;
    ...
    commit_skill_import_directory(&staging_dir, &target_dir).await?;
...
pub async fn import_skills(paths: &SkillPaths, source_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    let operation_id = tjuaeui_common::generate_prefixed_id("skill_import_op");
...
async fn import_skills_from_zip(paths: &SkillPaths, archive_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    ...
    let result = async {
        ...
        import_skill_dirs_batch(
            paths,
            skill_dirs,
            tjuaeui_common::generate_prefixed_id("skill_import_op"),
        )
        .await
    }
    .await;
...
async fn replace_existing_path(path: &Path) -> Result<(), ExtensionError> {
    ...
    if metadata.file_type().is_symlink() {
        // Windows 对目录链接（包括目标已失效的目录联接）要求使用
        // RemoveDirectoryW；remove_file 会返回 AccessDenied。
        match tokio::fs::remove_dir(path).await {
            Ok(()) => return Ok(()),
            Err(dir_err) if dir_err.kind() == io::ErrorKind::NotADirectory => {
                tokio::fs::remove_file(path)
                    .await
                    .map_err(|err| skill_import_io_error("删除已有文件链接", path, err))?;
                return Ok(());
            }
            Err(err) => return Err(skill_import_io_error("删除已有目录链接", path, err)),
        }
    }
    ...
    if metadata.file_type().is_symlink() || metadata.is_file() {
        tokio::fs::remove_file(path)
            .await
            .map_err(|err| skill_import_io_error("删除已有文件或链接", path, err))?;
    } else {
        tokio::fs::remove_dir_all(path)
            .await
            .map_err(|err| skill_import_io_error("删除已有目录", path, err))?;
    }

    Ok(())
}

fn skill_import_io_error(action: &str, path: &Path, err: io::Error) -> ExtensionError {
    ExtensionError::Io(io::Error::new(
        err.kind(),
        format!("{action}失败（{}）：{err}", path.display()),
    ))
}

async fn commit_skill_import_directory(staging_dir: &Path, target_dir: &Path) -> Result<(), ExtensionError> {
    // Windows 的安全软件或文件索引器可能会在刚复制完文件时短暂持有目录句柄，
    // 此时原子重命名会返回 PermissionDenied。有限重试可以保留原子提交语义，
    // 同时避免把瞬时占用误报为导入失败。
    const MAX_ATTEMPTS: usize = 8;
    for attempt in 0..MAX_ATTEMPTS {
        match tokio::fs::rename(staging_dir, target_dir).await {
            Ok(()) => return Ok(()),
            Err(err) if err.kind() == io::ErrorKind::PermissionDenied && attempt + 1 < MAX_ATTEMPTS => {
                tokio::time::sleep(std::time::Duration::from_millis(25 * (attempt as u64 + 1))).await;
            }
            Err(err) => return Err(skill_import_io_error("提交技能导入目录", target_dir, err)),
        }
    }
    unreachable!("重试循环总会返回结果")
}
...
async fn copy_skill_dir_for_import(
    ...
    tokio::fs::create_dir_all(dst)
        .await
        .map_err(|err| skill_import_io_error("创建技能导入目录", dst, err))?;

    let mut entries = tokio::fs::read_dir(src)
        .await
        .map_err(|err| skill_import_io_error("读取技能源目录", src, err))?;
    while let Some(entry) = entries
        .next_entry()
        .await
        .map_err(|err| skill_import_io_error("遍历技能源目录", src, err))?
    {
        ...
        let metadata = tokio::fs::symlink_metadata(&entry_path)
            .await
            .map_err(|err| skill_import_io_error("读取技能源文件信息", &entry_path, err))?;
        if metadata.file_type().is_symlink() {
        ...
        tokio::fs::copy(&entry_path, &dest_path)
            .await
            .map_err(|err| skill_import_io_error("复制技能文件", &entry_path, err))?;
    }
...
mod tests {
    ...
    use tjuaeui_db::{ISkillRepository, SqliteSkillRepository, init_database_memory};
    ...
    async fn import_skill_copies_small_files_without_name_based_filtering() {
        ...
        .unwrap();
        std::fs::write(source_dir.join("notes.tmp"), "small auxiliary file").unwrap();
        std::fs::create_dir_all(source_dir.join(".metadata")).unwrap();
        std::fs::write(source_dir.join(".metadata/config"), "repo metadata").unwrap();
        ...
        assert!(imported.join("notes.tmp").exists());
        assert!(imported.join(".metadata/config").exists());
    }
    ...
    async fn import_skills_replaces_dangling_link_with_copy() {
        ...
        assert_eq!(outcome.imported, vec!["dangling"], "导入失败详情：{:?}", outcome.failed);
        assert!(outcome.failed.is_empty());
