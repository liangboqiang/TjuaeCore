...
mod windows_tests {
    ...
    #[test]
    fn canonicalizes_windows_file_uri_lexically() {
        assert_eq!(
            canonicalize("file:///C:/Work/App/../Project/").unwrap(),
            canonicalize("file:///c:/work/project").unwrap()
        );
    }

    #[test]
    fn windows_path_uri_roundtrip_and_basename() {
        let path = Path::new(r"C:\Users\Tester\Tjuae");
        let canonical = canonicalize(&to_file_uri(path).unwrap()).unwrap();
        assert_eq!(basename(&canonical), "tjuae");
        let roundtrip = canonicalize(&to_file_uri(&fs_path(&canonical).unwrap()).unwrap()).unwrap();
