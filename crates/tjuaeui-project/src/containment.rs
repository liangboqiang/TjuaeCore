...
mod windows_tests {
    ...
    #[test]
    fn resolves_windows_child_and_rejects_escape() {
        let root = canonicalize("file:///C:/work/proj").unwrap();
        let child = resolve_relative(&root, "src/main.rs", FileOp::Read).unwrap();
        assert_eq!(child.relative_path, "src/main.rs");
        assert_eq!(child.resource_uri, "file:///C:/work/proj/src/main.rs");
        assert_eq!(
